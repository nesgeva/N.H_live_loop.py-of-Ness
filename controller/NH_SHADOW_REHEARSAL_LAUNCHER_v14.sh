...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR20B_V14_3fbb204524348b7e9d32409a81d417e817aec5a3a3239b87e215313490ea5963_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
capture_original_proof() {
  ...
  #     this file must stay byte-identical to itself across the run ---
  {
    for lf in "$LAUNCHER_V1_BASENAME" "$LAUNCHER_V2_BASENAME" \
              ...
              "$LAUNCHER_V13_BASENAME" "$LAUNCHER_V14_BASENAME"; do
      if [ -f "${CTRL}/${lf}" ]; then
...
for _plab in "$PRESERVED_LAB_V1" "$PRESERVED_LAB_REPAIR9" \
             ...
             "$PRESERVED_LAB_REPAIR19B_V13"; do
  if [ "$LAB" = "$_plab" ]; then
...
refresh_new_copy_controller() {
  local target="${SHADOW_WS}/controller/nh_loop.py"
  local staging="${SHADOW_WS}/controller/.nh_loop.py.repair20b-refresh"

  # THE REFRESH SOURCE IS THE ACCEPTED REPAIR-20B CANDIDATE, NOT THE REAL
  # CONTROLLER. It is asserted immediately, at the moment it is about to be read,
  # and it must be an ordinary non-symlink file: a symlink here could redirect
  # the copy to any file on the system.
  [ -L "$R20B_CANDIDATE" ] && die "the Repair-20B candidate is a SYMLINK:
        ${R20B_CANDIDATE}
      Refusing to refresh the shadow through it."
  [ -f "$R20B_CANDIDATE" ] \
    || die "the accepted Repair-20B candidate is missing at ${R20B_CANDIDATE}"
  expect "refresh source sha256 (Repair 20B candidate)" \
         "$(sha_of "$R20B_CANDIDATE")"   "$EXP_R20B_SHA"
  expect "refresh source bytes  (Repair 20B candidate)" \
         "$(bytes_of "$R20B_CANDIDATE")" "$EXP_R20B_BYTES"
  expect "refresh source lines  (Repair 20B candidate)" \
         "$(wc -l < "$R20B_CANDIDATE")"  "$EXP_R20B_LINES"
  ...
  cp -- "$R20B_CANDIDATE" "$staging" \
    || die "could not stage the Repair-20B candidate into the new shadow copy:
        ${staging}
      Nothing has been renamed over the new copy's controller."
  chmod 0644 -- "$staging" \
    || die "could not set the mode on the staged Repair-20B candidate: ${staging}"
  mv -f -- "$staging" "$target" \
    || die "could not move the staged Repair-20B candidate into place:
        ${target}"
  CONTROLLER_REFRESH="PERFORMED"
  note "the new copy's controller/nh_loop.py refreshed to the Repair-20B candidate"
}
...
assert_new_copy_after_refresh() {
  ...
  expect "new copy controller sha256 AFTER refresh (Repair 20B)" \
         "$(sha_of "$target")"   "$EXP_R20B_SHA"
  expect "new copy controller bytes  AFTER refresh (Repair 20B)" \
         "$(bytes_of "$target")" "$EXP_R20B_BYTES"
  expect "new copy controller lines  AFTER refresh (Repair 20B)" \
         "$(wc -l < "$target")"  "$EXP_R20B_LINES"
  expect "real controller sha256 unchanged AFTER refresh (Repair 20)" \
         "$(sha_of "${CTRL}/nh_loop.py")" "$EXP_LOOP_SHA"
  note "the refreshed shadow controller is the exact Repair-20B identity"
}
...
assert_new_copy_differs_only_by_controller() {
  ...
  cmp_out="$(
    PYTHONDONTWRITEBYTECODE=1 "$PY3" -B -c "$TREE_COMPARATOR" \
      ...
      "NEWCOPY" \
      "SEED" \
      "DIFFERS_EXACT:${REFRESH_ALLOWED_EXACT_LOOP}:${EXP_R20B_SHA}:${EXP_SEED_LOOP_SHA}" 2>&1
  )"
  cmp_rc=$?
  set -e
...
${cmp_out}
      ...
      controller that is not the exact Repair-19B/Repair-17 pair.
...
${cmp_out}
      ...
      Repair-17 controller while reporting a Repair-19B run."
      ;;
    *)
...
assert_frozen_identities() {
  ...
  # file that is present, and it is carried in the before/after proof set so any
  # change to it during the run is caught.
  [ -s "${CTRL}/${LAUNCHER_V14_BASENAME}" ] \
    || die "launcher v14 is missing or empty at ${CTRL}/${LAUNCHER_V14_BASENAME}"
  note "all fourteen launcher artefacts present in ${CTRL}"
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${R20B_CANDIDATE_BASENAME}" | LC_ALL=C sort
  )"
...
SHADOW_TOKEN="nh-shadow-repair20b-regression-v14-${EXP_R20B_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...
step "REPAIR-20B REGRESSION V14 OBSERVATIONS"

observe_repair20b_regression_v14() {
  local out="${PROOF}/repair20b-regression-v14-observations.txt"
  ...
  {
    printf 'REPAIR-20B REGRESSION V14 OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================================\n\n'
    ...
    printf '  refreshed to      : %s  (Repair 20B candidate)\n' "$EXP_R20B_SHA"
  ...
  note "repair-19b regression v13 observations written to ${out}"
}
observe_repair20b_regression_v14 || true

# --------------------------------------------------------------------------
# THE ONE NEW v14 OBSERVATION -- THE EXACT REPAIR-20B REGRESSION
# --------------------------------------------------------------------------
#
...
step "REPAIR-20B EXACT REGRESSION OBSERVATION"

observe_repair20b_exact_regression() {
  local out="${PROOF}/repair20b-exact-regression.txt"
  local s1="$S1_OUT" s1e="${PROOF}/${STAGE1_SLUG}.stderr"
  local b9_hit="no" zero_hit="no" issue_hit="no" kind_hit="no" recurred="false"
  local b9_needle="NH_B9_RETRY_VALUES_WIRING_INTO_B9_B10_BHOLD_B24_PACKAGE_COMPLETE_CLOSURE_RECORD_v1_0.md"

  grep -q -F -- "$b9_needle" "$s1" 2>/dev/null && b9_hit="yes"
  grep -q -F -- "$b9_needle" "$s1e" 2>/dev/null && b9_hit="yes"
  grep -q -F -- "0 bounded source record(s)" "$s1" 2>/dev/null && zero_hit="yes"
  grep -q -F -- "0 bounded source record(s)" "$s1e" 2>/dev/null && zero_hit="yes"
  grep -q -F -- "authority_control_retry_values_scope" "$s1" 2>/dev/null && issue_hit="yes"
  grep -q -F -- "authority_control_retry_values_scope" "$s1e" 2>/dev/null && issue_hit="yes"
  grep -q -F -- "wrapped_package_status_statement" "$s1" 2>/dev/null && kind_hit="yes"
  grep -q -F -- "wrapped_package_status_statement" "$s1e" 2>/dev/null && kind_hit="yes"

  # The exact recurrence requires ALL THREE halves of the v13 failure together:
  # that issue, that file, and that "0 bounded source record(s)" claim.
  if [ "$issue_hit" = "yes" ] && [ "$b9_hit" = "yes" ] && [ "$zero_hit" = "yes" ]; then
    recurred="true"
  fi

  {
    printf 'REPAIR-20B EXACT REGRESSION OBSERVATION -- descriptive only, no verdict\n'
    printf '======================================================================\n\n'
    printf 'WHAT THIS OBSERVES\n'
    printf '  the exact v13 Stage-1 refusal, and whether it recurs under the\n'
    printf '  Repair-20B controller. It assigns no verdict and passes nothing.\n\n'
    printf 'CONTROLLER UNDER TEST\n'
    printf '  shadow controller  : %s  (Repair 20B candidate)\n' "$EXP_R20B_SHA"
    ...
    printf 'THE EXACT v13 REFUSAL, COMPONENT BY COMPONENT\n'
    printf '  issue authority_control_retry_values_scope present : %s\n' "$issue_hit"
    printf '  accepted B9 retry-values closure record present    : %s\n' "$b9_hit"
    printf '  "0 bounded source record(s)" present               : %s\n' "$zero_hit"
    printf '  wrapped_package_status_statement present           : %s\n\n' "$kind_hit"
    printf 'REPAIR20B_EXACT_REGRESSION_RECURRED = %s\n\n' "$recurred"
    printf 'STAGE 2 question-coverage-review\n'
    printf '  reached                       %s\n' "$COVERAGE_RC"
    printf '  coverage_review_cleared       %s\n' "$OBS_S2_CLEARED"
    printf '  coverage_possible_gaps        %s\n\n' "$OBS_S2_POSSIBLE_GAPS"
    printf 'STAGE 3 interview-gate\n'
    printf '  exit                          %s\n' "$GATE_RC"
    printf '  mechanical_path_unlocked      %s\n\n' "$OBS_S3_UNLOCKED"
    printf 'STAGE 4 execute-next-claude-task\n'
    printf '  exit                          %s\n' "$EXEC_RC"
    printf '  stop_reason                   %s\n\n' "$OBS_S4_STOP_REASON"
    printf 'HOW TO READ THIS SECTION\n'
    printf '  recurred=true means the EXACT v13 technical refusal happened again.\n'
    printf '  recurred=false means those three components did not all appear. It\n'
    printf '  is NOT a pass: Stage 1 may have failed for an unrelated reason or\n'
    printf '  may not have run. Read the Stage-1 exit and stop reason above, and\n'
    printf '  the launcher RESULT block, which this section does not influence.\n'
    printf '  No semantic classification is made here and none may be inferred.\n'
  } > "$out" 2>/dev/null || true

  REPAIR20B_EXACT_REGRESSION_RECURRED="$recurred"
  printf '%s\n' "REPAIR20B_EXACT_REGRESSION_RECURRED=${recurred}"
}

observe_repair20b_exact_regression || true
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v14, REPAIR-20B REGRESSION) — RESULT\n'
printf '===========================================================\n'
printf 'SHADOW_LAB=%s\n'                        "$LAB"
printf 'REGRESSION_SOURCE=%s\n'                 "$REGRESSION_SOURCE"
printf 'REGRESSION_SOURCE_MODE=%s\n'            "$REGRESSION_SOURCE_MODE"
...
printf 'PROOF_V14_OBSERVATIONS=%s\n'                 "${PROOF}/repair20b-regression-v14-observations.txt"
printf 'PROOF_REPAIR20B_REGRESSION=%s\n'             "${PROOF}/repair20b-exact-regression.txt"
printf '===========================================================\n'
...
printf '  controller/nh_loop.py inside the disposable v13 shadow was replaced\n'
printf '  with the current real Repair-19B file, which was opened read-only as\n'
printf '  the source and was never written to. The historical seed still carries\n'
printf '  its own Repair-17 controller. The seven-event journal, its head and the\n'
