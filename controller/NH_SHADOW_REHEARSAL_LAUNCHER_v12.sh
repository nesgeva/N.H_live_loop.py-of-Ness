#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v12.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v12
...
#
# ==========================================================================
# WHAT v12 IS, AND WHAT IT CHANGES
# ==========================================================================
#
...
# Twelve launcher files now live in controller/. All twelve are accounted for
# explicitly: eleven by exact identity, and this one by presence.
...
# v1..v11 are ALL preserved unchanged, and all eleven are frozen here so this
# launcher fails closed if any of them moved. Each is historical validation
# evidence in its own right.
#
# v11 is this file's DIRECT PREDECESSOR and is preserved evidence in its own
# right. It RAN, exactly once, against the Repair-18 controller over the
# preserved seven-event state, and its lab holds the exact live failure v12
# regresses. Nothing about v11 is retracted by v12 existing: v11 was correct for
# the controller it was frozen to, and the rule it applied is wrong only under
# Repair 19B, which did not exist when it was written.
...
# --- controller: THE REPAIR-19B IDENTITY ---
#
# This is the CURRENT REAL controller, the one Ness accepted after the
# independent ChatGPT audit of Repair 19B, and it is the file this launcher
# refreshes the NEW SHADOW COPY to. It is never modified by this launcher and is
# opened read-only on every path.
readonly EXP_LOOP_SHA="235885af469a6b5f3f428d206c77072712ef4b8d68caa37acf43445fc321bf02"
readonly EXP_LOOP_BYTES="1825807"
readonly EXP_LOOP_LINES="39774"
...
# --- lab: NEW, keyed to REPAIR19B, to V12, to the exact Repair-19B controller
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR19B_V12_235885af469a6b5f3f428d206c77072712ef4b8d68caa37acf43445fc321bf02_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
capture_original_proof() {
  ...
  # --- all twelve launcher artefacts: v1-v11 must stay byte-identical, and this
  #     file must stay byte-identical to itself across the run ---
  {
    for lf in "$LAUNCHER_V1_BASENAME" "$LAUNCHER_V2_BASENAME" \
              ...
              "$LAUNCHER_V11_BASENAME" "$LAUNCHER_V12_BASENAME"; do
      if [ -f "${CTRL}/${lf}" ]; then
...
for _plab in "$PRESERVED_LAB_V1" "$PRESERVED_LAB_REPAIR9" \
             ...
             "$PRESERVED_LAB_REPAIR18_CONTINUATION_V11"; do
  if [ "$LAB" = "$_plab" ]; then
...
refresh_new_copy_controller() {
  local target="${SHADOW_WS}/controller/nh_loop.py"
  local staging="${SHADOW_WS}/controller/.nh_loop.py.repair19b-refresh"
  ...
  expect "refresh source controller sha256 (Repair 19B)" \
         "$(sha_of "${CTRL}/nh_loop.py")"   "$EXP_LOOP_SHA"
  expect "refresh source controller bytes  (Repair 19B)" \
         "$(bytes_of "${CTRL}/nh_loop.py")" "$EXP_LOOP_BYTES"
  expect "refresh source controller lines  (Repair 19B)" \
  ...
  cp -- "${CTRL}/nh_loop.py" "$staging" \
    || die "could not stage the Repair-19B controller into the new shadow copy:
        ${staging}
      Nothing has been renamed over the new copy's controller."
  chmod 0644 -- "$staging" \
    || die "could not set the mode on the staged Repair-19B controller: ${staging}"
  mv -f -- "$staging" "$target" \
    || die "could not move the staged Repair-19B controller into place:
        ${target}"
  CONTROLLER_REFRESH="PERFORMED"
  note "the new copy's controller/nh_loop.py refreshed to the Repair-19B file"
}
...
assert_new_copy_after_refresh() {
  ...
  expect "new copy controller sha256 AFTER refresh (Repair 19B)" \
         "$(sha_of "$target")"   "$EXP_LOOP_SHA"
  expect "new copy controller bytes  AFTER refresh (Repair 19B)" \
         "$(bytes_of "$target")" "$EXP_LOOP_BYTES"
  expect "new copy controller lines  AFTER refresh (Repair 19B)" \
         "$(wc -l < "$target")"  "$EXP_LOOP_LINES"
  note "the refreshed controller is the exact Repair-19B identity"
}
...
assert_new_copy_differs_only_by_controller() {
  ...
  cmp_out="$(
    PYTHONDONTWRITEBYTECODE=1 "$PY3" -B -c "$TREE_COMPARATOR" \
      ...
      "NEWCOPY" \
      "SEED" \
      "DIFFERS_EXACT:${REFRESH_ALLOWED_EXACT_LOOP}:${EXP_LOOP_SHA}:${EXP_SEED_LOOP_SHA}" 2>&1
  )"
  cmp_rc=$?
  set -e
...
assert_frozen_identities() {
  ...
  # Launcher v12 is THIS file. It cannot hash-gate on itself, but it must be the
  # file that is present, and it is carried in the before/after proof set so any
  # change to it during the run is caught.
  [ -s "${CTRL}/${LAUNCHER_V12_BASENAME}" ] \
    || die "launcher v12 is missing or empty at ${CTRL}/${LAUNCHER_V12_BASENAME}"
  note "all twelve launcher artefacts present in ${CTRL}"
  ...
  # The controller worktree must be EXACTLY these fifteen entries and nothing
  # else: the modified nh_loop.py, the twelve launcher artefacts, the gitignored
  # editor file, and the gitignored controller bytecode. Nothing is filtered out.
  #
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${LAUNCHER_V12_BASENAME}" | LC_ALL=C sort
  )"
...
#
# The token identifies FOUR things: Repair 19B, the v12 regression, the exact
...
SHADOW_TOKEN="nh-shadow-repair19b-regression-v12-${EXP_LOOP_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...
# disposable v12 shadow, which is precisely the corrected behaviour this run
# exists to observe.
#
...
# requires the v12 result to state STAGE2_MISSING_GAPS and STAGE2_OVER_ASK_GAPS,
...
#
# THIS IS THE CENTRAL PURPOSE OF v12. Under Repair 19B an admitted question is
...
# 4. THE REPAIR-19 DELIVERY GATE. THIS IS THE ONE RULE v12 EXISTS TO EXERCISE.
#
...
if ! stage_may_continue; then
  ...
  if ! stage_may_continue; then
    ...
    if ! stage_may_continue; then
      ...
      #
      # WHATEVER EXECUTE REPORTS IS THE FINAL v12 RESULT.
...

{
  ...
  printf '  questions_total               %s   <- NOT a gate in v12\n' "$OBS_S1_QUESTIONS_TOTAL"
  printf '  provisional_question_count    %s   <- NOT a gate in v12\n' "$OBS_S1_PROVISIONAL"
...
# ==========================================================================
# SECTION 11b — REPAIR-19B REGRESSION V12 OBSERVATIONS
...
step "REPAIR-19B REGRESSION V12 OBSERVATIONS"

observe_repair19b_regression_v12() {
  local out="${PROOF}/repair19b-regression-v12-observations.txt"
  ...
  {
    printf 'REPAIR-19B REGRESSION V12 OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================================\n\n'
    ...
    printf '  refreshed to      : %s  (Repair 19B)\n' "$EXP_LOOP_SHA"
  ...
  note "repair-19b regression v12 observations written to ${out}"
}
observe_repair19b_regression_v12 || true
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v12, REPAIR-19B REGRESSION) — RESULT\n'
printf '===========================================================\n'
printf 'SHADOW_LAB=%s\n'                        "$LAB"
printf 'REGRESSION_SOURCE=%s\n'                 "$REGRESSION_SOURCE"
printf 'REGRESSION_SOURCE_MODE=%s\n'            "$REGRESSION_SOURCE_MODE"
...
printf 'PROOF_V12_OBSERVATIONS=%s\n'                 "${PROOF}/repair19b-regression-v12-observations.txt"
printf '===========================================================\n'
...
printf '  controller/nh_loop.py inside the disposable v12 shadow was replaced\n'
