#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v20.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v20
#   (THE ACCEPTED REPAIR-27A CONTROLLER, FRESH INTEGRATED END-TO-END SHADOW
#    REHEARSAL, WITH THE HISTORICAL SEED LOCKED BY WHOLE-WORKSPACE IDENTITY)
#
# WHAT v20 CHANGES FROM v19, AND NOTHING ELSE
#   v19 WAS RUN, exactly once, and it completed. It is not corrected here and
#   nothing about it is retracted: it did precisely what it was built to do.
#   v20 is the SMALLEST SAFE REBASE of that completed launcher onto the accepted
#   Repair-27A controller, and it changes only what that rebase mechanically
#   requires:
#
#     1. VERSION PLUMBING. v19 -> v20 throughout the CURRENT-run identity: the
#        file name, the launcher constant set, the stage-observation artefact,
#        and every current-run report field that names THIS launcher.
#     2. THE EXACT PREDECESSOR, FROZEN. v19 is now a preserved launcher like
#        every one before it, gated by exact sha256 and byte count.
#     3. A FRESH v20 LAB AND MARKER, keyed to REPAIR27A/V20, to the exact
#        accepted Repair-27A controller identity, and to the routed signal this
#        rehearsal carries forward. No lab is ever reused.
#     4. THE COMPLETED v19 LAB, PRESERVED AS EVIDENCE. It holds the exact
#        Stage-4 preparation refusal Repair 27/27A exist to correct, so it is
#        named here for collision refusal, presence assertion and human
#        reporting, and it is never written to, never reused, never normalised
#        and never deleted.
#     5. REPAIR 26 BECOMES PRESERVED REPAIR EVIDENCE; REPAIR 27A BECOMES THE
#        SOLE REFRESH SOURCE. Repair 26 is still gated by exact identity and
#        still witnessed unmoved -- it is simply not copied anywhere. REPAIR 27
#        joins the preserved set on exactly the same terms.
#     6. THE REPAIR-27 AND REPAIR-27A CANDIDATES, each gated exactly as strictly
#        as the real controller and accounted for in the exact controller
#        worktree.
#
#   NOTHING ELSE IS REDESIGNED. The isolation model, the historical-seed
#   authentication and whole-workspace witness, the protected-original and
#   preserved-lab proofs, the credential witness, the tree comparator, the
#   report parsers, run_controller_stage(), the four-stage sequence and every
#   Stage-1/2/3 continuation rule are carried forward exactly as v19 had them.
#
# WHAT THE COMPLETED v19 RUN ACTUALLY DID
#   Its isolation self-test PASSED. Repair 26 was its sole shadow refresh
#   source. The protected real workspace, the preserved labs and the regression
#   seed all PASSED.
#
#     STAGE 1  question-validation      exit 0, ok true, inventory complete,
#                                       material_unknowns [], questions_total 0,
#                                       deliverable_question_count 0. This is
#                                       what proved accepted Repair 26 corrected
#                                       v18's 115-reference
#                                       MATERIAL_UNKNOWN_UNRESOLVED blocker.
#     STAGE 2  question-coverage-review exit 0, cleared true, 0 possible gaps.
#     STAGE 3  interview-gate           exit 0, gate unlocked true.
#     STAGE 4  execute-next-claude-task exit 1, stop_reason PREPARATION_FAILED,
#                                       0 Claude invocations, 0 Codex design
#                                       audits, 0 correction rounds.
#
#   THE STAGE-4 BLOCKER WAS A SOURCE-ENVELOPE CONTRADICTION. The Piece-3-cleared
#   historical shadow legitimately held FIVE untracked source paths, while
#   Stage 4 required the THREE package candidates to be the whole untracked
#   worktree. Repair 27 corrected that source-envelope model; ChatGPT's
#   independent audit then found two remaining problems in it -- the two
#   clearance-frozen NON-CANDIDATE baseline files were frozen by path and status
#   but not by byte identity, and an early failed resume-envelope freeze could
#   call a local fail_closed() before its definition executed, raising
#   UnboundLocalError. ACCEPTED REPAIR 27A corrected exactly those two while
#   preserving Repair 27's five-path source-envelope design. v19's lab is the
#   evidence for all of it, and it is preserved unchanged.
#
#   v19 REACHED NEITHER CLAUDE NOR A CODEX DESIGN AUDIT, and nothing here
#   claims otherwise. v18 also WAS RUN and remains preserved history. v16 and
#   v17 were NEVER RUN. All of that remains true and is still reported.
#
# WHAT v20 IS, AND WHAT IT IS NOT
#   It is the fresh INTEGRATED shadow rehearsal of the accepted Repair-27A
#   controller: the current accepted controller logic, run against the SAME
#   preserved historical pre-question shadow state v18 and v19 used, inside the
#   already-proven isolation model, to see what the real controller workflow
#   actually does now that the Stage-4 source-envelope, baseline-byte and
#   fail-closed-ordering corrections exist.
#
#   IT ASSUMES NO OUTCOME AT ALL. It does not assume Stage 4 must pass, that
#   Claude must be invoked, that a Codex design audit must run, that a
#   correction round must happen, or that the loop must reach PASS. It runs the
#   real Repair-27A controller once and records whatever the real controller
#   does. A NEW legitimate fail-closed stop is a VALID RESULT. No gate is
#   loosened to reach a later stage, and this file contains no machinery for
#   forcing the workflow past a stop.
#
#   IT IS NOT another broad controller audit, IT IS NOT a new repair, and IT IS
#   NOT a Repair-22-only regression.
#
#   REPAIRS 20B, 21, 22, 23, 24, 25, 26 AND 27 ARE PRESERVED REPAIR EVIDENCE
#   ONLY. Not one of them is a refresh source. Repair 27A is the SOLE refresh
#   source.
#
# ONE KNOWN DEPENDENCY IS DELIBERATELY LEFT OPEN
#   Accepted Repair 27A leaves the MANUAL route-audit-finding provenance
#   re-proof not yet carrying a non-empty resume baseline. That path is NOT part
#   of the normal Stage-4 execution rehearsed here. v20 never invokes it, never
#   injects a finding to force it, never repairs it, and never treats its being
#   open as a v20 failure. If Stage 4 legitimately stops at
#   CHATGPT_REVIEW_REQUIRED, this launcher stops there and records the
#   controller\047s own result.
#
# WHAT THIS IS
#   A disposable-laboratory launcher. It proves the protected original N.H /
#   controller workspace has not moved since this launcher was constructed, makes
#   an exact throwaway copy of THE PRESERVED v10 CONTINUATION SHADOW -- the same
#   controlled historical input every rehearsal since v12 has used, and the last
#   state that exists BEFORE the false v11 question was ever admitted --
#   REFRESHES EXACTLY ONE FILE IN THAT NEW COPY, and only in the new copy, to the
#   accepted Repair-25 controller, hides the real /home/ness behind a tmpfs,
#   republishes that regression copy at the EXACT normal path
#   /home/ness/NH_CLAUDE_CODEX_DESIGN_LOOP inside a Bubblewrap namespace, runs
#   the sequence
#
...
#
#   THE HISTORICAL SEED AND TODAY'S REAL WORKSPACE ARE TWO DIFFERENT FACTS. The
#   seed is the controlled rehearsal INPUT; the real current workspace is the
#   protected ORIGINAL. Each is independently frozen and independently proved.
#   They are never reconciled, never copied into one another, and never compared
#   as though they should now be identical.
#
...
#
# WHAT NESS'S ACCEPTANCE OF REPAIR 27A DOES AND DOES NOT AUTHORISE
#   Ness has explicitly accepted Repair 27A for its bounded controller-repair
#   scope and asked for it to be tested. In THIS FILE that acceptance authorises
#   exactly one thing: using nh_loop_REPAIR27A_CANDIDATE.py as the sole
#   controller refresh source for the disposable shadow below.
#
#   REPAIR 26 AND REPAIR 27 REMAIN ACCEPTED, AND IN THIS FILE BOTH ARE PRESERVED
#   EVIDENCE ONLY. Repair 27A is based on Repair 27, which is based on Repair 26;
#   neither acceptance is withdrawn by this file, and neither candidate is ever
#   copied anywhere by this file.
#
#   ONE RUN, AND ONLY AFTER AN INDEPENDENT AUDIT. Ness authorised ONE execution
#   of this exact launcher, conditional on ChatGPT independently auditing THIS
#   FILE as PASS first. This file makes no claim that such an audit has happened.
#
...
#   v20's. Every identity THIS run asserts is in SECTION 0 and nowhere else: the
#   real controller by the digest EXP_LOOP_SHA (no generation name is claimed for
#   it), the shadow refreshed to the accepted Repair-27A candidate EXP_R27A_SHA,
#   the lab and marker keyed to REPAIR27A/V20, and the seed at EXP_SEED_LOOP_SHA.
#
...
#             controller result, and v20 records it without reinterpreting it.
#
...
# Twenty launcher files now live in controller/. All twenty are accounted for
# explicitly: nineteen by exact identity, and this one by presence.
...
# v16 IS THIS FILE'S DIRECT PREDECESSOR AND IT WAS NEVER RUN.
#
# It was constructed and independently audited, and Ness did not run it: no v16
# lab exists, no v16 rehearsal result exists, and nothing in this file claims,
# requires or reads one. It is preserved launcher history exactly like v1..v15
# and is gated here by the exact identity read from disk when v17 was frozen.
...
# v17 IS THIS FILE'S DIRECT PREDECESSOR AND IT WAS NEVER RUN EITHER.
#
# It was constructed and independently audited, and that audit found the two
# confirmed defects v18 corrects -- the bytecode capture runtime blocker and the
# stale current-run Repair-22/v16 identity claims. It was therefore never
# executed: no v17 lab exists, no v17 rehearsal result exists, and nothing in
# this file claims, requires or reads one. It is preserved launcher history
# exactly like v1..v16 and is gated by the exact identity read from disk when
# v18 was frozen.
...
# v18 IS THIS FILE'S DIRECT PREDECESSOR AND IT WAS RUN, EXACTLY ONCE.
#
# It ran against the accepted Repair-25 controller over the preserved
# seven-event state, its isolation self-test PASSED, and its Stage 1 stopped
# FAIL-CLOSED with MATERIAL_UNKNOWN_UNRESOLVED before Stages 2, 3 and 4 could
# run. Its completed lab -- PRESERVED_LAB_REPAIR25_V18 below -- holds the exact
# provider-backed Stage-1 refusal Repair 26 exists to correct, so it is
# historical evidence in its own right.
#
# NOTHING ABOUT v18 IS RETRACTED BY v19 EXISTING. v18 was correct for the
# controller it was frozen to; the refusal it recorded was a real controller
# result, and Repair 26 is the bounded mechanical correction to the resolver
# class that produced it. v18 is preserved launcher history and is gated here by
# the exact identity read from disk when v19 was frozen. v19 never runs it.
...
# v19 IS THIS FILE'S DIRECT PREDECESSOR AND IT WAS RUN, EXACTLY ONCE.
#
# It ran against the accepted Repair-26 controller over the preserved
# seven-event state. Its Stage 1, Stage 2 and Stage 3 all PASSED -- which is
# what proved Repair 26 corrected v18's Stage-1 blocker -- and its Stage 4
# stopped FAIL-CLOSED with PREPARATION_FAILED, having invoked Claude ZERO times
# and run ZERO Codex design audits. Its completed lab --
# PRESERVED_LAB_REPAIR26_V19 below -- holds the exact Stage-4 source-envelope
# refusal Repair 27 and then Repair 27A exist to correct, so it is historical
# evidence in its own right.
#
# NOTHING ABOUT v19 IS RETRACTED BY v20 EXISTING. v19 was correct for the
# controller it was frozen to; the refusal it recorded was a real controller
# result. v19 is preserved launcher history and is gated here by the exact
# identity read from disk when v20 was frozen. v20 never runs it.
...
#                   and 25. EXP_LOOP_SHA equals none of them, which is exactly
#                   what "the real controller is never a refresh source and is
#                   never overwritten by one" has to mean;
...
#
# It WAS the refresh source of v14. In v20 it is preserved repair evidence only,
# exactly like Repair 21, 22, 23, 24, 25, 26 and 27: never a refresh source,
# never copied anywhere, gated by exact identity so this run can state that it
# did not move.
# Ness's earlier acceptance of Repair 20B authorised no N.H acceptance,
# adoption, Master/Map integration or implementation, and authorises nothing at
# all in this run.
...
#     (PRESERVED REPAIR EVIDENCE — NOT THE v20 REFRESH SOURCE) ---
#
...
#
# It WAS the refresh source of v16, which was never run. In v20 it is
# preserved repair evidence only, exactly like Repair 20B, 21, 23, 24, 25, 26
# and 27: never a refresh source, never copied anywhere, gated by exact identity
# so this run can state that it did not move.
#
...
# (I-03..I-07). In v20 it is PRESERVED EVIDENCE ONLY. It is never the refresh
# source, is never copied anywhere, and is gated by exact identity purely so
# this run can state that it did not move.
...
# defect. In v20 it is PRESERVED EVIDENCE ONLY, on exactly the same terms as
# Repair 23: never a refresh source, never copied, gated only so this run can
# witness it unmoved.
...
#
# It WAS the refresh source of v18, the completed rehearsal this file succeeds.
# Repair 25 corrected the execute-next-claude-task CHATGPT_REVIEW_REQUIRED stop
# so that a blocking chatgpt_review_required audit finding no longer reports
# itself as an already-made Piece-3 transition and no longer hands the operator
# route-audit-finding and question-validation as immediate commands. That
# correction stands, it is still accepted, and accepted Repair 26 is built
# directly on top of it.
#
# IN v20 REPAIR 25 IS PRESERVED REPAIR EVIDENCE ONLY, on exactly the same terms
# as Repair 20B, 21, 22, 23, 24, 26 and 27: it is gated by exact identity,
# witnessed in the before/during/after sweep, and never written to -- but it is
# NOT copied into the shadow by this launcher on any path. Its exact preserved
# identity is reported so this run can state that it did not move.
...
#
# It WAS the refresh source of v19, the completed rehearsal this file succeeds,
# and it did its job: v19's Stage 1, Stage 2 and Stage 3 all passed under it,
# which is exactly what proved Repair 26 corrected v18's 115-reference
# repository-reference blocker. That correction stands and is still accepted.
#
# IN v20 REPAIR 26 IS PRESERVED REPAIR EVIDENCE ONLY, on exactly the same terms
# as every earlier repair: gated by exact identity, witnessed in the
# before/during/after sweep, never written to, and NOT copied into the shadow by
# this launcher on any path.
...
#
# Repair 27 is the bounded correction to the Stage-4 source-envelope
# contradiction v19 exposed: it separated CANDIDATE CUSTODY from the complete
# clearance-frozen source worktree, so the five untracked paths Piece 3 had
# already validated stopped being unexplained entries the moment Stage 4 began.
#
# ChatGPT's independent audit then found two remaining problems in it, and
# accepted Repair 27A corrects exactly those two. Repair 27 is therefore
# PRESERVED REPAIR EVIDENCE in v20 -- never a refresh source, never copied
# anywhere, gated by exact identity so this run can state that it did not move.
...
# --- controller: THE ACCEPTED REPAIR-27A CANDIDATE -- THE SOLE v20 REFRESH SOURCE
#
# THIS IS THE ONE FILE v20 EXISTS TO EXERCISE, and it is the ONLY controller
# refresh source anywhere in this launcher.
#
# WHAT CHANGED BETWEEN v19 AND v20, AND IT IS THIS.
#
# v19 refreshed its shadow to Repair 26 and RAN. Repair 26 did its job -- Stages
# 1, 2 and 3 all passed under it. v19 then stopped at Stage 4, fail-closed, with
# PREPARATION_FAILED, having invoked Claude zero times and run zero Codex design
# audits, and that one refusal is the whole reason this launcher exists: the
# Piece-3-cleared historical shadow legitimately held FIVE untracked source
# paths, while Stage 4 demanded that the THREE package candidates be the entire
# untracked worktree.
#
# REPAIR 27 CORRECTED THAT SOURCE-ENVELOPE MODEL. ACCEPTED REPAIR 27A IS THE
# BOUNDED CORRECTION TO THE TWO PROBLEMS THE INDEPENDENT AUDIT THEN FOUND IN
# REPAIR 27, AND TO NOTHING ELSE:
#   * the two clearance-frozen NON-CANDIDATE baseline files were frozen by path
#     and Git status but NOT by byte identity, so a same-path, still-untracked
#     file whose contents changed kept an identical "?? path" line and passed
#     every fence. Repair 27A freezes their exact sha256 and byte length and
#     re-proves them across the whole normal Stage-4 lifetime;
#   * an early failed resume-envelope freeze could call a local fail_closed()
#     before that local definition executed, raising UnboundLocalError instead
#     of returning the ordinary fail-closed result. Repair 27A moves the
#     definition ahead of every caller, unchanged.
# Repair 27's five-path source-envelope design is preserved intact.
#
# THIS LAUNCHER TESTS THAT CORRECTION AND NOTHING BROADER. It implements neither
# rule, interprets neither, and contains no source-envelope, baseline-identity
# or fence logic in any executable path. All of it lives inside nh_loop.py.
#
# Ness has ACCEPTED Repair 27A for its bounded controller-repair scope.
# Acceptance authorises this shadow refresh and nothing else: the real
# controller is not installed to, and this candidate is copied into the NEW
# DISPOSABLE COPY only. It is never renamed to nh_loop.py on the real
# filesystem, never copied over the real controller, and never promoted.
#
...
# compiled from the real controller as it then stood. Carrying v11's absence
...
# RE-FROZEN FOR v17: THERE ARE NOW THREE .pyc FILES, NOT ONE.
#
# v16 froze a single bytecode file because a single one existed then. Two more
# exist now -- compiled from the Repair-23 and Repair-24 candidates by ordinary
# offline verification work -- and they are legitimate present state. Carrying
# v16's single-file expectation forward would fail this run closed on files that
# are simply there, and deleting them would be a write to the protected
# original, which this launcher never performs on any path.
#
...
# overwrite. The seed controller must be EXACTLY the digest below, and the real
# controller must be EXACTLY EXP_LOOP_SHA; either side being anything else fails
# the run CLOSED. Both are asserted BY DIGEST. No generation name is asserted for
# the real controller anywhere -- see the EXP_LOOP_* note in section 0.
...
# --- lab: NEW, keyed to REPAIR27A, to V20, to the exact accepted Repair-27A
...
#
# THE COMPLETED v18 AND v19 LAB PATHS ARE NOT THIS PATH AND ARE NEVER TOUCHED.
# Both DID run, so both labs exist; they are PRESERVED_LAB_REPAIR25_V18 and
# PRESERVED_LAB_REPAIR26_V19 below, both are evidence, and this launcher will
# neither reuse nor delete either.
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR27A_V20_53902f6fc020141dc567870ba75188f1622bc7241a022a5d36f465a6559ba43a_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
# The COMPLETED v18 rehearsal -- THIS FILE'S DIRECT PREDECESSOR, and the run
# whose result Repair 26 answers. It holds the exact provider-backed Stage-1
# MATERIAL_UNKNOWN_UNRESOLVED refusal that the accepted Repair-26 resolver
# correction exists to remove, so it is historical evidence in its own right.
#
# v19 must never collide with it, never write into it, never reuse it, never
# normalise it, never derive a seed from it and never delete it, and must
# witness that it did not move. It sits under /home/ness as an NH_AICP_*
# artefact, so the before/during/after preserved-artefact sweep already covers
# it entry by entry; it is ALSO named here so the collision refusal, the
# presence assertion and the human report can speak about it directly.
#
# IT IS NOT THIS RUN'S SEED. The regression source stays the preserved Repair-17
# CONTINUATION v10 shadow, exactly as it was for v18 -- see REGRESSION_SOURCE
# below. Seeding from v18's own completed lab would rehearse the aftermath of
# the refusal instead of the state that produces it.
...
# The COMPLETED v19 rehearsal -- THIS FILE'S DIRECT PREDECESSOR, and the run
# whose result Repair 27 and then accepted Repair 27A answer. Its Stages 1, 2
# and 3 all passed under Repair 26; its Stage 4 stopped fail-closed with
# PREPARATION_FAILED on the source-envelope contradiction, having invoked Claude
# zero times and run zero Codex design audits. It is historical evidence in its
# own right.
#
# v20 must never collide with it, never write into it, never reuse it, never
# normalise it, never derive a seed from it and never delete it, and must
# witness that it did not move. It sits under /home/ness as an NH_AICP_*
# artefact, so the before/during/after preserved-artefact sweep already covers
# it entry by entry; it is ALSO named here so the collision refusal, the
...
#
# IT IS NOT THIS RUN'S SEED. The regression source stays the preserved Repair-17
# CONTINUATION v10 shadow, exactly as it was for v18 and v19 -- see
# REGRESSION_SOURCE below.
...
# The point of this rule -- introduced in v13 and preserved unchanged ever
# since -- is that the historical expectation must PRE-DATE this correction. Walking the seed now and freezing whatever is found would bless any
...
# invariant. Accepted Repair 26 is built on accepted Repair 25 and INHERITS that
# behaviour unchanged; Repair 26 did not invent it, and renaming the field to
# Repair 26 would claim otherwise. The controller under test in this run is
# Repair 26; the invariant being observed is Repair 25's.
#
...
# The completed v18 lab is THE DIRECT PREDECESSOR EVIDENCE for THIS run: v18 RAN
...
if [ "$PRESERVED_LAB_REPAIR25_V18" = "$REGRESSION_SOURCE" ] \
  ...
  die "the completed v18 lab is being used as this run's regression seed:
        ${REGRESSION_SOURCE}
      v19 seeds from the preserved Repair-17 CONTINUATION v10 shadow, exactly as
      v18 did. Refusing to rehearse the aftermath of the refusal instead of the
      state that produces it."
fi
note "the completed v18 lab is present, is not this run's lab and is not its seed"

# The completed v19 lab is THE DIRECT PREDECESSOR EVIDENCE for THIS run: v19 RAN
...
if [ "$PRESERVED_LAB_REPAIR26_V19" = "$REGRESSION_SOURCE" ] \
  ...
  die "the completed v19 lab is being used as this run's regression seed:
        ${REGRESSION_SOURCE}
      v20 seeds from the preserved Repair-17 CONTINUATION v10 shadow, exactly as
      v18 and v19 did. Refusing to rehearse the aftermath of the refusal instead
      of the state that produces it."
fi
...
#
# The preserved v10 shadow was run with the Repair-17 controller, which the seed
# proves by digest. This launcher must therefore NOT simply run the byte-for-byte
# v10 shadow unchanged -- that would rehearse Repair 17 again and would test
# nothing the later repairs changed -- and must NOT copy the real workspace
# instead, which would discard the seven-event state and the unresolved routed
# signal with it. (The real controller's own generation name is deliberately not
# asserted anywhere: see the EXP_LOOP_* note in section 0. Only its digest is
# proved, and it is never a refresh source or destination on any path.)
#
...
#   AFTER      the refreshed file is the EXACT accepted Repair-27A identity --
#              sha256,
...
#
# THE REFRESH SOURCE IS THE ACCEPTED REPAIR-27A CANDIDATE, OPENED READ-ONLY.
#
# REBASED IN v20. The correction v18 made to this paragraph stands; only the
# candidate it names moves forward. EXP_LOOP_* is the REAL controller and has
# not been the refresh source since v14; in v20 the refresh source is
# ${R27A_CANDIDATE} and nothing else.
#
# WHAT IS ACTUALLY TRUE HERE:
#   * ${R27A_CANDIDATE} is the ONLY copy source. It is opened read-only, never
...
refresh_new_copy_controller() {
  local target="${SHADOW_WS}/controller/nh_loop.py"
  local staging="${SHADOW_WS}/controller/.nh_loop.py.repair27a-refresh"

  # THE REFRESH SOURCE IS THE ACCEPTED REPAIR-27A CANDIDATE, NOT THE REAL CONTROLLER AND
  ...
  [ -L "$R27A_CANDIDATE" ] && die "the accepted Repair-27A candidate is a SYMLINK:
        ${R27A_CANDIDATE}
      Refusing to refresh the shadow through it."
  [ -f "$R27A_CANDIDATE" ] \
    || die "the accepted Repair-27A candidate is missing at ${R27A_CANDIDATE}"
  expect "refresh source sha256 (accepted Repair 27A candidate)" \
         "$(sha_of "$R27A_CANDIDATE")"   "$EXP_R27A_SHA"
  expect "refresh source bytes  (accepted Repair 27A candidate)" \
         "$(bytes_of "$R27A_CANDIDATE")" "$EXP_R27A_BYTES"
  expect "refresh source lines  (accepted Repair 27A candidate)" \
         "$(wc -l < "$R27A_CANDIDATE")"  "$EXP_R27A_LINES"

  # AND REPAIR 20B, 21, 25, 26 AND 27 ARE RE-ASSERTED UNCHANGED AT THE SAME
  # MOMENT. All five are preserved repair evidence, none is the source, and
  # proving them here is what makes "the shadow was refreshed to Repair 27A and
  # to nothing else" measurable rather than merely asserted.
  ...
  cp -- "$R27A_CANDIDATE" "$staging" \
    || die "could not stage the accepted Repair-27A candidate into the new shadow copy:
        ${staging}
      Nothing has been renamed over the new copy's controller."
  chmod 0644 -- "$staging" \
    || die "could not set the mode on the staged Repair-27A candidate: ${staging}"
  mv -f -- "$staging" "$target" \
    || die "could not move the staged Repair-27A candidate into place:
        ${target}"
  CONTROLLER_REFRESH="PERFORMED"
  note "the new copy's controller/nh_loop.py refreshed to the accepted Repair-27A candidate"
}
...
assert_new_copy_after_refresh() {
  ...
  expect "new copy controller sha256 AFTER refresh (accepted Repair 27A candidate)" \
         "$(sha_of "$target")"   "$EXP_R27A_SHA"
  expect "new copy controller bytes  AFTER refresh (accepted Repair 27A candidate)" \
         "$(bytes_of "$target")" "$EXP_R27A_BYTES"
  expect "new copy controller lines  AFTER refresh (accepted Repair 27A candidate)" \
         "$(wc -l < "$target")"  "$EXP_R27A_LINES"
  ...
  expect "Repair27 unchanged AFTER refresh (preserved, not the source)" \
         "$(sha_of "$R27_CANDIDATE")" "$EXP_R27_SHA"
  note "the refreshed shadow controller is the exact accepted Repair-27A identity"
}
...
assert_new_copy_differs_only_by_controller() {
  ...
  cmp_out="$(
    PYTHONDONTWRITEBYTECODE=1 "$PY3" -B -c "$TREE_COMPARATOR" \
      ...
      "NEWCOPY" \
      "SEED" \
      "DIFFERS_EXACT:${REFRESH_ALLOWED_EXACT_LOOP}:${EXP_R27A_SHA}:${EXP_SEED_LOOP_SHA}" 2>&1
  )"
  cmp_rc=$?
  set -e
...
${cmp_out}
      ...
      controller that is not the exact Repair-27A/Repair-17 pair.
...
${cmp_out}
      ...
      Repair-17 controller while reporting a Repair-27A run."
      ;;
    *)
...
assert_frozen_identities() {
  ...
  # THE ACCEPTED REPAIR-27A CANDIDATE -- THE SOLE v20 REFRESH SOURCE, gated exactly
  # as strictly as the real controller and equally never written to. It is the one
  # file this rehearsal exists to exercise, and it is asserted here as a
  ...
  [ -f "$R27A_CANDIDATE" ] \
    || die "the accepted Repair-27A candidate is missing from ${CTRL}; it is the one
      file this rehearsal exists to test and must be present"
  ...
  # gated by exact identity like v1..v15. v20 never runs it.
  ...
  # identity like v1..v16. v20 never runs it.
  ...
  # by exact identity like v1..v17. v20 never runs it.
  ...
  # v19 is THIS file's DIRECT PREDECESSOR AND IT RAN, exactly once, against the
  # accepted Repair-26 controller. Its Stages 1-3 passed and its Stage 4 stopped
  ...
  # is the identity read from disk when v20 was frozen. v20 never runs it.
  ...
  # Launcher v20 is THIS file. It cannot hash-gate on itself, so it is accounted
  ...
  [ -s "${CTRL}/${LAUNCHER_V20_BASENAME}" ] \
    || die "launcher v20 is missing or empty at ${CTRL}/${LAUNCHER_V20_BASENAME}"
  note "all twenty launcher artefacts present in ${CTRL}"
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${R27A_CANDIDATE_BASENAME}" | LC_ALL=C sort
  )"
...
# ==========================================================================
# SECTION 7b — THE REPAIR-17 -> ACCEPTED-REPAIR-25 NEW-COPY CONTROLLER REFRESH
...
step "NEW-COPY CONTROLLER REFRESH: AFTER-STATE (must be the exact accepted Repair-25 file)"
assert_new_copy_after_refresh
...
#
# The token identifies FOUR things: Repair 27A, the v20 integrated rehearsal, the
# exact accepted Repair-27A controller sha256, and the routed signal this
# rehearsal carries forward. It therefore cannot collide with any earlier
# rehearsal marker.
SHADOW_TOKEN="nh-shadow-repair27a-integrated-v20-${EXP_R27A_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...
# ==========================================================================
# SECTION 11 — THE FOUR-STAGE REAL REHEARSAL SEQUENCE (ACCEPTED REPAIR 25)
# ==========================================================================
#
...
# disposable v20 shadow, which is the corrected Repair-19 behaviour this
# rehearsal expects to see.
#
...
#
# THIS RULE CAME FROM REPAIR 19 AND IS PRESERVED UNCHANGED. An admitted question
# is PROVISIONAL, so the ordinary corrected outcome is
...
if ! stage_may_continue; then
  ...
  if ! stage_may_continue; then
    ...
    if ! stage_may_continue; then
      ...
      #
      # WHATEVER EXECUTE REPORTS IS THE CONTROLLER RESULT, AND v20 RECORDS IT
      # WITHOUT REINTERPRETING IT. No launcher-side repair loop exists.
...
# --- THE REPAIR-25 SEAM FACTS, AS THE CONTROLLER REPORTED THEM (NEW IN v17) ---
#
# These are CONTROLLER FACTS. This launcher records them and invents no meaning
# for any of them. Every one is read through stage_scalar, which can set no stop
# reason and fail no gate, and not one is consulted by any continuation decision
# anywhere in this file. A field the controller did not report comes back as
# UNAVAILABLE; a stage that never ran comes back as NOT-RUN. Neither is ever
# silently rendered as a value, a count or a pass.
...

{
  ...
  printf '  questions_total               %s   <- NOT a gate\n' "$OBS_S1_QUESTIONS_TOTAL"
  printf '  provisional_question_count    %s   <- NOT a gate\n' "$OBS_S1_PROVISIONAL"
...
# ==========================================================================
# SECTION 11b — v20 STAGE OBSERVATIONS
...
step "v20 STAGE OBSERVATIONS"

observe_v20_stages() {
  local out="${PROOF}/v20-stage-observations.txt"
  ...
  {
    printf 'v20 STAGE OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================================\n\n'
    ...
    printf '  refreshed to      : %s  (accepted Repair 27A candidate)\n' "$EXP_R27A_SHA"
  ...
  note "v20 stage observations written to ${out}"
}
observe_v20_stages || true
...
observe_exact_regressions() {
  local out="${PROOF}/historical-stage1-refusals.txt"
  ...
  {
    printf 'HISTORICAL STAGE-1 REFUSAL OBSERVATION (v13 / v14 / v15 refusals)\n'
    printf '  -- descriptive only, no verdict, gates no stage\n'
    printf '======================================================================\n\n'
    printf 'WHAT THIS OBSERVES, AND WHAT IT IS NOT\n'
    printf '  whether any of the three exact PRIOR Stage-1 refusals recurs under\n'
    printf '  the accepted Repair 27A controller. It assigns no verdict, passes\n'
    printf '  nothing and gates no stage.\n'
    printf '  THIS IS NOT v20\047S PURPOSE. v20 is the fresh integrated end-to-end\n'
    printf '  rehearsal of Repair 27A; these three checks are retained historical\n'
    printf '  plumbing only, because a recurrence of an older refusal would still\n'
    printf '  be worth seeing. false here is NOT a pass and proves nothing about\n'
    printf '  Repair 27A.\n\n'
    ...
    printf '  The v16-era test requires NO count: the defect is missing required\n'
    printf '  closure paths, not the spelling of the number of them.\n\n'
    printf 'CONTROLLER UNDER TEST\n'
    printf '  shadow controller  : %s  (accepted Repair 27A candidate)\n' "$EXP_R27A_SHA"
    printf '  real controller    : %s  (unchanged, NOT installed)\n' "$EXP_LOOP_SHA"
    printf '  Repair20B          : %s  (preserved, NOT the refresh source)\n' "$EXP_R20B_SHA"
    printf '  Repair21           : %s  (preserved, NOT the refresh source)\n' "$EXP_R21_SHA"
    printf '  Repair22           : %s  (preserved, NOT the refresh source)\n' "$EXP_R22_SHA"
    printf '  Repair23           : %s  (preserved, NOT the refresh source)\n' "$EXP_R23_SHA"
    printf '  Repair24           : %s  (preserved, NOT the refresh source)\n' "$EXP_R24_SHA"
    printf '  Repair25           : %s  (preserved, NOT the refresh source)\n' "$EXP_R25_SHA"
    printf '  Repair26           : %s  (preserved, NOT the refresh source)\n' "$EXP_R26_SHA"
    printf '  Repair27           : %s  (preserved, NOT the refresh source)\n' "$EXP_R27_SHA"
    printf '  refresh state      : %s\n\n' "$CONTROLLER_REFRESH"
    ...
    printf 'THE PRE-CALL RELEVANT-SOURCE PATH, AS THE CONTROLLER REPORTED IT\n'
    printf '  These five fields were introduced by Repair 22 and extended by\n'
    printf '  Repair 23. They say whether the\n'
    ...
    printf 'HOW A STAGE-1 OUTCOME IS READ IN v20\n'
    printf '  A. any of the three flags true  -- that exact older refusal appeared\n'
    printf '     again under Repair 27A, in one message. Worth seeing; not a\n'
    printf '     verdict on anything.\n'
    printf '  B. all three false AND Stage 1 stopped for some other ordinary\n'
    printf '     controller reason. THIS IS A VALID RESULT. A later\n'
    printf '     MATERIAL_UNKNOWN, a coverage signal, a closed gate or any other\n'
    printf '     existing gate is preserved exactly and stops the sequence under\n'
    printf '     the ordinary rules. Nothing here fixes, bypasses, suppresses or\n'
    printf '     reinterprets it. The next blocker is not launcher work.\n'
    printf '  C. Stage 1 fully succeeded and the sequence continued.\n'
...
# invariant. The controller under test in v20 is ACCEPTED REPAIR 27A, which is
# built on accepted Repair 25 and INHERITS this behaviour unchanged. Repair 26
# did not invent it and does not claim it: the observer is checking that
# INHERITED Repair-25 seam still holds under Repair 26. That is why the field,
# the artefact and the verdict all keep the Repair-25 name.
#
...
# already independently audited and offline-tested; v20 is an INTEGRATED
...
observe_repair25_chatgpt_stop_handoff() {
...

  {
    ...
    printf 'CONTROLLER UNDER TEST\n'
    printf '  shadow controller : %s  (accepted Repair 27A)\n' "$EXP_R27A_SHA"
    printf '  Repair 27A is based on Repair 27, which is based on Repair 26,\n'
    printf '  which is based on accepted Repair 25, and it INHERITS the stop\n'
    printf '  behaviour observed below. This observer checks that INHERITED\n'
    printf '  Repair-25 seam; Repair 27A did not invent it.\n'
    ...
    if [ "$verdict" = "NOT-EXERCISED" ]; then
      ...
      printf '  is invoked to force the path. v20 is an integrated rehearsal, not\n'
      ...
      printf '  second question-validation run by this launcher: no (one call site)\n'
    ...
    printf '  The historical seed carried %s events before this run.\n' "$EXP_SEED_JOURNAL_EVENTS"
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v20, ACCEPTED REPAIR-27A, INTEGRATED) — RESULT\n'
printf '===========================================================\n'
printf 'SHADOW_LAB=%s\n'                        "$LAB"
printf 'REGRESSION_SOURCE=%s\n'                 "$REGRESSION_SOURCE"
printf 'REGRESSION_SOURCE_MODE=%s\n'            "$REGRESSION_SOURCE_MODE"
printf 'RUN_KIND=FRESH_INTEGRATED_END_TO_END_SHADOW_REHEARSAL\n'
...
printf 'REPAIR27A_ACCEPTED_BY_NESS=YES\n'
printf 'REPAIR27A_ACCEPTANCE_AUTHORISES_ONLY_THIS_SHADOW_REFRESH=YES\n'
printf 'REPAIR27A_INSTALLED=NO\n'
printf 'NH_ADOPTION_OR_ACCEPTANCE_PERFORMED=NO\n'
printf 'SHADOW_CONTROLLER_UNDER_TEST=REPAIR27A\n'
printf 'SHADOW_CONTROLLER_SHA256=%s\n'          "$EXP_R27A_SHA"
printf 'SHADOW_CONTROLLER_SOURCE=%s\n'          "$R27A_CANDIDATE"
printf 'SHADOW_REFRESH_SOURCE_IS_REPAIR27A_ONLY=YES\n'
...
printf 'REPAIR27A_KNOWN_MANUAL_ROUTE_AUDIT_PROVENANCE_DEPENDENCY=OPEN\n'
...
printf 'PREDECESSOR_LAUNCHER_SHA256=%s\n'       "$EXP_LAUNCHER_V19_SHA"
printf 'PREDECESSOR_LAUNCHER_WAS_RUN=YES\n'
printf 'PREDECESSOR_LAB=%s\n'                   "$PRESERVED_LAB_REPAIR26_V19"
printf 'PREDECESSOR_STAGE1_RESULT=PASS\n'
printf 'PREDECESSOR_STAGE2_RESULT=PASS\n'
printf 'PREDECESSOR_STAGE3_RESULT=PASS\n'
printf 'PREDECESSOR_STAGE4_STOP_REASON=PREPARATION_FAILED\n'
printf 'PREDECESSOR_CLAUDE_INVOCATIONS=0\n'
printf 'PREDECESSOR_CODEX_DESIGN_AUDIT_INVOCATIONS=0\n'
printf 'PREDECESSOR_CORRECTION_ROUNDS=0\n'
printf 'V18_WAS_RUN=YES\n'
printf 'V18_LAB=%s\n'                           "$PRESERVED_LAB_REPAIR25_V18"
printf 'V18_STAGE1_STOP_REASON=MATERIAL_UNKNOWN_UNRESOLVED\n'
printf 'V16_WAS_RUN=NO\n'
printf 'V17_WAS_RUN=NO\n'
printf 'LAST_COMPLETED_REHEARSAL_LAUNCHER=%s\n' "$LAUNCHER_V19_BASENAME"
printf 'LAST_COMPLETED_REHEARSAL_LAB=%s\n'      "$PRESERVED_LAB_REPAIR26_V19"
printf 'PRESERVED_COMPLETED_V18_LAB_REUSED=NO\n'
printf 'PRESERVED_COMPLETED_V18_LAB_USED_AS_SEED=NO\n'
printf 'PRESERVED_COMPLETED_V19_LAB_REUSED=NO\n'
printf 'PRESERVED_COMPLETED_V19_LAB_USED_AS_SEED=NO\n'
printf 'HISTORICAL_REFUSAL_OBSERVATIONS_ARE_NOT_V20_PURPOSE=YES\n'
...
printf 'REPAIR25_HANDOFF_IS_AN_INHERITED_SEAM_UNDER_REPAIR26=YES\n'
printf 'REPAIR25_HANDOFF_NOT_EXERCISED_IS_NOT_A_FAILURE=YES\n'
printf 'REPAIR25_HANDOFF_OBSERVATION_GATES_NO_STAGE=YES\n'
printf 'V20_INJECTED_A_FINDING_TO_FORCE_THE_BRANCH=NO\n'
printf 'V20_INVOKED_ROUTE_AUDIT_FINDING=NO\n'
printf 'V20_INVOKED_A_SECOND_QUESTION_VALIDATION=NO\n'
printf 'V20_INVOKED_NEXT_QUESTION_GROUP=NO\n'
printf 'V20_INVOKED_RECORD_NESS_ANSWER=NO\n'
printf 'V20_INVOKED_DECLARE_STRANDED_CANDIDATE=NO\n'
printf '\n'
...
printf 'PROOF_V20_OBSERVATIONS=%s\n'                 "${PROOF}/v20-stage-observations.txt"
printf 'PROOF_HISTORICAL_STAGE1_REFUSALS=%s\n'       "${PROOF}/historical-stage1-refusals.txt"
...
printf '  controller/nh_loop.py inside the disposable v20 shadow was replaced\n'
printf '  with the ACCEPTED REPAIR-27A CANDIDATE, which was opened read-only as\n'
printf '  the source and was never written to. Repair 27A was the ONLY refresh\n'
printf '  source: the real controller is byte-for-byte the file this launcher\n'
printf '  froze and re-proved before, during and after, and it was never a\n'
printf '  source and never a destination. Repair 20B, 21, 22, 23, 24, 25, 26 and\n'
printf '  27 are preserved repair evidence that this run read, proved unchanged,\n'
printf '  and never copied into the shadow.\n'
printf '  REPAIR 27A IS ACCEPTED BY NESS FOR ITS BOUNDED CONTROLLER-REPAIR SCOPE,\n'
...
printf '  implementation was authorised. The historical seed still carries its\n'
printf '  own Repair-17 controller. The seven-event journal, its head and the\n'
printf '  authenticity key were not refreshed and are byte-identical to the seed.\n'
printf '\n  WHAT THIS RUN WAS TESTING, EXACTLY.\n'
printf '  This is the FRESH INTEGRATED END-TO-END SHADOW REHEARSAL of the\n'
printf '  accepted Repair-27A controller: the current accepted controller logic,\n'
printf '  run against the SAME preserved historical pre-question shadow state\n'
printf '  v18 and v19 used, inside the already-proven isolation model, through\n'
printf '  the four real stages -- question-validation, question-coverage-review,\n'
printf '  interview-gate and execute-next-claude-task -- to see what the real\n'
printf '  controller workflow actually does now.\n'
printf '\n  ITS IMMEDIATE PURPOSE. v19 RAN. Its Stage 1, Stage 2 and Stage 3 all\n'
printf '  PASSED -- which is what proved accepted Repair 26 corrected v18\047s\n'
printf '  115-reference Stage-1 blocker -- and its Stage 4 then stopped,\n'
printf '  fail-closed, with PREPARATION_FAILED, having invoked Claude ZERO times\n'
printf '  and run ZERO Codex design audits.\n'
printf '  That Stage-4 refusal was a source-envelope contradiction: the\n'
printf '  Piece-3-cleared historical shadow legitimately held FIVE untracked\n'
printf '  source paths, while Stage 4 required the THREE package candidates to be\n'
printf '  the whole untracked worktree. Repair 27 corrected that model; accepted\n'
printf '  REPAIR 27A then corrected the two problems the independent audit found\n'
printf '  in Repair 27 -- the two clearance-frozen NON-CANDIDATE baseline files\n'
printf '  were frozen by path and Git status but not by byte identity, and an\n'
printf '  early failed resume-envelope freeze could raise UnboundLocalError\n'
printf '  instead of returning the ordinary fail-closed result. This run tests\n'
printf '  the real workflow after those bounded mechanical corrections.\n'
printf '  IT ASSUMES NO OUTCOME AT ALL. It does not assume Stage 4 must pass,\n'
printf '  that Claude must be invoked, that a Codex design audit must run, that\n'
printf '  a correction round must happen, or that the loop must reach PASS. The\n'
printf '  real Repair-27A controller was run once and whatever it did was\n'
printf '  recorded. A NEW legitimate fail-closed stop is a VALID RESULT, and no\n'
printf '  gate was loosened, bypassed or reinterpreted to reach a later stage.\n'
printf '  The completed v19 lab is preserved unchanged as the evidence of the\n'
printf '  refusal this run succeeds; it was never reused, never written to and\n'
printf '  never used as this run\047s seed. The completed v18 lab is preserved on\n'
printf '  exactly the same terms.\n'
printf '  IT IS NOT A REPAIR-22-ONLY REGRESSION, and it is not a broad controller\n'
printf '  audit. The three historical Stage-1 refusal checks it still carries --\n'
printf '  the v13 parser refusal, the v14 closure refusal and the v15\n'
printf '  missing-closure-path refusal Repair 22 originally corrected -- are\n'
printf '  retained HISTORY. They gate nothing, decide nothing, and are not this\n'
printf '  launcher\047s purpose; a false on any of them is not a pass.\n'
printf '  THE STAGE-4 OBSERVER WATCHES AN INHERITED REPAIR-25 SEAM. Repair 25\n'
printf '  corrected exactly one thing: the execute-next-claude-task\n'
printf '  CHATGPT_REVIEW_REQUIRED stop, so that a blocking\n'
printf '  chatgpt_review_required audit finding no longer reports itself as an\n'
printf '  already-made piece-3 transition and no longer hands the operator\n'
printf '  route-audit-finding and question-validation as immediate commands.\n'
printf '  Repair 27A inherits that behaviour through Repairs 27 and 26; it did\n'
printf '  not invent it. If Stage 4 reached that stop,\n'
printf '  REPAIR25_CHATGPT_STOP_HANDOFF above records what the controller\n'
printf '  actually reported; if it did not, that observation reads\n'
printf '  NOT-EXERCISED, which is not a failure and was not forced.\n'
printf '\n  ONE KNOWN DEPENDENCY IS DELIBERATELY LEFT OPEN, AND THIS RUN DID NOT\n'
printf '  TOUCH IT. Accepted Repair 27A leaves the MANUAL route-audit-finding\n'
printf '  provenance re-proof not yet carrying a non-empty resume baseline. That\n'
printf '  path is not part of the normal Stage-4 execution rehearsed here. This\n'
printf '  launcher never invoked route-audit-finding, never injected a finding to\n'
printf '  force it, never repaired it, and does not treat its being open as a v20\n'
printf '  failure. If Stage 4 stopped at CHATGPT_REVIEW_REQUIRED, this run\n'
printf '  stopped there and recorded the controller\047s own result.\n'
printf '\n  NO OUTCOME WAS REQUIRED, AND A LATER STOP WAS NOT WORKED AROUND.\n'
printf '  This rehearsal was not trying to make every stage complete. If any\n'
printf '  stage stopped on an existing MATERIAL_UNKNOWN, a coverage signal, a\n'
printf '  closed interview gate or any other existing controller gate, that stop\n'
printf '  was preserved exactly under the ordinary rules. No gate was altered,\n'
printf '  relaxed, bypassed or reinterpreted to make any repair look successful,\n'
printf '  and the next blocker was not turned into launcher work.\n'
