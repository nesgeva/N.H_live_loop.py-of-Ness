#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v18.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v18
#   (THE ACCEPTED REPAIR-25 CONTROLLER, FRESH INTEGRATED END-TO-END SHADOW
#    REHEARSAL, WITH THE HISTORICAL SEED LOCKED BY WHOLE-WORKSPACE IDENTITY)
#
# WHAT v18 CORRECTS IN v17, AND NOTHING ELSE
#   v17 was constructed, independently audited, found BLOCKED by two confirmed
#   launcher defects, and NEVER RUN. v18 corrects exactly those two:
#
#     A. THE BYTECODE CAPTURE RUNTIME BLOCKER. v17 moved the frozen controller
#        bytecode state from one .pyc to three and rewrote the assertion to
#        iterate the new arrays, but left capture_original_proof() reading the
#        obsolete singular scalar CTRL_PYCACHE_FILE, which v17 no longer
#        defines. Under `set -Eeuo pipefail` the FIRST capture would have
#        aborted the launcher before the rehearsal began. The capture now
#        records the COMPLETE frozen set from the SAME arrays the assertion
#        uses, and no compatibility scalar is reintroduced.
#
#     B. THE STALE CURRENT-RUN IDENTITY. v17 already refreshed from
#        $R25_CANDIDATE with the correct expected identity -- the executable
#        behaviour was right -- but its CURRENT-run proof name, staging path,
#        expectation labels, error and success messages, composite-pair wording
#        and final result block still called that Repair-25 operation
#        "Repair 22" and the shadow "v16". Those statements were false about the
#        current run and are corrected to Repair 25 / v18.
#
#   GENUINE HISTORY IS PRESERVED. The Repair-22 candidate constant, its exact
#   identity, its preserved-evidence gate, the v15 failure Repair 22 originally
#   corrected and the REPAIR22_EXACT_REGRESSION_RECURRED observation are all
#   truthful history and are NOT renamed. Nothing else about v17 is redesigned.
#
# WHAT v18 IS, AND WHAT IT IS NOT
#   It is the fresh INTEGRATED shadow rehearsal of the accepted Repair-25
#   controller: the current accepted controller logic, run against the preserved
#   historical pre-question shadow state, inside the already-proven isolation
#   model, to see what the real controller workflow actually does now.
#
#   IT IS NOT another broad controller audit, IT IS NOT a new repair, and IT IS
#   NOT a Repair-22-only regression. Its purpose is not to make every stage
#   complete. A legitimate controller stop is a legitimate result, and this file
#   contains no machinery for forcing the workflow past one.
#
#   ITS DIRECT PREDECESSOR, v17, WAS CONSTRUCTED AND NEVER RUN -- it was found
#   blocked by the two defects above. There is therefore NO completed v17
#   rehearsal lab and no completed v17 result, none is claimed anywhere in this
#   file, none is required by anything in it, and v17 is never executed by it.
#   v17 is preserved launcher history and is gated here by exact identity like
#   every launcher before it. The same remains true of v16, which was also
#   never run.
#
#   REPAIRS 20B, 21, 22, 23 AND 24 ARE PRESERVED REPAIR EVIDENCE ONLY. Not one
#   of them is a refresh source. Repair 25 is the SOLE refresh source.
#
# WHAT THIS IS
...
#
# WHAT NESS'S ACCEPTANCE OF REPAIR 25 DOES AND DOES NOT AUTHORISE
#   Ness has explicitly accepted Repair 25 for its bounded controller-repair
#   scope and asked for it to be tested. In THIS FILE that acceptance authorises
#   exactly one thing: using nh_loop_REPAIR25_CANDIDATE.py as the sole controller
...
#   v18's. Every identity THIS run asserts is in SECTION 0 and nowhere else: the
#   real controller by the digest EXP_LOOP_SHA (no generation name is claimed for
#   it), the shadow refreshed to the accepted Repair-25 candidate EXP_R25_SHA,
#   the lab and marker keyed to REPAIR25/V18, and the seed at EXP_SEED_LOOP_SHA.
#
...
#             controller result, and v18 records it without reinterpreting it.
#
...
#
# It WAS the refresh source of v14. In v18 it is preserved repair evidence only,
# exactly like Repair 21, 22, 23 and 24: never a refresh source, never copied
...
#     (PRESERVED REPAIR EVIDENCE — NOT THE v18 REFRESH SOURCE) ---
#
...
#
# It WAS the refresh source of v16, which was never run. In v18 it is
# preserved repair evidence only, exactly like Repair 20B, 21, 23 and 24:
...
# (I-03..I-07). In v18 it is PRESERVED EVIDENCE ONLY. It is never the refresh
...
# defect. In v18 it is PRESERVED EVIDENCE ONLY, on exactly the same terms as
...
# --- controller: THE ACCEPTED REPAIR-25 CANDIDATE -- THE SOLE v18 REFRESH SOURCE
#
# THIS IS THE ONE FILE v18 EXISTS TO EXERCISE, and it is the ONLY controller
# refresh source anywhere in this launcher. Repair 25 corrected the
...
# route-audit-finding and question-validation as immediate commands.
#
# Ness has ACCEPTED it for that bounded scope. Acceptance authorises this shadow
# refresh and nothing else: the real controller is not installed to, and this
# candidate is copied into the NEW DISPOSABLE COPY only.
#
...
# --- lab: NEW, keyed to REPAIR25, to V18, to the exact accepted Repair-25
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR25_V18_91b89aadafe613f28393269c6dcb3321d8872de0bf6c97f3cdbc874be7e97595_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
# THE ONE NEW v18 OBSERVATION FIELD.
#
...
capture_original_proof() {
  ...
  # --- the ACCEPTED REPAIR-25 candidate identity (CORRECTED IN v18) ---
  #
  # This block hashes $R25_CANDIDATE and always did in v17 -- the executable
  # behaviour was already correct. What was wrong was the NAME: it called the
  # thing it was hashing "the Repair-22 candidate" and wrote the artefact as
  # repair25_candidate.identity, so the current run's own refresh-source proof
  # was filed under the name of a DIFFERENT, preserved candidate. A proof whose
  # name misidentifies what it proves is worse than no proof, so it is renamed
  # to what it actually measures. Genuine preserved Repair-22 evidence elsewhere
  # in this file keeps its own name and is untouched.
  ...
  # --- all EIGHTEEN launcher artefacts: v1-v17 must stay byte-identical, and
  #     this file must stay byte-identical to itself across the run ---
  #
  # CORRECTED IN v18. The sweep stopped at v16, so v17 -- the file that was
  # actually running -- was never carried in its own before/after proof, and a
  # change to it during a run would not have been caught. v17 and v18 are both
  # included now. v18 cannot hash-gate on itself in the frozen-identity gate,
  # which is exactly why it must appear HERE: the before/during/after comparison
  # is what proves the running launcher did not change underneath itself.
  {
    for lf in "$LAUNCHER_V1_BASENAME" "$LAUNCHER_V2_BASENAME" \
              ...
              "$LAUNCHER_V17_BASENAME" "$LAUNCHER_V18_BASENAME"; do
      if [ -f "${CTRL}/${lf}" ]; then
...
#   AFTER      the refreshed file is the EXACT accepted Repair-25 identity --
#              sha256,
...
#
# THE REFRESH SOURCE IS THE ACCEPTED REPAIR-25 CANDIDATE, OPENED READ-ONLY.
#
# CORRECTED IN v18. This paragraph was inherited from v13, where EXP_LOOP_* was
# BOTH the real controller AND the refresh source because they were the same
# file. They have not been the same file since v14, and in v18 the refresh
# source is ${R25_CANDIDATE} and nothing else. The paragraph nevertheless still
# named the real controller as this function's copy source, which contradicted
# section 0, contradicted assert_real_controller_is_no_known_candidate(),
# contradicted SHADOW_REFRESH_SOURCE_IS_REPAIR25_ONLY=YES, and contradicted the
# code below.
#
# WHAT IS ACTUALLY TRUE HERE:
#   * ${R25_CANDIDATE} is the ONLY copy source. It is opened read-only, never
...
refresh_new_copy_controller() {
  local target="${SHADOW_WS}/controller/nh_loop.py"
  local staging="${SHADOW_WS}/controller/.nh_loop.py.repair25-refresh"

  # THE REFRESH SOURCE IS THE ACCEPTED REPAIR-25 CANDIDATE, NOT THE REAL CONTROLLER AND
  ...
  [ -L "$R25_CANDIDATE" ] && die "the accepted Repair-25 candidate is a SYMLINK:
        ${R25_CANDIDATE}
      Refusing to refresh the shadow through it."
  [ -f "$R25_CANDIDATE" ] \
    || die "the accepted Repair-25 candidate is missing at ${R25_CANDIDATE}"
  expect "refresh source sha256 (accepted Repair 25 candidate)" \
         "$(sha_of "$R25_CANDIDATE")"   "$EXP_R25_SHA"
  expect "refresh source bytes  (accepted Repair 25 candidate)" \
         "$(bytes_of "$R25_CANDIDATE")" "$EXP_R25_BYTES"
  expect "refresh source lines  (accepted Repair 25 candidate)" \
         "$(wc -l < "$R25_CANDIDATE")"  "$EXP_R25_LINES"
  ...
  cp -- "$R25_CANDIDATE" "$staging" \
    || die "could not stage the accepted Repair-25 candidate into the new shadow copy:
        ${staging}
      Nothing has been renamed over the new copy's controller."
  chmod 0644 -- "$staging" \
    || die "could not set the mode on the staged Repair-25 candidate: ${staging}"
  mv -f -- "$staging" "$target" \
    || die "could not move the staged Repair-25 candidate into place:
        ${target}"
  CONTROLLER_REFRESH="PERFORMED"
  note "the new copy's controller/nh_loop.py refreshed to the accepted Repair-25 candidate"
}
...
assert_new_copy_after_refresh() {
  ...
  expect "new copy controller sha256 AFTER refresh (accepted Repair 25 candidate)" \
         "$(sha_of "$target")"   "$EXP_R25_SHA"
  expect "new copy controller bytes  AFTER refresh (accepted Repair 25 candidate)" \
         "$(bytes_of "$target")" "$EXP_R25_BYTES"
  expect "new copy controller lines  AFTER refresh (accepted Repair 25 candidate)" \
         "$(wc -l < "$target")"  "$EXP_R25_LINES"
  ...
  expect "Repair21 unchanged AFTER refresh (preserved, not the source)" \
         "$(sha_of "$R21_CANDIDATE")" "$EXP_R21_SHA"
  note "the refreshed shadow controller is the exact accepted Repair-25 identity"
}
...
assert_new_copy_differs_only_by_controller() {
  ...
  cmp_out="$(
    PYTHONDONTWRITEBYTECODE=1 "$PY3" -B -c "$TREE_COMPARATOR" \
      ...
      "NEWCOPY" \
      "SEED" \
      "DIFFERS_EXACT:${REFRESH_ALLOWED_EXACT_LOOP}:${EXP_R25_SHA}:${EXP_SEED_LOOP_SHA}" 2>&1
  )"
  cmp_rc=$?
  set -e
...
${cmp_out}
      ...
      controller that is not the exact Repair-25/Repair-17 pair.
...
${cmp_out}
      ...
      Repair-17 controller while reporting a Repair-25 run."
      ;;
    *)
...
assert_frozen_identities() {
  ...
  # THE REPAIR-20B CANDIDATE -- PRESERVED REPAIR EVIDENCE in v18, not the refresh
  ...
  # THE REPAIR-21 CANDIDATE -- PRESERVED REPAIR EVIDENCE in v18, not the refresh
  # source. Gated exactly as strictly as the real controller and equally never
  # written to.
  ...
  # THE REPAIR-22 CANDIDATE -- PRESERVED REPAIR EVIDENCE in v18, not the refresh
  # source. Gated exactly as strictly as the real controller and equally never
  # written to.
  ...
  # THE ACCEPTED REPAIR-25 CANDIDATE -- THE SOLE v18 REFRESH SOURCE, gated exactly
  ...
  [ -f "$R25_CANDIDATE" ] \
    || die "the accepted Repair-25 candidate is missing from ${CTRL}; it is the one
  ...
  # gated by exact identity like v1..v15. v18 never runs it.
  ...
  # v17 is this file's DIRECT PREDECESSOR AND IT WAS NEVER RUN. It was
  # constructed, independently audited, and found blocked by the two defects v18
  # corrects, so no v17 lab and no v17 result exist and none is claimed. It is
  # preserved launcher history and is gated by exact identity like v1..v16.
  # v18 never runs it.
  ...
  # Launcher v18 is THIS file. It cannot hash-gate on itself, so it is accounted
  ...
  [ -s "${CTRL}/${LAUNCHER_V18_BASENAME}" ] \
    || die "launcher v18 is missing or empty at ${CTRL}/${LAUNCHER_V18_BASENAME}"
  note "all eighteen launcher artefacts present in ${CTRL}"
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${R25_CANDIDATE_BASENAME}" | LC_ALL=C sort
  )"
...
#
# The token identifies FOUR things: Repair 25, the v18 integrated rehearsal, the
# exact accepted Repair-25 controller sha256, and the routed signal this rehearsal
# carries forward. It therefore cannot collide with any earlier rehearsal marker.
SHADOW_TOKEN="nh-shadow-repair25-integrated-v18-${EXP_R25_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...
# disposable v18 shadow, which is the corrected Repair-19 behaviour this
...
if ! stage_may_continue; then
  ...
  if ! stage_may_continue; then
    ...
    if ! stage_may_continue; then
      ...
      #
      # WHATEVER EXECUTE REPORTS IS THE CONTROLLER RESULT, AND v18 RECORDS IT
...
# ==========================================================================
# SECTION 11b — v18 STAGE OBSERVATIONS
...
step "v18 STAGE OBSERVATIONS"

observe_v18_stages() {
  local out="${PROOF}/v18-stage-observations.txt"
  ...
  {
    printf 'v18 STAGE OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================================\n\n'
    ...
    printf '  refreshed to      : %s  (accepted Repair 25 candidate)\n' "$EXP_R25_SHA"
...
observe_exact_regressions() {
...

  {
    ...
    printf '  the accepted Repair 25 controller. It assigns no verdict, passes\n'
    printf '  nothing and gates no stage.\n'
    printf '  THIS IS NOT v18\047S PURPOSE. v18 is the fresh integrated end-to-end\n'
    printf '  rehearsal of Repair 25; these three checks are retained historical\n'
    ...
    printf '  shadow controller  : %s  (accepted Repair 25 candidate)\n' "$EXP_R25_SHA"
    ...
    printf 'HOW A STAGE-1 OUTCOME IS READ IN v18\n'
    printf '  A. any of the three flags true  -- that exact older refusal appeared\n'
    printf '     again under Repair 25, in one message. Worth seeing; not a\n'
...
# ==========================================================================
# SECTION 11c — THE REPAIR-25 CHATGPT-STOP HANDOFF OBSERVATION  (NEW IN v17)
...
# already independently audited and offline-tested; v18 is an INTEGRATED
...
observe_repair25_chatgpt_stop_handoff() {
...

  {
    ...
    printf 'CONTROLLER UNDER TEST\n'
    printf '  shadow controller : %s  (accepted Repair 25)\n' "$EXP_R25_SHA"
    printf '  refresh state     : %s\n\n' "$CONTROLLER_REFRESH"
    ...
    if [ "$verdict" = "NOT-EXERCISED" ]; then
      ...
      printf '  is invoked to force the path. v18 is an integrated rehearsal, not\n'
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v18, ACCEPTED REPAIR-25, INTEGRATED) — RESULT\n'
printf '===========================================================\n'
printf 'SHADOW_LAB=%s\n'                        "$LAB"
printf 'REGRESSION_SOURCE=%s\n'                 "$REGRESSION_SOURCE"
printf 'REGRESSION_SOURCE_MODE=%s\n'            "$REGRESSION_SOURCE_MODE"
...
printf 'REPAIR25_ACCEPTED_BY_NESS=YES\n'
printf 'REPAIR25_ACCEPTANCE_AUTHORISES_ONLY_THIS_SHADOW_REFRESH=YES\n'
printf 'REPAIR25_INSTALLED=NO\n'
printf 'NH_ADOPTION_OR_ACCEPTANCE_PERFORMED=NO\n'
printf 'SHADOW_CONTROLLER_UNDER_TEST=REPAIR25\n'
printf 'SHADOW_CONTROLLER_SHA256=%s\n'          "$EXP_R25_SHA"
printf 'SHADOW_CONTROLLER_SOURCE=%s\n'          "$R25_CANDIDATE"
printf 'SHADOW_REFRESH_SOURCE_IS_REPAIR25_ONLY=YES\n'
...
printf 'V18_INJECTED_A_FINDING_TO_FORCE_THE_BRANCH=NO\n'
printf 'V18_INVOKED_ROUTE_AUDIT_FINDING=NO\n'
printf 'V18_INVOKED_A_SECOND_QUESTION_VALIDATION=NO\n'
printf 'V18_INVOKED_NEXT_QUESTION_GROUP=NO\n'
printf 'V18_INVOKED_RECORD_NESS_ANSWER=NO\n'
printf '\n'
...
printf 'PROOF_V18_OBSERVATIONS=%s\n'                 "${PROOF}/v18-stage-observations.txt"
...
printf '  controller/nh_loop.py inside the disposable v18 shadow was replaced\n'
printf '  with the ACCEPTED REPAIR-25 CANDIDATE, which was opened read-only as\n'
printf '  the source and was never written to. Repair 25 was the ONLY refresh\n'
...
printf '  Repair 23 and Repair 24 are preserved repair evidence that this run\n'
printf '  read,\n'
printf '  proved unchanged, and never copied into the shadow.\n'
printf '  REPAIR 25 IS ACCEPTED BY NESS FOR ITS BOUNDED CONTROLLER-REPAIR SCOPE,\n'
...
printf '  accepted Repair-25 controller: the current accepted controller logic,\n'
printf '  run against the preserved historical pre-question shadow state, inside\n'
printf '  the already-proven isolation model, through the four real stages --\n'
printf '  question-validation, question-coverage-review, interview-gate and\n'
printf '  execute-next-claude-task -- to see what the real controller workflow\n'
printf '  actually does now.\n'
...
printf '  Repair 25 itself corrected exactly one thing: the\n'
printf '  execute-next-claude-task CHATGPT_REVIEW_REQUIRED stop, so that a\n'
printf '  blocking chatgpt_review_required audit finding no longer reports itself\n'
printf '  as an already-made piece-3 transition and no longer hands the operator\n'
printf '  route-audit-finding and question-validation as immediate commands. If\n'
printf '  Stage 4 reached that stop, REPAIR25_CHATGPT_STOP_HANDOFF above records\n'
printf '  what the controller actually reported; if it did not, that observation\n'
printf '  reads NOT-EXERCISED, which is not a failure and was not forced.\n'
