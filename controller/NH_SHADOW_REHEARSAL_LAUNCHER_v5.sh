#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v5.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v5  (REPAIR-11 CONTROLLER)
#
# WHAT THIS IS
...
#   namespace, runs the REPAIR-11 sequence
#
#       1. python3 nh_loop.py question-coverage-review     (one real GPT review)
#       2. python3 nh_loop.py interview-gate               (read-only, no model)
#       3. python3 nh_loop.py execute-next-claude-task     (at most once)
#
...
#
# ==========================================================================
# WHAT v5 IS, AND WHAT IT CHANGES
# ==========================================================================
#
#   v5 IS A BOUNDED REBASE OF v4, NOT A REDESIGN.
...
#
# USAGE
#   bash /home/ness/NH_CLAUDE_CODEX_DESIGN_LOOP/controller/NH_SHADOW_REHEARSAL_LAUNCHER_v5.sh
#
...
# Five launcher files now live in controller/. All five are accounted for
# explicitly: four by exact identity, and this one by presence.
...
# v1, v2, v3 and v4 are ALL preserved unchanged, and all four are frozen here so
# this launcher fails closed if any of them moved. Each is historical validation
...
# --- controller: THE REPAIR-11 IDENTITY ---
readonly EXP_LOOP_SHA="9eace9acc4a551b0ccb325172818040bcb38cd15e22da822c863053f34aa9859"
readonly EXP_LOOP_BYTES="1684347"
readonly EXP_LOOP_LINES="36980"
...
# --- lab: NEW, keyed to the Repair-11 controller identity ---
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR11_9eace9acc4a551b0ccb325172818040bcb38cd15e22da822c863053f34aa9859"
...
# --- the three stage capture files, named exactly as the run order ---
readonly COV_STDOUT="${PROOF}/coverage-review.stdout"
readonly COV_STDERR="${PROOF}/coverage-review.stderr"
readonly COV_RCFILE="${PROOF}/coverage-review.exit-code"
readonly GATE_STDOUT="${PROOF}/interview-gate.stdout"
readonly GATE_STDERR="${PROOF}/interview-gate.stderr"
readonly GATE_RCFILE="${PROOF}/interview-gate.exit-code"
readonly EXEC_STDOUT="${PROOF}/execute.stdout"
readonly EXEC_STDERR="${PROOF}/execute.stderr"
readonly EXEC_RCFILE="${PROOF}/execute.exit-code"
...
# Proof artefacts of PRESERVED EARLIER REHEARSAL LABS -- the v1, Repair-9 and
# Repair-10 labs. Kept as their own set, with their own verdict and their own
# reserved exit code, so a preserved-evidence failure is never reported as, or
# hidden behind, a protected-workspace verdict.
...
# The three stage results. NOT-RUN is a real, reported state and is never
...
report_state() {
  ...
  if [ "$RUN_STARTED" = "1" ]; then
    ...
    printf '  stage 1  question-coverage-review : %s\n' "$COVERAGE_RC" >&2
    printf '  stage 2  interview-gate           : %s\n' "$GATE_RC" >&2
    printf '  stage 3  execute-next-claude-task : %s\n' "$EXEC_RC" >&2
  else
...
capture_original_proof() {
  ...
  # --- all five launcher artefacts: v1-v4 must stay byte-identical, and this
  ...
  # Every earlier shadow-rehearsal lab -- the v1, Repair-9 and Repair-10 labs --
  # is preserved evidence. This launcher's own lab is excluded because it is
  # being written by design. If any preserved lab changes at all between before
  # and after, the comparison fails and this run fails closed.
  #
...
# with any of them.
for _plab in "$PRESERVED_LAB_V1" "$PRESERVED_LAB_REPAIR9" "$PRESERVED_LAB_REPAIR10"; do
  if [ "$LAB" = "$_plab" ]; then
...
assert_frozen_identities() {
  ...
  # Launcher v5 is THIS file. It cannot hash-gate on itself, but it must be the
  # file that is present, and it is carried in the before/after proof set so any
  # change to it during the run is caught.
  [ -s "${CTRL}/${LAUNCHER_V5_BASENAME}" ] \
    || die "launcher v5 is missing or empty at ${CTRL}/${LAUNCHER_V5_BASENAME}"
  note "all five launcher artefacts present in ${CTRL}"
  ...
  # The controller worktree must be EXACTLY these seven entries and nothing
  # else: the modified nh_loop.py, the five launcher artefacts, and the one
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${LAUNCHER_V5_BASENAME}" | LC_ALL=C sort
  )"
...
# All nine stage-capture files are created here, so the watch commands printed
# below actually work the moment they are offered. Without this they do not
# exist until the redirects in section 11 create them, and `tail -f` on a
# missing file exits immediately with "no files remaining" -- guidance that
# looks helpful and cannot be followed.
#
# The three exit-code files are pre-seeded with NOT-RUN rather than left empty,
# because an empty exit-code file is ambiguous: it reads equally as "the stage
# has not run yet", "the stage ran and wrote nothing" and "the write failed".
# NOT-RUN says which. Section 11 overwrites each with a real integer the moment
# its stage finishes.
...
printf 'NOT-RUN\n' > "$GATE_RCFILE"
printf 'NOT-RUN\n' > "$EXEC_RCFILE"
chmod 0600 "$COV_STDOUT" "$COV_STDERR" "$COV_RCFILE" \
           "$GATE_STDOUT" "$GATE_STDERR" "$GATE_RCFILE" \
           "$EXEC_STDOUT" "$EXEC_STDERR" "$EXEC_RCFILE"
...
say  "        events without it -- and the Repair-11 coverage review appends"
say  "        exactly such an event when it clears. Delete the lab after review."
say  ""
say  "        TO WATCH THIS RUN LIVE, from another terminal, run any of these:"
say  ""
say  "          tail -F ${COV_STDERR}"
say  "          tail -F ${COV_STDOUT}"
say  ""
say  "          tail -F ${GATE_STDERR}"
say  "          tail -F ${GATE_STDOUT}"
say  ""
say  "          tail -F ${EXEC_STDERR}"
say  "          tail -F ${EXEC_STDOUT}"
say  ""
say  "        Or all six at once:"
say  ""
say  "          tail -F ${PROOF}/coverage-review.std* ${PROOF}/interview-gate.std* ${PROOF}/execute.std*"
say  ""
say  "        Repair 11 writes its live progress to stderr -- the fresh GPT"
say  "        correction-specification review, correction round, model pinned,"
say  "        new-or-resumed Claude session, seeded target, fresh OpenAI/Codex"
say  "        audit and verdict. stdout carries the machine report."
say  ""
...
#
# The token is keyed to the Repair-11 controller identity, so it cannot collide
...
SHADOW_TOKEN="nh-shadow-repair11-${EXP_LOOP_SHA}"
...
# ==========================================================================
# SECTION 11 — THE REPAIR-11 REAL TEST SEQUENCE
# ==========================================================================
#
# Three stages, in this exact order, each inside Bubblewrap, each at most once:
#
#   1. question-coverage-review      one real fresh GPT/Codex review
#   2. interview-gate                read-only, no provider call
#   3. execute-next-claude-task      exactly once
#
# There is NO retry loop anywhere here and nothing reruns any stage. If a stage
# fails, it fails once. The controller owns its own bounded internal correction
# loop, its own lifetime correction-round limit of 5, its own OpenAI reasoning
# role (gpt-5.6-sol at effort HIGH, invoked through the read-only, ephemeral
# Codex CLI transport) and its own Claude configuration (claude-opus-5 at xhigh,
# with one package session per execution), and all of that is left exactly as it
# is.

# run_controller_stage <label> <command> <stdout> <stderr> <exit-code-file>
#
# Runs ONE controller command inside its own namespace and records its three
# outputs separately. The result is returned through the global STAGE_RC rather
# than through the function's own exit status ON PURPOSE: under `set -e` a
# function that returns non-zero would abort the launcher at the call site, and
# a non-zero controller exit is an ORDINARY, EXPECTED outcome this launcher
# exists to record -- aborting there would destroy the after-proof, which is the
# most important thing this launcher produces.
run_controller_stage() {
  local label="$1" cmd="$2" so="$3" se="$4" rcf="$5"
  local rc
...
# --------------------------------------------------------------------------
# STAGE 1 — THE FRESH INDEPENDENT QUESTION-COVERAGE REVIEW
# --------------------------------------------------------------------------
#
# One real GPT/Codex review. It is read-only against N.H, ephemeral, and resumes
# no audit or design history. It cannot create a question, cannot address Ness,
# cannot admit, retire, park or settle anything, and cannot unlock anything by
# itself.
#
# Nothing here bypasses or manufactures coverage clearance. This launcher writes
# no journal event, sets no clearance, edits no interview state and supplies no
# review reply. Whatever clearance exists after this stage was recorded by the
# controller, inside the shadow, from a real model reply that the controller
# itself validated and bound to a proved source manifest.
run_controller_stage "STAGE 1 — question-coverage-review" \
                     "question-coverage-review" \
                     "$COV_STDOUT" "$COV_STDERR" "$COV_RCFILE"
COVERAGE_RC="$STAGE_RC"

if [ "$COVERAGE_RC" -ne 0 ]; then
  # A TECHNICAL failure of the review. Not a coverage gap -- a gap exits ZERO,
  # see the header. This is a preflight failure, an unmet precondition, a Codex
  # transport failure, source that moved under the review, a reply that failed
  # validation, or a source manifest that could not be proved.
  say ""
  say "STAGE 1 exited ${COVERAGE_RC}. That is a TECHNICAL failure of the coverage"
  say "review, not a coverage gap -- a gap exits 0 and routes signals."
  say "Stages 2 and 3 are NOT run. The evidence is preserved and this launcher"
  say "continues straight to the outside after-proof, which is the part that"
  say "matters most."
else
  say ""
  say "STAGE 1 exited 0. Note carefully: this means the review COMPLETED, not"
  say "that coverage is clear. Whether it cleared is read from"
  say "coverage_review_cleared and coverage_possible_gaps in the controller's own"
  say "report, never from this exit code."

  # ------------------------------------------------------------------------
  # STAGE 2 — THE INTERVIEW GATE
  # ------------------------------------------------------------------------
  #
  # Read-only. No provider call of any kind. This stage is OBSERVATIONAL: it
  # records whether the mechanical path is currently unlocked after the fresh
  # coverage review. A CLOSED gate is a legitimate state, is not a failure of
  # this stage, and is not a failure of this launcher.
  run_controller_stage "STAGE 2 — interview-gate" \
                       "interview-gate" \
                       "$GATE_STDOUT" "$GATE_STDERR" "$GATE_RCFILE"
  GATE_RC="$STAGE_RC"
  ...
  if [ "$GATE_RC" -ne 0 ]; then
    say ""
    say "STAGE 2 exited ${GATE_RC}. The gate could not be evaluated cleanly, which"
    say "is a technical failure rather than an ordinary closed gate: a closed gate"
    say "reports itself and exits 0. Stage 3 is NOT run. The evidence is preserved"
    say "and this launcher continues to the outside after-proof."
  else
    # ----------------------------------------------------------------------
    # STAGE 3 — THE ONE EXECUTE
    # ----------------------------------------------------------------------
    #
    # EXACTLY ONE outer execution of the controller. There is no retry loop here
    # and nothing reruns it.
    #
    # If the coverage review found a possible missed Ness choice, this stage is
    # EXPECTED to fail closed at its own real Piece-3 gate, with zero Claude
    # invocations. That is VALID EVIDENCE that Repair 11's gate works, not a
    # launcher fault, and it is recorded as such.
    #
    # If coverage is clear and the gate is open, the controller may naturally
    # continue into: GPT mechanical design specification -> Claude application ->
    # fresh GPT audit -> if needed GPT correction specification -> Claude
    # correction -> fresh GPT audit. The controller owns all of that behaviour
    # and its lifetime correction limit of 5.
    run_controller_stage "STAGE 3 — execute-next-claude-task" \
...
if [ "$EXEC_RC" != "NOT-RUN" ]; then
  SHADOW_RC="$EXEC_RC"
elif [ "$GATE_RC" != "NOT-RUN" ]; then
  SHADOW_RC="$GATE_RC"
else
  SHADOW_RC="$COVERAGE_RC"
fi

# ==========================================================================
# SECTION 11b — REPAIR-11 OBSERVATIONS (READ-ONLY, DECIDES NOTHING)
# ==========================================================================
#
# A convenience pass over the lab's OWN captured output, so the Repair-11
# behaviours are easy to inspect without reading thousands of lines by hand.
#
# IT RECORDS; IT DECIDES NOTHING. It sets no verdict, it can fail no check, and
# every safety verdict below is computed exactly as it was in v4.
#
# NO PARTICULAR NATURAL OUTCOME IS REQUIRED. In particular:
#   - a coverage gap is NOT a launcher failure. It is the review doing its job;
#   - ZERO Claude invocations may be entirely correct, if the gate stayed closed;
#   - ZERO resumed Claude invocations may be entirely correct, if no correction
#     round was needed;
#   - a closed interview gate is a legitimate state, not a fault.
# These observations record what happened. They do not invent PASS.
#
# It reads only the nine stage files this launcher created. It never reads the
# shadow candidates and never echoes candidate content: it prints scalar,
...
step "REPAIR-11 OBSERVATIONS"

observe_repair11() {
  local out="${PROOF}/repair11-observations.txt"
  local n_new n_resume n_seed n_audit n_verdict n_corrspec n_stopreview n_gapline

  # Live-progress markers, counted from the execute stage's stderr. Each string
  # is a fixed, controller-owned prefix emitted by nh_loop.py's own emit_live.
  n_new="$(grep -c -F -- 'invoking claude (new session'                        "$EXEC_STDERR" 2>/dev/null || true)"
  n_resume="$(grep -c -F -- 'resuming the package claude session'              "$EXEC_STDERR" 2>/dev/null || true)"
  n_seed="$(grep -c -F -- 'next target seeded byte-identical'                  "$EXEC_STDERR" 2>/dev/null || true)"
  n_audit="$(grep -c -F -- 'fresh codex design audit starting'                 "$EXEC_STDERR" 2>/dev/null || true)"
  n_verdict="$(grep -c -F -- 'codex verdict for'                               "$EXEC_STDERR" 2>/dev/null || true)"
  n_corrspec="$(grep -c -F -- 'running ONE fresh gpt correction-specification' "$EXEC_STDERR" 2>/dev/null || true)"
  n_stopreview="$(grep -c -F -- 'running ONE fresh gpt review of the claude specification stop' "$EXEC_STDERR" 2>/dev/null || true)"
  n_gapline="$(grep -c -E '^       routed signal '                             "$COV_STDOUT"  2>/dev/null || true)"

  {
    printf 'REPAIR-11 OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================\n\n'

    printf 'STAGE EXIT CODES\n'
    printf '  stage 1  question-coverage-review : %s\n' "$COVERAGE_RC"
    printf '  stage 2  interview-gate           : %s\n' "$GATE_RC"
    printf '  stage 3  execute-next-claude-task : %s\n' "$EXEC_RC"
    printf '  NOT-RUN means the sequence stopped before that stage, which is a\n'
    printf '  different fact from a stage that ran and returned a code.\n\n'
    ...
    printf '  gap as a signal and reports stop reason coverage_gaps_routed. So a 0\n'
    printf '  from stage 1 does NOT mean coverage is clear. Read\n'
    printf '  coverage_review_cleared and coverage_possible_gaps below instead.\n\n'

    printf '======================================================\n'
    printf 'STAGE 1 -- FRESH INDEPENDENT QUESTION-COVERAGE REVIEW\n'
    printf '======================================================\n'
    printf 'CONTROLLER-OWNED SCALARS (from its own JSON report):\n'
    grep -E '"(coverage_review_invoked|coverage_review_exit_code|coverage_review_cleared|coverage_possible_gaps|coverage_validation_set_id|coverage_package_scope_id|coverage_scope_root_path|coverage_source_paths_checked|coverage_manifest_sha256|coverage_relevant_closure_paths|coverage_relevant_closure_sha256|coverage_required_issue_source_paths|coverage_inventory_issue_count|coverage_binding_bytes|interview_stop_reason|ok)":' \
         "$COV_STDOUT" 2>/dev/null | cut -c1-200 || true
    printf '\n'

    printf 'THE REVIEWING ROLE, as the controller stated it for itself:\n'
    grep -E '^  (model|effort|sandbox|claude|asks Ness):' "$COV_STDOUT" 2>/dev/null | cut -c1-200 || true
    printf '\n'

    printf 'ROUTED REVIEW SIGNALS (identifiers only, never gap wording):\n'
    printf '  routed signal lines recorded: %s\n' "$n_gapline"
    grep -E '^       routed signal ' "$COV_STDOUT" 2>/dev/null | head -50 | cut -c1-200 || true
    ...
    printf '===================================\n'
    printf 'STAGE 2 -- INTERVIEW GATE (read-only)\n'
    printf '===================================\n'
    grep -E '"(mechanical_path_unlocked|coverage_review_current|coverage_review_validation_set_id|validation_set_id|validation_pages_recorded|validation_pages_expected|validation_inventory_complete|revalidation_required|open_group_index|stale_question_count|routed_issues_awaiting_validation|interview_stop_reason|ok)":' \
         "$GATE_STDOUT" 2>/dev/null | cut -c1-200 || true
    printf '\n'
    printf 'THE GATE IN THE CONTROLLER OWN WORDS:\n'
    grep -E '^  (the mechanical|fresh independent coverage review|genuine questions still blocking|routing signals not yet validated|complete package-bound validation|questions withheld|current-package dependencies)' \
         "$GATE_STDOUT" 2>/dev/null | head -20 | cut -c1-200 || true
    printf '\n'

    printf '=============================================\n'
    printf 'STAGE 3 -- EXECUTE-NEXT-CLAUDE-TASK (one only)\n'
    printf '=============================================\n'
    ...
    grep -E '"(claude_model|claude_output_format|claude_allowed_tools|claude_permission_mode|claude_package_session_scope|claude_timeout_seconds)":' \
         "$EXEC_STDOUT" 2>/dev/null | cut -c1-200 || true
    printf '\n'
    ...
    grep -E '"(preparation_ok|prepared_classification|claude_instruction_source|mechanical_design_specification_sha256)":' \
         "$EXEC_STDOUT" 2>/dev/null | cut -c1-200 || true
    printf '  A null mechanical_design_specification_sha256 means no validated GPT\n'
    printf '  specification was produced on this path. That may be correct: the\n'
    printf '  run may have stopped before preparation could reach it.\n\n'
    ...
    grep -E '"(initial_claude_session_mode|initial_claude_session_ok|claude_invoked|claude_package_session_established|claude_package_session_lost|claude_package_session_lost_reason|claude_package_session_invocations|claude_package_session_new_invocations|claude_package_session_resumed_invocations|initial_claude_invocations|claude_correction_invocations|total_claude_invocations)":' \
         "$EXEC_STDOUT" 2>/dev/null | cut -c1-200 || true
    printf '\n'

    printf 'CORRECTION LOOP AND FRESH GPT AUDITS:\n'
    grep -E '"(correction_loop_enabled|correction_round_limit|correction_rounds_started|correction_rounds_completed|correction_performed|total_codex_design_audit_invocations|design_audit_codex_invocations|final_design_audit_verdict|final_highest_severity|final_chatgpt_review_required|stop_reason)":' \
         "$EXEC_STDOUT" 2>/dev/null | cut -c1-200 || true
    printf '\n'
    ...
    if [ "${n_new:-0}" -eq 0 ] 2>/dev/null; then
      printf '  ZERO Claude invocations. This is NOT a failure. If the coverage\n'
      printf '  review routed a possible gap, or the interview gate otherwise\n'
      printf '  stayed closed, execute-next-claude-task is EXPECTED to fail closed\n'
      printf '  at its own gate without invoking Claude at all. Read\n'
      printf '  mechanical_path_unlocked in stage 2 and piece3_interview_gate_unlocked\n'
      printf '  and stop_reason in stage 3 to see which happened.\n'
    else
    ...
    if [ "${n_resume:-0}" -gt 0 ] 2>/dev/null; then
      ...
      printf '  round, there was nothing to resume. Read stop_reason and\n'
      printf '  final_design_audit_verdict above to see which happened.\n'
    fi
    printf '\n'
  ...
  note "repair-11 observations written to ${out}"
}
observe_repair11 || true

# ==========================================================================
# SECTION 12 — RE-PROVE THE ORIGINAL, FROM OUTSIDE THE NAMESPACE
# ==========================================================================
#
# This runs REGARDLESS of how the three stages went -- whether stage 1 failed
# technically and stages 2 and 3 never ran, whether the gate stayed closed, or
# whether the controller ran to completion and returned any code at all. The
# ordinary success or failure of the controller has no bearing on whether the
# protected original must be re-proved. It always must.
...
printf '\n'
printf '==================================================\n'
printf 'N.H SHADOW LIVE REHEARSAL (v5, REPAIR-11) — RESULT\n'
printf '==================================================\n'
printf 'SHADOW_LAB=%s\n'               "$LAB"
printf 'FROZEN_CONTROLLER_SHA256=%s\n' "$EXP_LOOP_SHA"
printf 'STAGE1_COVERAGE_REVIEW_EXIT=%s\n' "$COVERAGE_RC"
printf 'STAGE2_INTERVIEW_GATE_EXIT=%s\n'  "$GATE_RC"
printf 'STAGE3_EXECUTE_EXIT=%s\n'         "$EXEC_RC"
...
printf 'COVERAGE_STDOUT=%s\n'          "$COV_STDOUT"
printf 'COVERAGE_STDERR=%s\n'          "$COV_STDERR"
printf 'GATE_STDOUT=%s\n'              "$GATE_STDOUT"
printf 'GATE_STDERR=%s\n'              "$GATE_STDERR"
printf 'EXECUTE_STDOUT=%s\n'           "$EXEC_STDOUT"
printf 'EXECUTE_STDERR=%s\n'           "$EXEC_STDERR"
printf 'REPAIR11_OBSERVATIONS=%s\n'    "${PROOF}/repair11-observations.txt"
printf '==================================================\n'
...
printf '\nA STAGE-1 EXIT OF 0 DOES NOT MEAN COVERAGE IS CLEAR. A coverage review\n'
printf 'that finds possible gaps routes each one as a signal and exits 0. Read\n'
printf 'coverage_review_cleared and coverage_possible_gaps in:\n'
printf '  %s\n' "${PROOF}/repair11-observations.txt"
