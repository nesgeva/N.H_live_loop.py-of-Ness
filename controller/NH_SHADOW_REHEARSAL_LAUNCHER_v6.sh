#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v6.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v6  (REPAIR-14 CONTROLLER)
#
# WHAT THIS IS
...
#   namespace, runs the REPAIR-14 sequence
#
...
#
# ==========================================================================
# WHAT v6 IS, AND WHAT IT CHANGES
# ==========================================================================
#
#   v6 IS A BOUNDED REBASE OF v5, NOT A REDESIGN.
...
#
# USAGE
#   bash /home/ness/NH_CLAUDE_CODEX_DESIGN_LOOP/controller/NH_SHADOW_REHEARSAL_LAUNCHER_v6.sh
#
...
# Six launcher files now live in controller/. All six are accounted for
# explicitly: five by exact identity, and this one by presence.
...
# v1..v5 are ALL preserved unchanged, and all five are frozen here so this
...
# --- controller: THE REPAIR-14 IDENTITY ---
readonly EXP_LOOP_SHA="76b60e90e2bc6835edb088df7126428e2365e16cb376cb00274a086de1698957"
readonly EXP_LOOP_BYTES="1729587"
readonly EXP_LOOP_LINES="37948"
...
# --- lab: NEW, keyed to the Repair-14 controller identity ---
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR14_76b60e90e2bc6835edb088df7126428e2365e16cb376cb00274a086de1698957"
...
capture_original_proof() {
  ...
  # --- all six launcher artefacts: v1-v5 must stay byte-identical, and this
  #     file must stay byte-identical to itself across the run ---
  {
    ...
    for lf in "$LAUNCHER_V1_BASENAME" "$LAUNCHER_V2_BASENAME" \
              ...
              "$LAUNCHER_V5_BASENAME" "$LAUNCHER_V6_BASENAME"; do
      if [ -f "${CTRL}/${lf}" ]; then
...
assert_frozen_identities() {
  ...
  # Launcher v6 is THIS file. It cannot hash-gate on itself, but it must be the
  # file that is present, and it is carried in the before/after proof set so any
  # change to it during the run is caught.
  [ -s "${CTRL}/${LAUNCHER_V6_BASENAME}" ] \
    || die "launcher v6 is missing or empty at ${CTRL}/${LAUNCHER_V6_BASENAME}"
  note "all six launcher artefacts present in ${CTRL}"
  ...
  # The controller worktree must be EXACTLY these eight entries and nothing
  # else: the modified nh_loop.py, the six launcher artefacts, and the one
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${LAUNCHER_V6_BASENAME}" | LC_ALL=C sort
  )"
...
#
# The token is keyed to the Repair-14 controller identity, so it cannot collide
...
SHADOW_TOKEN="nh-shadow-repair14-${EXP_LOOP_SHA}"
...
# ==========================================================================
# SECTION 11 — THE REPAIR-14 REAL TEST SEQUENCE
# ==========================================================================
#
...
# ==========================================================================
# SECTION 11b — REPAIR-14 OBSERVATIONS (READ-ONLY, DECIDES NOTHING)
# ==========================================================================
#
...
step "REPAIR-14 OBSERVATIONS"

observe_repair14() {
  local out="${PROOF}/repair14-observations.txt"
  ...
  {
    printf 'REPAIR-14 OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================\n\n'
...
printf '\n'
printf '==================================================\n'
printf 'N.H SHADOW LIVE REHEARSAL (v6, REPAIR-14) — RESULT\n'
printf '==================================================\n'
printf 'SHADOW_LAB=%s\n'                     "$LAB"
...
unset _slug
printf 'REPAIR14_OBSERVATIONS=%s\n'    "${PROOF}/repair14-observations.txt"
printf '==================================================\n'
