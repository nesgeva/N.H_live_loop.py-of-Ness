#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v7.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v7  (REPAIR-15 CONTROLLER)
#
# WHAT THIS IS
...
#   namespace, runs the REPAIR-15 sequence
#
...
#
# ==========================================================================
# WHAT v7 IS, AND WHAT IT CHANGES
# ==========================================================================
#
#   v7 IS A BOUNDED REBASE OF v6, NOT A REDESIGN.
...
#
# USAGE
#   bash /home/ness/NH_CLAUDE_CODEX_DESIGN_LOOP/controller/NH_SHADOW_REHEARSAL_LAUNCHER_v7.sh
#
...
# Seven launcher files now live in controller/. All seven are accounted for
# explicitly: six by exact identity, and this one by presence.
...
# v1..v6 are ALL preserved unchanged, and all six are frozen here so this
...
# lab is the Repair-14 evidence named above. Its stage-2 refusal is the defect
# Repair 15 corrects. Nothing about v6 is retracted by v7 existing.
...
# --- controller: THE REPAIR-15 IDENTITY ---
readonly EXP_LOOP_SHA="94f0734f65da7e13964858c06f9e9fec4774f72045115caa03ecc34759d4a9e9"
readonly EXP_LOOP_BYTES="1741855"
readonly EXP_LOOP_LINES="38180"
...
# --- lab: NEW, keyed to the Repair-15 controller identity ---
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR15_94f0734f65da7e13964858c06f9e9fec4774f72045115caa03ecc34759d4a9e9"
...
capture_original_proof() {
  ...
  # --- all seven launcher artefacts: v1-v6 must stay byte-identical, and this
...
for _plab in "$PRESERVED_LAB_V1" "$PRESERVED_LAB_REPAIR9" \
             "$PRESERVED_LAB_REPAIR10" "$PRESERVED_LAB_REPAIR14"; do
  if [ "$LAB" = "$_plab" ]; then
...
assert_frozen_identities() {
  ...
  # Launcher v7 is THIS file. It cannot hash-gate on itself, but it must be the
  # file that is present, and it is carried in the before/after proof set so any
  # change to it during the run is caught.
  [ -s "${CTRL}/${LAUNCHER_V7_BASENAME}" ] \
    || die "launcher v7 is missing or empty at ${CTRL}/${LAUNCHER_V7_BASENAME}"
  note "all seven launcher artefacts present in ${CTRL}"
  ...
  # The controller worktree must be EXACTLY these nine entries and nothing
  # else: the modified nh_loop.py, the seven launcher artefacts, and the one
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${LAUNCHER_V7_BASENAME}" | LC_ALL=C sort
  )"
...
#
# The token is keyed to the Repair-15 controller identity, so it cannot collide
...
SHADOW_TOKEN="nh-shadow-repair15-${EXP_LOOP_SHA}"
...
# ==========================================================================
# SECTION 11 — THE REPAIR-15 REAL TEST SEQUENCE
# ==========================================================================
#
...
# ==========================================================================
# SECTION 11b — REPAIR-15 OBSERVATIONS (READ-ONLY, DECIDES NOTHING)
# ==========================================================================
#
...
step "REPAIR-15 OBSERVATIONS"

observe_repair15() {
  local out="${PROOF}/repair15-observations.txt"
  ...
  {
    printf 'REPAIR-15 OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================\n\n'
  ...
  note "repair-14 observations written to ${out}"
}
observe_repair15 || true
...
printf '\n'
printf '==================================================\n'
printf 'N.H SHADOW LIVE REHEARSAL (v7, REPAIR-15) — RESULT\n'
printf '==================================================\n'
printf 'SHADOW_LAB=%s\n'                     "$LAB"
...
unset _slug
printf 'REPAIR15_OBSERVATIONS=%s\n'    "${PROOF}/repair15-observations.txt"
printf '==================================================\n'
