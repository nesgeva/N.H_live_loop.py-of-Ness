#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v8.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v8  (REPAIR-16 CONTROLLER)
#
# WHAT THIS IS
...
#   namespace, runs the REPAIR-16 sequence
#
...
#
# ==========================================================================
# WHAT v8 IS, AND WHAT IT CHANGES
# ==========================================================================
#
#   v8 IS A BOUNDED REBASE OF v7, NOT A REDESIGN.
...
#
# USAGE
#   bash /home/ness/NH_CLAUDE_CODEX_DESIGN_LOOP/controller/NH_SHADOW_REHEARSAL_LAUNCHER_v8.sh
#
...
# Eight launcher files now live in controller/. All eight are accounted for
# explicitly: seven by exact identity, and this one by presence.
...
# v1..v7 are ALL preserved unchanged, and all seven are frozen here so this
...
# --- controller: THE REPAIR-16 IDENTITY ---
readonly EXP_LOOP_SHA="882fe3e510544b97361bd31853d39d5269b0e1fb4d39dbc2b0a387f139b10833"
readonly EXP_LOOP_BYTES="1748707"
readonly EXP_LOOP_LINES="38307"
...
# --- lab: NEW, keyed to the Repair-16 controller identity ---
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR16_882fe3e510544b97361bd31853d39d5269b0e1fb4d39dbc2b0a387f139b10833"
...
# Repair-10, Repair-14 and Repair-15 labs, the last of which is the completed
# v7 rehearsal. Kept as their own set, with their own verdict and their own
...
capture_original_proof() {
  ...
  # --- all eight launcher artefacts: v1-v7 must stay byte-identical, and this
  #     file must stay byte-identical to itself across the run ---
  {
    ...
    for lf in "$LAUNCHER_V1_BASENAME" "$LAUNCHER_V2_BASENAME" \
              ...
              "$LAUNCHER_V7_BASENAME" "$LAUNCHER_V8_BASENAME"; do
      if [ -f "${CTRL}/${lf}" ]; then
  ...
  # Repair-14 and Repair-15 labs, the last being the completed v7 rehearsal --
  ...
  for d in "${WS_SPECIAL_DIRS[@]}"; do
    ...
    if [ ! -e "$d" ] && [ ! -L "$d" ]; then
      die "an explained workspace-root directory is MISSING:
        ${d}
      v8 is frozen to this exact set of three empty directories. One of them
...
assert_frozen_identities() {
  ...
  # Launcher v8 is THIS file. It cannot hash-gate on itself, but it must be the
  # file that is present, and it is carried in the before/after proof set so any
  # change to it during the run is caught.
  [ -s "${CTRL}/${LAUNCHER_V8_BASENAME}" ] \
    || die "launcher v8 is missing or empty at ${CTRL}/${LAUNCHER_V8_BASENAME}"
  note "all eight launcher artefacts present in ${CTRL}"
  ...
  # The controller worktree must be EXACTLY these ten entries and nothing
  # else: the modified nh_loop.py, the eight launcher artefacts, and the one
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${LAUNCHER_V8_BASENAME}" | LC_ALL=C sort
  )"
...
#
# The token is keyed to the Repair-16 controller identity, so it cannot collide
...
SHADOW_TOKEN="nh-shadow-repair16-${EXP_LOOP_SHA}"
...
# ==========================================================================
# SECTION 11 — THE REPAIR-16 REAL TEST SEQUENCE
# ==========================================================================
#
...
# ==========================================================================
# SECTION 11b — REPAIR-16 OBSERVATIONS (READ-ONLY, DECIDES NOTHING)
# ==========================================================================
#
...
step "REPAIR-16 OBSERVATIONS"

observe_repair16() {
  local out="${PROOF}/repair16-observations.txt"
  ...
  {
    printf 'REPAIR-16 OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================\n\n'
  ...
  note "repair-16 observations written to ${out}"
}
observe_repair16 || true
...
printf '\n'
printf '==================================================\n'
printf 'N.H SHADOW LIVE REHEARSAL (v8, REPAIR-16) — RESULT\n'
printf '==================================================\n'
printf 'SHADOW_LAB=%s\n'                     "$LAB"
...
unset _slug
printf 'REPAIR16_OBSERVATIONS=%s\n'    "${PROOF}/repair16-observations.txt"
printf '==================================================\n'
