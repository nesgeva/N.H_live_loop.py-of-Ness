#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v9.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v9  (REPAIR-17 CONTROLLER)
#
# WHAT THIS IS
...
#
# ==========================================================================
# WHAT v9 IS, AND WHAT IT CHANGES
# ==========================================================================
#
#   v9 IS A BOUNDED REBASE OF v8, NOT A REDESIGN.
...
#
# USAGE
#   bash /home/ness/NH_CLAUDE_CODEX_DESIGN_LOOP/controller/NH_SHADOW_REHEARSAL_LAUNCHER_v9.sh
#
...
# Nine launcher files now live in controller/. All nine are accounted for
# explicitly: eight by exact identity, and this one by presence.
...
# --- lab: NEW, keyed to the Repair-17 controller identity ---
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR17_6a3ce02e4a0448d5a7206f6eb4260bbcb87989da9f0a541feafea7dd4be0dc19"
...
capture_original_proof() {
  ...
  # --- all nine launcher artefacts: v1-v8 must stay byte-identical, and this
...
for _plab in "$PRESERVED_LAB_V1" "$PRESERVED_LAB_REPAIR9" \
             ...
             "$PRESERVED_LAB_REPAIR15" "$PRESERVED_LAB_REPAIR16"; do
  if [ "$LAB" = "$_plab" ]; then
...
assert_frozen_identities() {
  ...
  # Launcher v9 is THIS file. It cannot hash-gate on itself, but it must be the
  # file that is present, and it is carried in the before/after proof set so any
  # change to it during the run is caught.
  [ -s "${CTRL}/${LAUNCHER_V9_BASENAME}" ] \
    || die "launcher v9 is missing or empty at ${CTRL}/${LAUNCHER_V9_BASENAME}"
  note "all nine launcher artefacts present in ${CTRL}"
  ...
  # The controller worktree must be EXACTLY these eleven entries and nothing
  # else: the modified nh_loop.py, the nine launcher artefacts, and the one
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${LAUNCHER_V9_BASENAME}" | LC_ALL=C sort
  )"
...
#
# The token is keyed to the Repair-17 controller identity, so it cannot collide
...
SHADOW_TOKEN="nh-shadow-repair17-${EXP_LOOP_SHA}"
...
# ==========================================================================
# SECTION 11 — THE REPAIR-17 REAL TEST SEQUENCE
# ==========================================================================
#
...
# ==========================================================================
# SECTION 11b — REPAIR-17 OBSERVATIONS (READ-ONLY, DECIDES NOTHING)
# ==========================================================================
#
...
step "REPAIR-17 OBSERVATIONS"

observe_repair17() {
  local out="${PROOF}/repair17-observations.txt"
  ...
  {
    printf 'REPAIR-17 OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================\n\n'
  ...
  note "repair-17 observations written to ${out}"
}
observe_repair17 || true
...
printf '\n'
printf '==================================================\n'
printf 'N.H SHADOW LIVE REHEARSAL (v9, REPAIR-17) — RESULT\n'
printf '==================================================\n'
printf 'SHADOW_LAB=%s\n'                     "$LAB"
...
unset _slug
printf 'REPAIR17_OBSERVATIONS=%s\n'    "${PROOF}/repair17-observations.txt"
printf '==================================================\n'
