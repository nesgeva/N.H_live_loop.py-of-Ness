#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v19.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v19
#   (THE ACCEPTED REPAIR-26 CONTROLLER, FRESH INTEGRATED END-TO-END SHADOW
#    REHEARSAL, WITH THE HISTORICAL SEED LOCKED BY WHOLE-WORKSPACE IDENTITY)
#
# WHAT v19 CHANGES FROM v18, AND NOTHING ELSE
#   v18 WAS RUN, exactly once, and it completed. It is not corrected here and
#   nothing about it is retracted: it did precisely what it was built to do.
#   v19 is the SMALLEST SAFE REBASE of that completed launcher onto the accepted
#   Repair-26 controller, and it changes only what that rebase mechanically
#   requires:
#
#     1. VERSION PLUMBING. v18 -> v19 throughout the CURRENT-run identity: the
...
#     2. THE EXACT PREDECESSOR, FROZEN. v18 is now a preserved launcher like
#        every one before it, gated by exact sha256 and byte count.
#     3. A FRESH v19 LAB AND MARKER, keyed to REPAIR26/V19, to the exact
#        accepted Repair-26 controller identity, and to the routed signal this
#        rehearsal carries forward. No lab is ever reused.
#     4. THE COMPLETED v18 LAB, PRESERVED AS EVIDENCE. It holds the exact
#        Stage-1 refusal Repair 26 exists to correct, so it is named here for
#        collision refusal, presence assertion and human reporting, and it is
#        never written to, never reused, never normalised and never deleted.
#     5. REPAIR 25 BECOMES PRESERVED REPAIR EVIDENCE; REPAIR 26 BECOMES THE
#        SOLE REFRESH SOURCE. Repair 25 is still gated by exact identity and
#        still witnessed unmoved -- it is simply not copied anywhere.
#     6. THE REPAIR-26 CANDIDATE, gated exactly as strictly as the real
#        controller and accounted for in the exact controller worktree.
#
...
#   Stage-1/2/3 continuation rule are carried forward exactly as v18 had them.
#
# WHAT THE COMPLETED v18 RUN ACTUALLY DID
#   Its isolation self-test PASSED. Repair 25 was its sole shadow refresh
#   source. The protected real workspace, the preserved labs and the regression
#   seed all PASSED. Stage 1 ran ONCE and stopped FAIL-CLOSED with
#   MATERIAL_UNKNOWN_UNRESOLVED, and Stages 2, 3 and 4 therefore NEVER RAN.
#
#   That Stage-1 stop was the repository-reference resolution class that
#   accepted Repair 26 has since corrected: intentionally preserved
#   accepted-package / active-candidate duplicate documents, and one unique
#   same-directory extensionless Markdown alias. v18's lab is the evidence for
#   that, and it is preserved unchanged.
#
#   v16 AND v17 WERE NEVER RUN. That remains true and is still reported.
#
# WHAT v19 IS, AND WHAT IT IS NOT
#   It is the fresh INTEGRATED shadow rehearsal of the accepted Repair-26
#   controller: the current accepted controller logic, run against the SAME
#   preserved historical pre-question shadow state v18 used, inside the
#   already-proven isolation model, to see what the real controller workflow
#   actually does now that the bounded resolver correction exists.
#
#   IT DOES NOT ASSUME STAGE 1 MUST NOW PASS. It runs the real Repair-26
#   controller once and records whatever the real controller does. A NEW
#   legitimate fail-closed stop is a VALID RESULT. No gate is loosened to reach
#   Stage 2, 3 or 4, and this file contains no machinery for forcing the
...
#
#   REPAIRS 20B, 21, 22, 23, 24 AND 25 ARE PRESERVED REPAIR EVIDENCE ONLY. Not
#   one of them is a refresh source. Repair 26 is the SOLE refresh source.
#
# WHAT THIS IS
...
#
# WHAT NESS'S ACCEPTANCE OF REPAIR 26 DOES AND DOES NOT AUTHORISE
#   Ness has explicitly accepted Repair 26 for its bounded controller-repair
#   scope and asked for it to be tested. In THIS FILE that acceptance authorises
#   exactly one thing: using nh_loop_REPAIR26_CANDIDATE.py as the sole controller
#   refresh source for the disposable shadow below.
#
#   REPAIR 25 REMAINS ACCEPTED, AND IN THIS FILE IT IS PRESERVED EVIDENCE ONLY.
#   Repair 26 is based on it and inherits it; its acceptance is not withdrawn by
#   this file, and its candidate is never copied anywhere by this file.
#
...
#   v19's. Every identity THIS run asserts is in SECTION 0 and nowhere else: the
#   real controller by the digest EXP_LOOP_SHA (no generation name is claimed for
#   it), the shadow refreshed to the accepted Repair-26 candidate EXP_R26_SHA,
#   the lab and marker keyed to REPAIR26/V19, and the seed at EXP_SEED_LOOP_SHA.
#
...
#             controller result, and v19 records it without reinterpreting it.
#
...
# Nineteen launcher files now live in controller/. All nineteen are accounted
# for explicitly: eighteen by exact identity, and this one by presence.
...
#
# It WAS the refresh source of v14. In v19 it is preserved repair evidence only,
# exactly like Repair 21, 22, 23, 24 and 25: never a refresh source, never copied
...
#     (PRESERVED REPAIR EVIDENCE — NOT THE v19 REFRESH SOURCE) ---
#
...
#
# It WAS the refresh source of v16, which was never run. In v19 it is
# preserved repair evidence only, exactly like Repair 20B, 21, 23, 24 and 25:
# never a refresh source, never copied anywhere, gated by exact identity so
...
# (I-03..I-07). In v19 it is PRESERVED EVIDENCE ONLY. It is never the refresh
...
# defect. In v19 it is PRESERVED EVIDENCE ONLY, on exactly the same terms as
...
#
# IN v19 REPAIR 25 IS PRESERVED REPAIR EVIDENCE ONLY, on exactly the same terms
# as Repair 20B, 21, 22, 23 and 24: it is gated by exact identity, witnessed in
# the before/during/after sweep, and never written to -- but it is NOT copied
# into the shadow by this launcher on any path. Its exact preserved identity is
# reported so this run can state that it did not move.
...
# --- controller: THE ACCEPTED REPAIR-26 CANDIDATE -- THE SOLE v19 REFRESH SOURCE
#
# THIS IS THE ONE FILE v19 EXISTS TO EXERCISE, and it is the ONLY controller
# refresh source anywhere in this launcher.
#
# WHAT CHANGED BETWEEN v18 AND v19, AND IT IS THIS.
#
# v18 refreshed its shadow to Repair 25 and RAN. Repair 25 did its job -- its
# own stop branch was simply never reached, which is not a failure. v18 then
# stopped at Stage 1, fail-closed, with MATERIAL_UNKNOWN_UNRESOLVED, and that
# one refusal is the whole reason this launcher exists: question-validation
# could not resolve a class of repository reference that is not genuinely
# ambiguous at all.
#
# REPAIR 26 IS THE BOUNDED CORRECTION TO EXACTLY THAT, AND TO NOTHING ELSE. It
# is based on accepted Repair 25 and adds exactly two mechanically proved cases
# to what the resolver counts as RESOLVED:
#   * an intentionally preserved accepted-package / active-candidate duplicate
#     resolves ONLY when the exact Repair-26 conditions prove the two files
#     byte-equivalent, and it then returns BOTH physical witnesses rather than
#     choosing between the two folders;
#   * one unique same-directory extensionless Markdown alias resolves under a
#     narrow physical-filename rule.
# GENUINE AMBIGUITY REMAINS FAIL-CLOSED, and Repairs 19 through 25 are preserved
# unchanged inside it.
#
# THIS LAUNCHER TESTS THAT CORRECTION AND NOTHING BROADER. It implements neither
# rule, interprets neither, and contains no resolver, closure, binding or
# source-selection logic in any executable path. Both live inside nh_loop.py.
#
# Ness has ACCEPTED Repair 26 for its bounded controller-repair scope.
...
# --- lab: NEW, keyed to REPAIR26, to V19, to the exact accepted Repair-26
...
#
# THE COMPLETED v18 LAB PATH IS NOT THIS PATH AND IS NEVER TOUCHED. v18 DID run,
# so a completed v18 lab exists; it is PRESERVED_LAB_REPAIR25_V18 below, it is
# evidence, and this launcher will neither reuse nor delete it.
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR26_V19_8970e9e4ee4e1d2219b36ec89215b8e92ccd1ecadce2dd1ff111c63b6ba18c36_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
capture_original_proof() {
  ...
  #
  # PRESERVED REPAIR EVIDENCE in v19, not the refresh source. It WAS v18's
  ...
  # --- the ACCEPTED REPAIR-26 candidate identity (NEW IN v19) ---
  #
  ...
  # --- all NINETEEN launcher artefacts: v1-v18 must stay byte-identical, and
  #     this file must stay byte-identical to itself across the run ---
  #
  # v19 cannot hash-gate on itself in the frozen-identity gate, which is exactly
...
#   AFTER      the refreshed file is the EXACT accepted Repair-26 identity --
#              sha256,
...
#
# THE REFRESH SOURCE IS THE ACCEPTED REPAIR-26 CANDIDATE, OPENED READ-ONLY.
#
# REBASED IN v19. The correction v18 made to this paragraph stands; only the
# candidate it names moves forward. EXP_LOOP_* is the REAL controller and has
# not been the refresh source since v14; in v19 the refresh source is
# ${R26_CANDIDATE} and nothing else.
#
# WHAT IS ACTUALLY TRUE HERE:
#   * ${R26_CANDIDATE} is the ONLY copy source. It is opened read-only, never
...
refresh_new_copy_controller() {
  local target="${SHADOW_WS}/controller/nh_loop.py"
  local staging="${SHADOW_WS}/controller/.nh_loop.py.repair26-refresh"

  # THE REFRESH SOURCE IS THE ACCEPTED REPAIR-26 CANDIDATE, NOT THE REAL CONTROLLER AND
  ...
  [ -L "$R26_CANDIDATE" ] && die "the accepted Repair-26 candidate is a SYMLINK:
        ${R26_CANDIDATE}
      Refusing to refresh the shadow through it."
  [ -f "$R26_CANDIDATE" ] \
    || die "the accepted Repair-26 candidate is missing at ${R26_CANDIDATE}"
  expect "refresh source sha256 (accepted Repair 26 candidate)" \
         "$(sha_of "$R26_CANDIDATE")"   "$EXP_R26_SHA"
  expect "refresh source bytes  (accepted Repair 26 candidate)" \
         "$(bytes_of "$R26_CANDIDATE")" "$EXP_R26_BYTES"
  expect "refresh source lines  (accepted Repair 26 candidate)" \
         "$(wc -l < "$R26_CANDIDATE")"  "$EXP_R26_LINES"

  # AND REPAIR 20B, REPAIR 21 AND REPAIR 25 ARE RE-ASSERTED UNCHANGED AT THE SAME
  # MOMENT. All three are preserved repair evidence, none is the source, and
  # proving them here is what makes "the shadow was refreshed to Repair 26 and to
  # nothing else" measurable rather than merely asserted.
  ...
  cp -- "$R26_CANDIDATE" "$staging" \
    || die "could not stage the accepted Repair-26 candidate into the new shadow copy:
        ${staging}
      Nothing has been renamed over the new copy's controller."
  chmod 0644 -- "$staging" \
    || die "could not set the mode on the staged Repair-26 candidate: ${staging}"
  mv -f -- "$staging" "$target" \
    || die "could not move the staged Repair-26 candidate into place:
        ${target}"
  CONTROLLER_REFRESH="PERFORMED"
  note "the new copy's controller/nh_loop.py refreshed to the accepted Repair-26 candidate"
}
...
assert_new_copy_after_refresh() {
  ...
  expect "new copy controller sha256 AFTER refresh (accepted Repair 26 candidate)" \
         "$(sha_of "$target")"   "$EXP_R26_SHA"
  expect "new copy controller bytes  AFTER refresh (accepted Repair 26 candidate)" \
         "$(bytes_of "$target")" "$EXP_R26_BYTES"
  expect "new copy controller lines  AFTER refresh (accepted Repair 26 candidate)" \
         "$(wc -l < "$target")"  "$EXP_R26_LINES"
  ...
  expect "Repair25 unchanged AFTER refresh (preserved, not the source)" \
         "$(sha_of "$R25_CANDIDATE")" "$EXP_R25_SHA"
  note "the refreshed shadow controller is the exact accepted Repair-26 identity"
}
...
assert_new_copy_differs_only_by_controller() {
  ...
  cmp_out="$(
    PYTHONDONTWRITEBYTECODE=1 "$PY3" -B -c "$TREE_COMPARATOR" \
      ...
      "NEWCOPY" \
      "SEED" \
      "DIFFERS_EXACT:${REFRESH_ALLOWED_EXACT_LOOP}:${EXP_R26_SHA}:${EXP_SEED_LOOP_SHA}" 2>&1
  )"
  cmp_rc=$?
  set -e
...
${cmp_out}
      ...
      controller that is not the exact Repair-26/Repair-17 pair.
...
${cmp_out}
      ...
      Repair-17 controller while reporting a Repair-26 run."
      ;;
    *)
...
assert_frozen_identities() {
  ...
  # THE REPAIR-20B CANDIDATE -- PRESERVED REPAIR EVIDENCE in v19, not the refresh
  ...
  # THE REPAIR-21 CANDIDATE -- PRESERVED REPAIR EVIDENCE in v19, not the refresh
  # source. Gated exactly as strictly as the real controller and equally never
  # written to.
  ...
  # THE REPAIR-22 CANDIDATE -- PRESERVED REPAIR EVIDENCE in v19, not the refresh
  # source. Gated exactly as strictly as the real controller and equally never
  # written to.
  ...
  # THE ACCEPTED REPAIR-25 CANDIDATE -- PRESERVED REPAIR EVIDENCE in v19, not the
  ...
  # THE ACCEPTED REPAIR-26 CANDIDATE -- THE SOLE v19 REFRESH SOURCE, gated exactly
  ...
  [ -f "$R26_CANDIDATE" ] \
    || die "the accepted Repair-26 candidate is missing from ${CTRL}; it is the one
  ...
  # gated by exact identity like v1..v15. v19 never runs it.
  ...
  # identity like v1..v16. v19 never runs it.
  ...
  # v18 is THIS file's DIRECT PREDECESSOR AND IT RAN, exactly once, against the
  # accepted Repair-25 controller. Its completed lab holds the exact Stage-1
  # refusal Repair 26 corrects. It is now gated by exact identity like v1..v17,
  # and that expectation is the identity read from disk when v19 was frozen.
  # v19 never runs it.
  ...
  # Launcher v19 is THIS file. It cannot hash-gate on itself, so it is accounted
  ...
  [ -s "${CTRL}/${LAUNCHER_V19_BASENAME}" ] \
    || die "launcher v19 is missing or empty at ${CTRL}/${LAUNCHER_V19_BASENAME}"
  note "all nineteen launcher artefacts present in ${CTRL}"
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${R26_CANDIDATE_BASENAME}" | LC_ALL=C sort
  )"
...
#
# The token identifies FOUR things: Repair 26, the v19 integrated rehearsal, the
# exact accepted Repair-26 controller sha256, and the routed signal this rehearsal
# carries forward. It therefore cannot collide with any earlier rehearsal marker.
SHADOW_TOKEN="nh-shadow-repair26-integrated-v19-${EXP_R26_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...
# disposable v19 shadow, which is the corrected Repair-19 behaviour this
...
if ! stage_may_continue; then
  ...
  if ! stage_may_continue; then
    ...
    if ! stage_may_continue; then
      ...
      #
      # WHATEVER EXECUTE REPORTS IS THE CONTROLLER RESULT, AND v19 RECORDS IT
...
# ==========================================================================
# SECTION 11b — v19 STAGE OBSERVATIONS
...
step "v19 STAGE OBSERVATIONS"

observe_v19_stages() {
  local out="${PROOF}/v19-stage-observations.txt"
  ...
  {
    printf 'v19 STAGE OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================================\n\n'
    ...
    printf '  refreshed to      : %s  (accepted Repair 26 candidate)\n' "$EXP_R26_SHA"
  ...
  note "v19 stage observations written to ${out}"
}
observe_v19_stages || true
...
observe_exact_regressions() {
...

  {
    ...
    printf '  the accepted Repair 26 controller. It assigns no verdict, passes\n'
    printf '  nothing and gates no stage.\n'
    printf '  THIS IS NOT v19\047S PURPOSE. v19 is the fresh integrated end-to-end\n'
    printf '  rehearsal of Repair 26; these three checks are retained historical\n'
    ...
    printf '  shadow controller  : %s  (accepted Repair 26 candidate)\n' "$EXP_R26_SHA"
    ...
    printf 'HOW A STAGE-1 OUTCOME IS READ IN v19\n'
    printf '  A. any of the three flags true  -- that exact older refusal appeared\n'
    printf '     again under Repair 26, in one message. Worth seeing; not a\n'
...
# invariant. The controller under test in v19 is ACCEPTED REPAIR 26, which is
...
# already independently audited and offline-tested; v19 is an INTEGRATED
...
observe_repair25_chatgpt_stop_handoff() {
...

  {
    ...
    printf 'CONTROLLER UNDER TEST\n'
    printf '  shadow controller : %s  (accepted Repair 26)\n' "$EXP_R26_SHA"
    printf '  Repair 26 is based on accepted Repair 25 and INHERITS the stop\n'
    printf '  behaviour observed below. This observer checks that INHERITED\n'
    printf '  Repair-25 seam; Repair 26 did not invent it.\n'
    ...
    if [ "$verdict" = "NOT-EXERCISED" ]; then
      ...
      printf '  is invoked to force the path. v19 is an integrated rehearsal, not\n'
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v19, ACCEPTED REPAIR-26, INTEGRATED) — RESULT\n'
printf '===========================================================\n'
printf 'SHADOW_LAB=%s\n'                        "$LAB"
printf 'REGRESSION_SOURCE=%s\n'                 "$REGRESSION_SOURCE"
printf 'REGRESSION_SOURCE_MODE=%s\n'            "$REGRESSION_SOURCE_MODE"
...
printf 'REPAIR26_ACCEPTED_BY_NESS=YES\n'
printf 'REPAIR26_ACCEPTANCE_AUTHORISES_ONLY_THIS_SHADOW_REFRESH=YES\n'
printf 'REPAIR26_INSTALLED=NO\n'
printf 'NH_ADOPTION_OR_ACCEPTANCE_PERFORMED=NO\n'
printf 'SHADOW_CONTROLLER_UNDER_TEST=REPAIR26\n'
printf 'SHADOW_CONTROLLER_SHA256=%s\n'          "$EXP_R26_SHA"
printf 'SHADOW_CONTROLLER_SOURCE=%s\n'          "$R26_CANDIDATE"
printf 'SHADOW_REFRESH_SOURCE_IS_REPAIR26_ONLY=YES\n'
...
printf 'HISTORICAL_REFUSAL_OBSERVATIONS_ARE_NOT_V19_PURPOSE=YES\n'
...
printf 'V19_INJECTED_A_FINDING_TO_FORCE_THE_BRANCH=NO\n'
printf 'V19_INVOKED_ROUTE_AUDIT_FINDING=NO\n'
printf 'V19_INVOKED_A_SECOND_QUESTION_VALIDATION=NO\n'
printf 'V19_INVOKED_NEXT_QUESTION_GROUP=NO\n'
printf 'V19_INVOKED_RECORD_NESS_ANSWER=NO\n'
printf '\n'
...
printf 'PROOF_V19_OBSERVATIONS=%s\n'                 "${PROOF}/v19-stage-observations.txt"
...
printf '  controller/nh_loop.py inside the disposable v19 shadow was replaced\n'
printf '  with the ACCEPTED REPAIR-26 CANDIDATE, which was opened read-only as\n'
printf '  the source and was never written to. Repair 26 was the ONLY refresh\n'
...
printf '  source and never a destination. Repair 20B, Repair 21, Repair 22,\n'
printf '  Repair 23, Repair 24 and Repair 25 are preserved repair evidence that\n'
printf '  this run read, proved unchanged, and never copied into the shadow.\n'
printf '  REPAIR 26 IS ACCEPTED BY NESS FOR ITS BOUNDED CONTROLLER-REPAIR SCOPE,\n'
...
printf '  accepted Repair-26 controller: the current accepted controller logic,\n'
printf '  run against the SAME preserved historical pre-question shadow state\n'
printf '  v18 used, inside the already-proven isolation model, through the four\n'
printf '  real stages -- question-validation, question-coverage-review,\n'
printf '  interview-gate and execute-next-claude-task -- to see what the real\n'
printf '  controller workflow actually does now.\n'
printf '\n  ITS IMMEDIATE PURPOSE. v18 RAN and stopped at Stage 1, fail-closed,\n'
printf '  with MATERIAL_UNKNOWN_UNRESOLVED, before Stages 2, 3 and 4 could run.\n'
printf '  That refusal was the repository-reference resolver blocker accepted\n'
printf '  Repair 26 corrects: intentionally preserved accepted/active duplicate\n'
printf '  documents proved byte-equivalent, and one unique same-directory\n'
printf '  extensionless Markdown alias. This run tests the real workflow after\n'
printf '  that bounded mechanical correction.\n'
printf '  IT DOES NOT ASSUME STAGE 1 MUST NOW PASS. The real Repair-26 controller\n'
printf '  was run once and whatever it did was recorded. A NEW legitimate\n'
printf '  fail-closed stop is a VALID RESULT, and no gate was loosened to reach\n'
printf '  Stage 2, 3 or 4. The completed v18 lab is preserved unchanged as the\n'
printf '  evidence of the refusal this run succeeds; it was never reused, never\n'
printf '  written to and never used as this run\047s seed.\n'
...
printf '  Repair 26 is based on accepted Repair 25 and INHERITS that behaviour;\n'
printf '  it did not invent it. If Stage 4 reached that stop,\n'
