#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v23.sh
#
...
#
# WHAT v23 CHANGES FROM v22, AND NOTHING ELSE
#   v22 WAS NEVER RUN. It was created, independently audited, and found
#   mechanically correct: exactly one outer stage, accepted Repair 28 as the sole
...
#   intact. v23 is ONE BOUNDED REPORT-TRUTH CORRECTION of that audited file.
#
...
#
#   THE DEFECT v23 CORRECTS. v22's executable behaviour was already v23-shaped --
#   Repair 28 under test, the v21 shadow as the source -- but a large body of
#   INHERITED CURRENT-RUN WORDING still described older runs. It named this
#   launcher v21, called the continuation source the completed v20 shadow, called
#   the source controller Repair 27A, and called Repair 27B the controller under
#   test. Two separate blocks each claimed, "uniquely", to be this run's
#   continuation source. Those statements were false about the run this launcher
#   would actually perform, and in places they contradicted each other.
#
#   WHAT v23 DOES NOT DO. It does not rewrite true history. v20 really did run
...
#   THIS LAUNCHER                v23.
#   ITS DIRECT FILE BASE         v22, which was created and audited but NEVER
#                                RUN. There is no v22 lab and no v22 result.
...
#   execution rehearsed here. v23 never invokes it, never injects a finding to
...
# IT IS NOT THIS FILE'S DIRECT PREDECESSOR (v22 IS) AND IT IS NOT THIS RUN'S
...
# identity read from disk when v21 was frozen. v23 never runs it.
...
# disk when v22 was frozen. v23 never runs it.
...
# v22 is THIS file's IMMEDIATE BASE. It was created and independently
# audited but WAS NEVER RUN, so there is no v22 lab and no v22 result to
# authenticate. It is gated by exact identity like every launcher before it.
...
#
# It WAS the refresh source of v15. Preserved repair evidence only in v23.
...
# written and, in v23, never copied.
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR28_CONTINUATION_V23_6d5e716fc6b2d777c4f65c726588a6e96b98ee67277f907c7d253ecc48edc172_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
report_state() {
  ...
  printf 'authenticated and for nothing else: in v23 it is never copied.\n' >&2
...
if [ "$PRESERVED_LAB_REPAIR25_V18" = "$REGRESSION_SOURCE" ] \
  ...
  die "the completed v18 lab is being used as this run's regression seed:
        ${REGRESSION_SOURCE}
      v23 continues from the completed v21 shadow, and its lineage seed is the
...
# direct predecessor (v22 is) and it is NOT this run's continuation source (the
...
# --------------------------------------------------------------------------
#
# THE WHOLE PREMISE OF v23 RESTS ON THIS GATE. v23 does not rerun Doors 1, 2 and
...
# --------------------------------------------------------------------------
#
# v23 continues the exact state the Repair-27B CLAUDE_SPECIFICATION_STOP
  ...
  if [ -d "$PROOF" ]; then
    {
      ...
      printf 'is why v23 does not rerun them,\n'
...
assert_frozen_identities() {
  ...
  # THE ACCEPTED REPAIR-27A CANDIDATE -- PRESERVED REPAIR EVIDENCE in v23, and
  ...
  # source. It is gated by exact identity like v1..v20. v23 never runs it.
  ...
  # v22 is THIS file's IMMEDIATE BASE. It was created and audited but NEVER
  # RUN, so it has no lab and no result of its own. It is gated by exact
  # identity like v1..v21. v23 never runs it.
  [ -f "${CTRL}/${LAUNCHER_V22_BASENAME}" ] \
    || die "launcher v22 is missing from ${CTRL}; it is this file's audited base and must be present"
  ...
  # Launcher v23 is THIS file. It cannot hash-gate on itself, so it is accounted
  # for BY PRESENCE, and it is carried in the before/during/after proof set so any
  # change to it during the run is caught there instead.
  [ -s "${CTRL}/${LAUNCHER_V23_BASENAME}" ] \
    || die "launcher v23 is missing or empty at ${CTRL}/${LAUNCHER_V23_BASENAME}"
  note "all twenty-three launcher artefacts present in ${CTRL}"
...
# carries.
SHADOW_TOKEN="nh-shadow-repair28-continuation-v23-${EXP_R28_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...
observe_v23_stage() {
  local out="${PROOF}/v23-stage-observations.txt"
  local s4="$S4_OUT"
...

  {
    ...
    printf '\n'
    printf '  THIS IS THE COMPLETED v21 SHADOW, THE EXACT STATE THE REPAIR-27B\n'
    ...
    printf '  whole against the preserved v11 witness, and in v23 it was never\n'
    ...
    printf '  lineage seed      : %s  (Repair 17, preserved, NOT copied in v23)\n' "$EXP_SEED_LOOP_SHA"
...
# invariant. The controller under test in v23 is ACCEPTED REPAIR 28, which is
...
# already independently audited and offline-tested; v23 is a CONTINUATION
...
observe_repair25_chatgpt_stop_handoff() {
...

  {
    ...
    if [ "$verdict" = "NOT-EXERCISED" ]; then
      ...
      printf '  is invoked to force the path. v23 is an integrated rehearsal, not\n'
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v23, ACCEPTED REPAIR-28, STAGE-4-ONLY CONTINUATION) — RESULT\n'
printf '===========================================================\n'
printf 'LAUNCHER=%s\n'                          "$LAUNCHER_V23_BASENAME"
printf 'SHADOW_LAB=%s\n'                        "$LAB"
...
printf 'HISTORICAL_LINEAGE_SEED_COPIED_IN_V23=NO\n'
printf '\n'
...
printf 'PREDECESSOR_LAB=NONE_V22_WAS_NOT_RUN\n'
printf '\n'
...
printf '\n'
printf 'DOORS_1_TO_3_RERUN_BY_V23=NO\n'
printf 'DOOR1_QUESTION_VALIDATION_STAGE_EXISTS_IN_V23=NO\n'
printf 'DOOR2_QUESTION_COVERAGE_REVIEW_STAGE_EXISTS_IN_V23=NO\n'
printf 'DOOR3_INTERVIEW_GATE_STAGE_EXISTS_IN_V23=NO\n'
...
printf 'V23_INJECTED_A_FINDING_TO_FORCE_THE_BRANCH=NO\n'
printf 'V23_INVOKED_ROUTE_AUDIT_FINDING=NO\n'
printf 'V23_INVOKED_QUESTION_VALIDATION=NO\n'
printf 'V23_INVOKED_QUESTION_COVERAGE_REVIEW=NO\n'
printf 'V23_INVOKED_INTERVIEW_GATE=NO\n'
printf 'V23_INVOKED_NEXT_QUESTION_GROUP=NO\n'
printf 'V23_INVOKED_RECORD_NESS_ANSWER=NO\n'
printf 'V23_INVOKED_DECLARE_STRANDED_CANDIDATE=NO\n'
printf 'V23_MANUFACTURED_A_NESS_QUESTION=NO\n'
printf 'V23_MANUFACTURED_A_FINDING=NO\n'
printf '\n'
...
printf 'PROOF_V23_OBSERVATIONS=%s\n'                 "${PROOF}/v23-stage-observations.txt"
...
printf '  preserved v11 witness, and NEVER COPIED in v23. The PROTECTED ORIGINAL\n'
...
printf '  controller/nh_loop.py inside the disposable v23 shadow was replaced\n'
printf '  with the ACCEPTED REPAIR-27B CANDIDATE, which was opened read-only as\n'
printf '  the source and was never written to. Repair 28 was the ONLY refresh\n'
