#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v25.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v25
#   (THE ACCEPTED REPAIR-28 CONTROLLER, STAGE-4-ONLY CONTINUATION OF THE
#    COMPLETED v21 SHADOW, WITH DOORS 1-3 NOT RERUN)
#
# WHAT v25 CHANGES FROM v24, AND NOTHING ELSE
#   v24 WAS NEVER RUN. It was created and audited, and its executable mechanics
...
#   intact. v24's report-truth correction was, however, STILL INCOMPLETE. An
#   independent audit of the ACTUAL v24 file found that v24's own
#   "PROVENANCE OF THIS RUN, STATED ONCE AND EXACTLY" block still named an older
#   launcher; that v24's Repair-27A section still gave Repair 27A a live role it
#   no longer had; that v24's Repair-27B section still gave Repair 27B a live
#   role it no longer had; and that a body of stale wording in v24's comments,
#   embedded-program comments, failure messages and printed report text still
#   named older runs. v25 COMPLETES that correction and nothing else.
#
#   ONE OF v24'S DEFECTS WAS MACHINE-READABLE, NOT COSMETIC. v24 printed a
#   predecessor NAME taken from one launcher generation while printing the
#   predecessor SHA256 and BYTE COUNT taken from a different one, so a reader of
#   v24's result block would have been told a name and a digest belonging to two
#   different files. v25 freezes the exact v24 identity and binds all three
#   predecessor fields to that single frozen identity.
#
...
#   forward from v24 unchanged.
#
#   WHAT v25 DOES NOT DO. It does not rewrite true history. v20 really did run
...
#   CLAUDE_SPECIFICATION_STOP. v22, v23 and v24 were each created and never run.
#   Every statement describing those events is preserved as history and is
#   labelled as history. Only wording that purported to describe THIS launcher,
#   THIS run, THIS continuation source, THIS refresh, THIS controller under test
#   or THIS result was corrected.
#
# THE PROVENANCE OF THIS RUN, STATED ONCE AND EXACTLY
#   THIS LAUNCHER                v25.
#   ITS DIRECT FILE BASE         v24, which was created and audited but NEVER
#                                RUN. There is no v24 lab and no v24 result.
...
#                                inside the completed v21 shadow. It is the FROM
#                                side of this run's one refresh, and it is NOT
#                                this run's refresh source.
...
#   PRESERVED HISTORICAL REPAIR  Repair 27A. It was v20's refresh source. It is
#                                NOT the controller this run's continuation
#                                source carries and it is NOT copied anywhere.
...
#   MISMATCH, and to nothing else. It is the controller v25 puts under test.
#
...
#
# WHAT v25 IS, AND WHAT IT IS NOT
...
#   execution rehearsed here. v25 never invokes it, never injects a finding to
#   force it, never repairs it, and never treats its being open as a v25 failure.
...
#     v20 itself was seeded from. In v25 it is PRESERVED EVIDENCE ONLY: it is
...
#   v25's. Every identity THIS run asserts is in SECTION 0 and nowhere else: the
...
#   lab and marker keyed to REPAIR28/V25, and the seed at EXP_SEED_LOOP_SHA.
#
...
readonly LAUNCHER_V25_BASENAME="NH_SHADOW_REHEARSAL_LAUNCHER_v25.sh"
...
# IT IS NOT THIS FILE'S DIRECT PREDECESSOR (v24 IS) AND IT IS NOT THIS RUN'S
...
#
# NOTHING ABOUT v20 IS RETRACTED BY v25 EXISTING. v20 was correct for the
...
# identity read from disk when v21 was frozen. v25 never runs it.
...
# disk when v22 was frozen. v25 never runs it.
...
# It is v23's base, not this file's; this file's immediate base is v24 below.
...
# v23 IS PRESERVED LAUNCHER HISTORY. It was created and audited but WAS NEVER
# RUN, so there is no v23 lab and no v23 result to authenticate. It is v24's
# base, not this file's. It is gated by exact identity like every launcher
# before it.
...
# v24 is THIS file's IMMEDIATE BASE. It was created and independently audited but
# WAS NEVER RUN, so there is no v24 lab and no v24 result to authenticate. These
# are the bytes read from disk when v25 was frozen, and they are what the
# predecessor fields in the RESULT block report.
readonly EXP_LAUNCHER_V24_SHA="0bf716250e9838ce88b242da5388f6aae672c95801421575a9ab2fdd51f0a2bd"
readonly EXP_LAUNCHER_V24_BYTES="407278"
...
#
# It WAS the refresh source of v15. Preserved repair evidence only in v25.
...
#     (PRESERVED HISTORICAL REPAIR EVIDENCE ONLY) ---
#
# It WAS the refresh source of v20. It remains ACCEPTED. In v25 it is preserved
# repair evidence and nothing more: it is gated by exact identity, witnessed in
# the before/during/after sweep, never written to, and NOT copied into the shadow
# by this launcher on any path.
#
# IT IS NOT THE CONTROLLER INSIDE THIS RUN'S CONTINUATION SOURCE. v20 ran under
# Repair 27A, but the state this launcher continues is the COMPLETED v21 SHADOW,
# which was run under accepted Repair 27B. The controller sitting inside the
# continuation source is therefore Repair 27B, and EXP_R27B_SHA -- not
# EXP_R27A_SHA -- is what the new copy's controller must be BEFORE the refresh.
#
# THE REFRESH IN SECTION 7b IS Repair 27B -> Repair 28, IN THE NEW COPY ONLY.
# Repair 27A is on neither side of it.
...
#
# v21 TESTED THAT CORRECTION, AND IT IS NOT WHAT THIS LAUNCHER EXISTS TO TEST.
# v25 exists to exercise accepted Repair 28. Repair 27B's role here is twofold
# and historical: it is preserved accepted repair evidence, and it is the SOURCE
# controller stored inside the completed v21 shadow -- the identity the new copy
# must equal BEFORE the refresh. This launcher implements none of Repair 27B,
# interprets none of it, and contains no preparation, specification, task_ready
# or not_ready_reason logic in any executable path. All of it lives inside
# nh_loop.py.
#
# NESS'S ACCEPTANCE OF REPAIR 28, NOT OF REPAIR 27B, IS WHAT AUTHORISES THIS
# SHADOW REFRESH. Repair 27B remains accepted for its own bounded scope, but it
# is NOT copied anywhere by this launcher on any path: the only controller
# candidate ever copied over the shadow's controller/nh_loop.py is
# nh_loop_REPAIR28_CANDIDATE.py. Neither candidate is ever renamed to nh_loop.py
# on the real filesystem, ever copied over the real controller, or ever promoted.
#
...
# --- ACCEPTED REPAIR 28: THE SOLE SHADOW-CONTROLLER REFRESH SOURCE OF v25 ---
#
...
#
# IT IS NOT INSTALLED. It is copied ONLY into the fresh disposable v25 shadow,
...
# written and, in v25, never copied.
...
# skipping Doors 1 to 3 is unjustified and the whole premise of v25 is false. So
...
# --------------------------------------------------------------------------
#
# WHY THIS GATE EXISTS. v25 continues the state the v21 CLAUDE_SPECIFICATION_STOP
# happened in. If the recorded v21 result is not what this launcher believes it
# is, the whole premise of v25 is false. So the v21 result is PROVED, from the
...
# --- lab: NEW, keyed to REPAIR28, to V25, to the exact accepted Repair-28
#     controller identity (EXP_R28_SHA) AND to the routed signal this rehearsal
#     carries forward.
#
...
#
# THE COMPLETED v21 LAB PATH IS NOT THIS PATH. v21 DID run, so its lab exists; it
# is PRESERVED_LAB_REPAIR27B_V21 above, it is this run's READ-ONLY continuation
# source, and this launcher will neither reuse nor delete it. The completed v20
# lab is a separate preserved path and is not this path either.
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR28_CONTINUATION_V25_6d5e716fc6b2d777c4f65c726588a6e96b98ee67277f907c7d253ecc48edc172_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
# invariant. The controller under test here is accepted Repair 28, which is
# built, through 27B, 27A, 27, 26, on accepted Repair 25 and INHERITS that
# behaviour unchanged; Repair 28 did not invent it, and renaming the field would
# claim otherwise. Repair 27B is an inherited intermediate on that chain and is
# this run's SOURCE controller, not the controller being observed under test.
#
...
report_state() {
  ...
  printf 'authenticated and for nothing else: in v25 it is never copied.\n' >&2
...
capture_original_proof() {
  ...
  #
  # PRESERVED HISTORICAL REPAIR EVIDENCE. It is NOT this run's refresh source
  # and NOT the controller under test; accepted Repair 28 is both.
  # Captured before, during and after exactly like the real controller, so the
  # sweep proves the file this rehearsal reads was not edited underneath it.
  {
  ...
  #
  # PRESERVED SOURCE-CONTROLLER EVIDENCE: the controller stored inside the
  # completed v21 shadow. It is NOT this run's refresh source and NOT the
  # controller under test; accepted Repair 28 is both.
  # Captured before, during and after exactly like the real controller, so the
  ...
  #                                   v21 shadow -- journal, head, key, marker,
...
if [ "$PRESERVED_LAB_REPAIR25_V18" = "$REGRESSION_SOURCE" ] \
  ...
  die "the completed v18 lab is being used as this run's regression seed:
        ${REGRESSION_SOURCE}
      v25 continues from the completed v21 shadow, and its lineage seed is the
...
# direct predecessor (v24 is) and it is NOT this run's continuation source (the
...
# the false v11 question, and it is the lineage root of the completed v21 shadow
# this run actually continues from -- v20 is an intermediate historical run on
# that same lineage. Its complete authentication is carried
...
# one way only: there is no older independent witness of the completed v21
...
if FROZEN_ONLY:
    ...
    #
    # There is no older independent witness of the completed v21 shadow, and none
    ...
    # that carry the rest -- the Repair-27B controller identity, the authenticity
    ...
    # v21 marker -- are asserted by the shell gate and are not derived from this
    # manifest.
    live = build_from_live(root)
...
# --------------------------------------------------------------------------
#
# THE WHOLE PREMISE OF v25 RESTS ON THIS GATE. v25 does not rerun Doors 1, 2 and
  ...
  for needle in "$EXP_V20_S4_CRASH_SUBSCRIPT" \
    ...
    if ! grep -q -F -- "$needle" "${s4}.stderr"; then
      ...
      Repair-27A crash. This line was required and is absent:
        ${needle}
      v21 WAS CREATED to continue from THAT crash, and v25 continues from
      v21's own recorded stop, so this ancestor evidence must be exactly what
      it is recorded to be. Refusing to continue from a Stage-4 result that is
      not the one recorded. Nothing is reinterpreted and no v20 stage is
      rerun."
    fi
  done
  ...
  if ! grep -q -F -- "$needle" "$file"; then
    die "the preserved v21 Stage-4 report does not carry a required recorded
      fact. This exact line was required and is absent:
        ${needle}
      v25 exists to continue from THAT recorded stop. Refusing to continue from
...
# --------------------------------------------------------------------------
#
# v25 continues the exact state the Repair-27B CLAUDE_SPECIFICATION_STOP
  ...
  if [ -d "$PROOF" ]; then
    {
      ...
      printf 'is why v25 does not rerun them,\n'
...
#
# THE REFRESH SOURCE IS THE ACCEPTED REPAIR-28 CANDIDATE, OPENED READ-ONLY.
# The new copy's controller must be accepted Repair 27B BEFORE the copy and
# accepted Repair 28 AFTER it. Repair 27B is the FROM side, never a copy source.
#
...
refresh_new_copy_controller() {
  ...
  # shadow was refreshed to Repair 28 and to nothing else" measurable rather
...
# still carries the continuation source's own v21 marker, because the v25 marker
...
assert_frozen_identities() {
  ...
  # THE ACCEPTED REPAIR-27A CANDIDATE -- PRESERVED REPAIR EVIDENCE in v25, and
  ...
  # predecessor, and is gated by identity exactly like v1..v10. v25 never runs it.
  ...
  # predecessor, and is gated by identity exactly like v1..v11. v25 never runs it.
  ...
  # identity like v1..v12. v25 never runs it.
  ...
  # identity like v1..v13. v25 never runs it.
  ...
  # witnesses rather than from today's filename. v25 never runs it.
  ...
  # gated by exact identity like v1..v15. v25 never runs it.
  ...
  # identity like v1..v16. v25 never runs it.
  ...
  # by exact identity like v1..v17. v25 never runs it.
  ...
  # is the identity read from disk when v20 was frozen. v25 never runs it.
  ...
  # file's predecessor (v24 is) and it is NOT this run's continuation source (the
  ...
  # source. It is gated by exact identity like v1..v20. v25 never runs it.
  ...
  # It is gated by exact identity like v1..v21. v25 never runs it.
  ...
  # v23 is PRESERVED LAUNCHER HISTORY. It was created and audited but NEVER RUN,
  # so it has no lab and no result of its own. It is v24's base, not this file's.
  # It is gated by exact identity like v1..v22. v25 never runs it.
  [ -f "${CTRL}/${LAUNCHER_V23_BASENAME}" ] \
    || die "launcher v23 is missing from ${CTRL}; it is preserved launcher history and must be present"
  ...
  # v24 is THIS file's IMMEDIATE BASE. It was created and audited but NEVER RUN,
  # so it has no lab and no result of its own. It is gated by exact identity like
  # v1..v23, and that expectation is the identity read from disk when v25 was
  # frozen. v25 never runs it.
  [ -f "${CTRL}/${LAUNCHER_V24_BASENAME}" ] \
    || die "launcher v24 is missing from ${CTRL}; it is this file's audited base and must be present"
  expect "launcher v24 sha256" "$(sha_of "${CTRL}/${LAUNCHER_V24_BASENAME}")"   "$EXP_LAUNCHER_V24_SHA"
  expect "launcher v24 bytes"  "$(bytes_of "${CTRL}/${LAUNCHER_V24_BASENAME}")" "$EXP_LAUNCHER_V24_BYTES"

  # Launcher v25 is THIS file. It cannot hash-gate on itself, so it is accounted
  ...
  [ -s "${CTRL}/${LAUNCHER_V25_BASENAME}" ] \
    || die "launcher v25 is missing or empty at ${CTRL}/${LAUNCHER_V25_BASENAME}"
  note "all twenty-five launcher artefacts present in ${CTRL}"
  ...
  # modified nh_loop.py, the twenty-five launcher artefacts, the eleven preserved
  # repair candidates, the gitignored editor file, and the gitignored controller
  # bytecode. Nothing is filtered out.
  #
  # THE v25 LINE IS THE ONLY ADDITION v25 MAKES TO THIS SET. It is this file,
...
# comparison -- both legitimately saw the v21 marker on both sides and matched it.
#
# It sits at the workspace root, outside both Git checkouts, so neither
# repository worktree status is affected.
#
# The token identifies FOUR things: Repair 28, the v25 Stage-4-only
...
# carries.
SHADOW_TOKEN="nh-shadow-repair28-continuation-v25-${EXP_R28_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...

{
  ...
  printf 'STAGE 4 execute-next-claude-task (the only stage v25 runs)\n'
  printf '  exit                          %s\n' "$EXEC_RC"
  printf '  ok                            %s\n' "$OBS_S4_OK"
...
# ==========================================================================
# SECTION 11b — v25 STAGE OBSERVATIONS
...
# truncated.
step "v25 STAGE OBSERVATIONS"

observe_v25_stage() {
  local out="${PROOF}/v25-stage-observations.txt"
  local s4="$S4_OUT"
  ...
  {
    printf 'v25 STAGE OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================================\n\n'
    ...
    printf '  whole against the preserved v11 witness, and in v25 it was never\n'
    ...
    printf '  lineage seed      : %s  (Repair 17, preserved, NOT copied in v25)\n' "$EXP_SEED_LOOP_SHA"
  ...
  note "v25 stage observations written to ${out}"
}
observe_v25_stage || true
...
# invariant. The controller under test in v25 is ACCEPTED REPAIR 28, which is
...
# already independently audited and offline-tested; v25 is a CONTINUATION
...
observe_repair25_chatgpt_stop_handoff() {
...

  {
    ...
    if [ "$verdict" = "NOT-EXERCISED" ]; then
      ...
      printf '  is invoked to force the path. v25 is an integrated rehearsal, not\n'
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v25, ACCEPTED REPAIR-28, STAGE-4-ONLY CONTINUATION) — RESULT\n'
printf '===========================================================\n'
printf 'LAUNCHER=%s\n'                          "$LAUNCHER_V25_BASENAME"
printf 'SHADOW_LAB=%s\n'                        "$LAB"
...
printf 'HISTORICAL_LINEAGE_SEED_COPIED_IN_V25=NO\n'
printf '\n'
...
printf '\n'
printf 'DOORS_1_TO_3_RERUN_BY_V25=NO\n'
printf 'DOOR1_QUESTION_VALIDATION_STAGE_EXISTS_IN_V25=NO\n'
printf 'DOOR2_QUESTION_COVERAGE_REVIEW_STAGE_EXISTS_IN_V25=NO\n'
printf 'DOOR3_INTERVIEW_GATE_STAGE_EXISTS_IN_V25=NO\n'
...
printf 'V25_INJECTED_A_FINDING_TO_FORCE_THE_BRANCH=NO\n'
printf 'V25_INVOKED_ROUTE_AUDIT_FINDING=NO\n'
printf 'V25_INVOKED_QUESTION_VALIDATION=NO\n'
printf 'V25_INVOKED_QUESTION_COVERAGE_REVIEW=NO\n'
printf 'V25_INVOKED_INTERVIEW_GATE=NO\n'
printf 'V25_INVOKED_NEXT_QUESTION_GROUP=NO\n'
printf 'V25_INVOKED_RECORD_NESS_ANSWER=NO\n'
printf 'V25_INVOKED_DECLARE_STRANDED_CANDIDATE=NO\n'
printf 'V25_MANUFACTURED_A_NESS_QUESTION=NO\n'
printf 'V25_MANUFACTURED_A_FINDING=NO\n'
printf '\n'
...
printf 'PROOF_V25_OBSERVATIONS=%s\n'                 "${PROOF}/v25-stage-observations.txt"
...
printf '  This run is v25. It continues the completed v21 shadow, and the Doors\n'
...
printf '  preserved v11 witness, and NEVER COPIED in v25. The PROTECTED ORIGINAL\n'
...
printf '  controller/nh_loop.py inside the disposable v25 shadow was verified to\n'
...
printf '  repaired the dependency, and does not treat its being open as a v25\n'
