...
# --- controller: THE REPAIR-21 CANDIDATE (the v15 refresh source) ---
#
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR21_V15_a54f9e5ffd77470e9c0eb60552aa207e5ab24c8b45bff09765fe552fa5c910cd_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
for _plab in "$PRESERVED_LAB_V1" "$PRESERVED_LAB_REPAIR9" \
             ...
             "$PRESERVED_LAB_REPAIR20B_V14"; do
  if [ "$LAB" = "$_plab" ]; then
...
refresh_new_copy_controller() {
  local target="${SHADOW_WS}/controller/nh_loop.py"
  local staging="${SHADOW_WS}/controller/.nh_loop.py.repair21-refresh"
  ...
  [ -L "$R21_CANDIDATE" ] && die "the Repair-21 candidate is a SYMLINK:
        ${R21_CANDIDATE}
      Refusing to refresh the shadow through it."
  [ -f "$R21_CANDIDATE" ] \
    || die "the Repair-21 candidate is missing at ${R21_CANDIDATE}"
  expect "refresh source sha256 (Repair 21 candidate)" \
         "$(sha_of "$R21_CANDIDATE")"   "$EXP_R21_SHA"
  expect "refresh source bytes  (Repair 21 candidate)" \
         "$(bytes_of "$R21_CANDIDATE")" "$EXP_R21_BYTES"
  expect "refresh source lines  (Repair 21 candidate)" \
         "$(wc -l < "$R21_CANDIDATE")"  "$EXP_R21_LINES"

  # AND REPAIR 20B IS RE-ASSERTED UNCHANGED AT THE SAME MOMENT. It is preserved
  # repair evidence, it is not the source, and proving it here is what makes
  # "the shadow was refreshed to Repair 21 and to nothing else" measurable.
  ...
  cp -- "$R21_CANDIDATE" "$staging" \
    || die "could not stage the Repair-21 candidate into the new shadow copy:
        ${staging}
      Nothing has been renamed over the new copy's controller."
  chmod 0644 -- "$staging" \
    || die "could not set the mode on the staged Repair-21 candidate: ${staging}"
  mv -f -- "$staging" "$target" \
    || die "could not move the staged Repair-21 candidate into place:
        ${target}"
  CONTROLLER_REFRESH="PERFORMED"
  note "the new copy's controller/nh_loop.py refreshed to the Repair-21 candidate"
}
...
assert_new_copy_after_refresh() {
  ...
  expect "new copy controller sha256 AFTER refresh (Repair 21)" \
         "$(sha_of "$target")"   "$EXP_R21_SHA"
  expect "new copy controller bytes  AFTER refresh (Repair 21)" \
         "$(bytes_of "$target")" "$EXP_R21_BYTES"
  expect "new copy controller lines  AFTER refresh (Repair 21)" \
         "$(wc -l < "$target")"  "$EXP_R21_LINES"
  expect "real controller sha256 unchanged AFTER refresh (Repair 20)" \
         "$(sha_of "${CTRL}/nh_loop.py")" "$EXP_LOOP_SHA"
  note "the refreshed shadow controller is the exact Repair-21 identity"
}
...
assert_new_copy_differs_only_by_controller() {
  ...
  cmp_out="$(
    PYTHONDONTWRITEBYTECODE=1 "$PY3" -B -c "$TREE_COMPARATOR" \
      ...
      "NEWCOPY" \
      "SEED" \
      "DIFFERS_EXACT:${REFRESH_ALLOWED_EXACT_LOOP}:${EXP_R21_SHA}:${EXP_SEED_LOOP_SHA}" 2>&1
  )"
  cmp_rc=$?
  set -e
...
assert_frozen_identities() {
  ...
  # THE REPAIR-21 CANDIDATE -- the refresh source, gated exactly as strictly as
  # the real controller and equally never written to.
  [ -L "$R21_CANDIDATE" ] && die "the Repair-21 candidate is a SYMLINK: ${R21_CANDIDATE}"
  [ -f "$R21_CANDIDATE" ] \
    || die "the Repair-21 candidate is missing from ${CTRL}; it is the one file
  ...
  # Launcher v15 is THIS file. It cannot hash-gate on itself, but it must be the
  # file that is present, and it is carried in the before/after proof set.
  [ -s "${CTRL}/${LAUNCHER_V15_BASENAME}" ] \
    || die "launcher v15 is missing or empty at ${CTRL}/${LAUNCHER_V15_BASENAME}"
  note "all fifteen launcher artefacts present in ${CTRL}"
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${R21_CANDIDATE_BASENAME}" | LC_ALL=C sort
  )"
...
SHADOW_TOKEN="nh-shadow-repair21-regression-v15-${EXP_R21_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...
step "REPAIR-21 REGRESSION V15 OBSERVATIONS"

observe_repair21_regression_v15() {
  local out="${PROOF}/repair21-regression-v15-observations.txt"
  ...
  {
    printf 'REPAIR-21 REGRESSION V15 OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================================\n\n'
    ...
    printf '  refreshed to      : %s  (Repair 21 candidate)\n' "$EXP_R21_SHA"
...
observe_repair21_regression_v15 || true
...
step "REPAIR-21 EXACT REGRESSION OBSERVATION"
...
observe_exact_regressions() {
  local out="${PROOF}/repair21-exact-regression.txt"
  ...
  local r20b="false" r21="false" b9_anywhere="no" stage1_ran="yes"
  ...
  {
    printf 'REPAIR-21 / REPAIR-20B EXACT REGRESSION OBSERVATION -- descriptive only\n'
    printf '======================================================================\n\n'
    printf 'WHAT THIS OBSERVES\n'
    printf '  whether either exact prior Stage-1 refusal recurs under Repair 21.\n'
    printf '  It assigns no verdict, passes nothing and gates no stage.\n\n'
    printf 'HOW A RECURRENCE IS DECIDED (CORRECTED IN v15)\n'
    ...
    printf '  shadow controller  : %s  (Repair 21 candidate)\n' "$EXP_R21_SHA"
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v15, REPAIR-21 REGRESSION) — RESULT\n'
printf '===========================================================\n'
printf 'SHADOW_LAB=%s\n'                        "$LAB"
printf 'REGRESSION_SOURCE=%s\n'                 "$REGRESSION_SOURCE"
printf 'REGRESSION_SOURCE_MODE=%s\n'            "$REGRESSION_SOURCE_MODE"
...
printf 'SHADOW_CONTROLLER_UNDER_TEST=REPAIR21\n'
...
printf 'PROOF_V15_OBSERVATIONS=%s\n'                 "${PROOF}/repair21-regression-v15-observations.txt"
printf 'PROOF_REPAIR21_REGRESSION=%s\n'              "${PROOF}/repair21-exact-regression.txt"
printf '===========================================================\n'
