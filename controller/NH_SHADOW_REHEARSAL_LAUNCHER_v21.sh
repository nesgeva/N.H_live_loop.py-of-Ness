#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v21.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v21
#   (THE ACCEPTED REPAIR-27B CONTROLLER, STAGE-4-ONLY CONTINUATION OF THE
#    COMPLETED v20 SHADOW, WITH DOORS 1-3 NOT RERUN)
#
# WHAT v21 CHANGES FROM v20, AND NOTHING ELSE
#   v20 WAS RUN, exactly once, and it completed. It is not corrected here and
#   nothing about it is retracted: it did precisely what it was built to do, and
#   its Stages 1, 2 and 3 all PASSED. v21 is the SMALLEST SAFE CONTINUATION of
#   that completed run onto the accepted Repair-27B controller, and it changes
#   only what a STAGE-4-ONLY continuation mechanically requires:
#
#     1. VERSION PLUMBING. v20 -> v21 throughout the CURRENT-run identity: the
...
#     2. THE EXACT PREDECESSOR, FROZEN. v20 is now a preserved launcher like
#        every one before it, gated by exact sha256 and byte count.
#     3. A FRESH v21 LAB AND MARKER, keyed to REPAIR27B/V21, to the exact
#        accepted Repair-27B controller identity, and to the routed signal this
#        rehearsal carries forward. No lab is ever reused.
#     4. THE COMPLETED v20 LAB BECOMES BOTH PRESERVED EVIDENCE AND THIS RUN'S
#        CONTINUATION SOURCE. It holds the exact Stage-4 crash Repair 27B exists
#        to correct AND the Doors-1-to-3 clearance this continuation carries
#        forward. It is named here for collision refusal, presence assertion,
#        result authentication, human reporting and as the READ-ONLY source side
#        of one rsync. It is never written to, never reused as a lab, never
#        normalised and never deleted.
#     5. REPAIR 27A BECOMES PRESERVED REPAIR EVIDENCE; REPAIR 27B BECOMES THE
#        SOLE REFRESH SOURCE. Repair 27A is still gated by exact identity and
#        still witnessed unmoved -- it is simply not copied anywhere. It remains
#        accepted, and it is the controller the continuation source carries.
#     6. THE v20 RESULT IS AUTHENTICATED BEFORE IT MAY BE CONTINUED FROM. The
#        recorded Stage-1/2/3 success and the exact Stage-4 Repair-27A crash are
#        proved from the preserved v20 artefacts themselves, by frozen digest AND
#        by re-reading the controller's own reported fields. Nothing is
#        reinterpreted and nothing is regenerated.
#     7. EXACTLY ONE OUTER CONTROLLER STAGE. execute-next-claude-task, once.
#        question-validation, question-coverage-review and interview-gate are NOT
#        run as stages by this launcher, and no constant, call site or command
#        string able to run them exists in any executable line of this file.
#
...
#   preserved-lab proofs, the credential witness, the tree comparator, the report
#   parser, run_controller_stage() and every fail-closed path are carried forward
#   exactly as v20 had them.
#
# WHAT THE COMPLETED v20 RUN ACTUALLY DID
...
#
#   THAT CRASH IS THE WHOLE REASON THIS LAUNCHER EXISTS, AND IT IS NOT A STOP.
#   An ordinary controller fail-closed stop writes a machine report and exits 1
#   with a stop_reason. This one produced NO report at all: stdout is zero bytes.
#   run_prepare_next_claude_task read
#       prepared["mechanical_design_specification"]["requirements"]
#   unconditionally, while a task_ready = false preparation legitimately carries
#   no mechanical design specification, so the subscript raised TypeError.
#
#   ACCEPTED REPAIR 27B IS THE BOUNDED CORRECTION TO EXACTLY THAT, AND TO NOTHING
#   ELSE. It reads the requirement count only where a specification exists, and
#   otherwise leaves the field at the None it was initialised to -- not zero,
#   which would falsely report a specification carrying no requirements, and not
#   an invented empty specification -- and appends one plain line saying no new
#   specification was required. It is a report-only block: which kind of
#   task_ready = false this is remains read downstream from the bounded
#   not_ready_reason field alone, exactly as before. THIS LAUNCHER IMPLEMENTS
#   NONE OF THAT, interprets none of it, and contains no preparation,
#   specification or task_ready logic in any executable path. All of it lives
#   inside nh_loop.py.
#
# WHY A STAGE-4-ONLY CONTINUATION IS LEGITIMATE, AND WHY IT IS NOT A SHORTCUT
#   Doors 1, 2 and 3 are not skipped. They were RUN, for real, by v20, against
#   this exact state, with two real GPT reviews, and they PASSED. Their outcome
#   is not a memory of a report: it is recorded in the interview journal that
#   this continuation carries forward BYTE-IDENTICALLY -- nine authenticated,
#   chained events whose ninth is the question_coverage_review_recorded clearance
#   itself. Rerunning them would burn two more real provider calls to re-derive a
#   clearance that is already durably recorded, and would produce a DIFFERENT
#   state from the one the crash occurred in.
#
...
#   under Repair 27B, it fails closed, and that is a VALID RESULT this launcher
#   records without working around.
#
# WHAT v21 IS, AND WHAT IT IS NOT
#   It is the Stage-4-only continuation rehearsal of the accepted Repair-27B
#   controller: the current accepted controller logic, run against the EXACT
#   post-Doors-1-to-3 shadow state the v20 crash occurred in, inside the
#   already-proven isolation model, to see what execute-next-claude-task actually
#   does now that the preparation-report crash is corrected.
#
...
#   Repair-27B controller once and records whatever the real controller does. A
...
#
#   REPAIRS 20B, 21, 22, 23, 24, 25, 26, 27 AND 27A ARE PRESERVED REPAIR EVIDENCE
#   ONLY. Not one of them is a refresh source. Repair 27B is the SOLE refresh
#   source.
#
...
#   execution rehearsed here. v21 never invokes it, never injects a finding to
#   force it, never repairs it, and never treats its being open as a v21 failure.
...
#   controller workspace has not moved since this launcher was constructed, proves
#   the completed v20 result is what it is recorded to be, makes an exact
#   throwaway copy of THE PRESERVED v20 SHADOW -- the post-Doors-1-to-3 state the
#   Repair-27A crash occurred in -- REFRESHES EXACTLY ONE FILE IN THAT NEW COPY,
#   and only in the new copy, to the accepted Repair-27B controller, hides the
...
#     THE CONTINUATION SOURCE is the completed v20 shadow. It is the state this
...
#     v20 itself was seeded from. In v21 it is PRESERVED EVIDENCE ONLY: it is
...
#   source proof.
#
# WHAT NESS'S ACCEPTANCE OF REPAIR 27B DOES AND DOES NOT AUTHORISE
#   Ness has explicitly accepted Repair 27B for its bounded controller-repair
#   scope and authorised continuation of the shadow rehearsal. In THIS FILE that
#   acceptance authorises exactly one thing: using nh_loop_REPAIR27B_CANDIDATE.py
#   as the sole controller refresh source for the disposable shadow below.
#
#   REPAIR 27A REMAINS ACCEPTED, AND IN THIS FILE IT IS PRESERVED EVIDENCE ONLY.
#   Repair 27B is based on Repair 27A; that acceptance is not withdrawn by this
#   file, and Repair 27A is never copied anywhere by this file.
#
...
#   about any preserved rehearsal lab including the completed v20 lab. Those
...
#   lineage seed and against every preserved lab -- the completed v20 lab
...
# v20 IS THIS FILE'S DIRECT PREDECESSOR AND IT WAS RUN, EXACTLY ONCE.
#
...
# evidence in its own right and it is this run's continuation source.
#
# NOTHING ABOUT v20 IS RETRACTED BY v21 EXISTING. v20 was correct for the
...
# identity read from disk when v21 was frozen. v21 never runs it.
...
#
# It WAS the refresh source of v15. Preserved repair evidence only in v21.
...
# --- controller: THE ACCEPTED REPAIR-27B CANDIDATE -- THE SOLE v21 REFRESH SOURCE
#
# THIS IS THE ONE FILE v21 EXISTS TO EXERCISE, and it is the ONLY controller
# refresh source anywhere in this launcher.
#
# WHAT CHANGED BETWEEN v20 AND v21, AND IT IS THIS.
#
...
# written and, in v21, never copied.
...
# --------------------------------------------------------------------------
# THE CONTINUATION SOURCE — THE COMPLETED v20 SHADOW            (NEW IN v21)
# --------------------------------------------------------------------------
#
# THIS IS THE STATE THE REPAIR-27A CRASH ACTUALLY HAPPENED IN, and that is
...
readonly CONTINUATION_SOURCE="${PRESERVED_LAB_REPAIR27A_V20}/shadow/NH_CLAUDE_CODEX_DESIGN_LOOP"
readonly CONTINUATION_SOURCE_MODE="COMPLETED_V20_POST_DOORS_1_TO_3_SHADOW"
...
# The continuation source's frozen identity. Read from the preserved v20 shadow
...
# THE CONTINUATION SOURCE'S OWN CONTROLLER -- THE EXACT ACCEPTED REPAIR-27A FILE.
#
# This is what makes the refresh a bounded, provable transition rather than an
# overwrite. The continuation controller must be EXACTLY EXP_R27A_SHA, and the
# real controller must be EXACTLY EXP_LOOP_SHA; either side being anything else
# fails the run CLOSED.
readonly EXP_CONT_LOOP_SHA="53902f6fc020141dc567870ba75188f1622bc7241a022a5d36f465a6559ba43a"
readonly EXP_CONT_LOOP_BYTES="2025002"
readonly EXP_CONT_LOOP_LINES="43767"

# The v20 shadow marker the continuation source carries, frozen by content. It is
# positive proof that this source is a v20 SHADOW and not any real workspace.
readonly EXP_CONT_V20_MARKER_SHA="14d90d233e5b918c6d43259dae21eb3fd03031798059564aab55bb39f87b4a67"
...
# historical-witness gate uses, against the completed v20 shadow.
#
...
#   manifest:
#     1. the continuation controller must be EXACTLY the accepted Repair-27A
#        candidate, whose identity is frozen independently in this section and
#        gated independently against the real controller directory;
...
#        carries;
#     4. the v20 shadow marker must carry the exact v20 token content.
...
readonly EXP_CONT_MANIFEST_SHA="4a7cc938e5583c758380eaa07e895ed366cddfc9da19baa55a9c331c2c3e0c67"
...
# skipping Doors 1 to 3 is unjustified and the whole premise of v21 is false. So
...
# is PRESERVED_LAB_REPAIR27A_V20 above, it is this run's READ-ONLY continuation
# source, and this launcher will neither reuse nor delete it.
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR27B_CONTINUATION_V21_137bec6c6463556b17aefa077e6d08cbf60c4e34c359b62d428596c4f82cd9b9_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
report_state() {
  ...
  printf 'authenticated and for nothing else: in v21 it is never copied.\n' >&2
  printf 'The v20 lab is this run\047s CONTINUATION SOURCE. It was read to be\n' >&2
  printf 'copied, to be authenticated, and to have the completed v20 result\n' >&2
  printf 'proved from its own artefacts. Nothing else was done to it: no create,\n' >&2
  printf 'write, append, truncate, rename, link, unlink, chmod, chown or utimes\n' >&2
  printf 'was issued against it, and no Git command was run inside it.\n' >&2
  if [ "$AFTER_PROOF_DONE" = "1" ]; then
...
for _plab in "$PRESERVED_LAB_V1" "$PRESERVED_LAB_REPAIR9" \
             ...
             "$PRESERVED_LAB_REPAIR27A_V20"; do
  if [ "$LAB" = "$_plab" ]; then
...
if [ "$PRESERVED_LAB_REPAIR25_V18" = "$REGRESSION_SOURCE" ] \
  ...
  die "the completed v18 lab is being used as this run's regression seed:
        ${REGRESSION_SOURCE}
      v21 continues from the completed v20 shadow, and its lineage seed is the
...
# The completed v20 lab is THIS FILE'S DIRECT PREDECESSOR EVIDENCE and, uniquely,
# THIS RUN'S CONTINUATION SOURCE. v20 RAN: its Stages 1, 2 and 3 passed and its
# Stage 4 crashed inside the preparation report, which is exactly what accepted
# Repair 27B corrects.
#
# BOTH FACTS ARE TRUE AT ONCE AND THE SECOND GRANTS NO WRITE PERMISSION. It is
# never written to, never normalised, never repaired, never reused as a lab and
# never deleted. It IS read: to authenticate the recorded v20 result, to
# authenticate the state being continued, and as the SOURCE side of one rsync.
# The before/during/after preserved-artefact sweep is what proves it did not
# move.
if [ ! -d "$PRESERVED_LAB_REPAIR27A_V20" ] || [ -L "$PRESERVED_LAB_REPAIR27A_V20" ]; then
  die "the completed v20 rehearsal lab is missing or is not an ordinary directory:
      ...
      It holds the exact Stage-4 crash Repair 27B corrects AND the Doors-1-to-3
      clearance this continuation carries forward. Refusing to continue without
...
note "the completed v20 lab is present, is not this run's lab, and IS its continuation source"
...
# --------------------------------------------------------------------------
#
# THE WHOLE PREMISE OF v21 RESTS ON THIS GATE. v21 does not rerun Doors 1, 2 and
...
# --------------------------------------------------------------------------
#
# v21 continues the exact state the Repair-27A crash happened in, and that state
...
#
#   1. SHAPE. The preserved v20 lab and the continuation source inside it exist,
#      are real directories, and are not symlinks. A symlink here could redirect
#      the rsync SOURCE and silently seed the run from somewhere else entirely.
#
#   2. IDENTITY. The journal, journal head, authenticity key and v20 shadow
...
#      accepted Repair-27B candidate's own event digest, its HMAC over its own
...
#   Repair-27B candidate is imported READ-ONLY as a module and its own functions
...
assert_continuation_source() {
  # 1. SHAPE.
  if [ -L "$PRESERVED_LAB_REPAIR27A_V20" ]; then
    die "the preserved v20 lab is a SYMLINK:
        ${PRESERVED_LAB_REPAIR27A_V20}
      A symlink here could redirect the continuation copy. Refusing to continue."
  fi
  if [ ! -d "$PRESERVED_LAB_REPAIR27A_V20" ]; then
    die "the preserved v20 lab is missing or is not a directory:
        ${PRESERVED_LAB_REPAIR27A_V20}
      It is BOTH the completed v20 evidence and this run's only authorised
  ...
  unset _cf
  note "the preserved v20 lab and its continuation source are real, non-symlink directories"
  ...
  expect "continuation v20 shadow marker sha256" \
         "$(sha_of "${CONTINUATION_SOURCE}/${SHADOW_MARKER_NAME}")" "$EXP_CONT_V20_MARKER_SHA"
  ...
  # 2b. THE CONTINUATION SOURCE'S OWN CONTROLLER -- THE EXACT REPAIR-27A FILE.
  #
  # The refresh in section 7b is authorised as a transition between two NAMED
  # identities. If the continuation controller is anything other than the exact
  # accepted Repair-27A file this launcher was frozen against, then whatever the
  ...
  expect "continuation controller sha256 (accepted Repair 27A)" \
         "$(sha_of "$CONT_LOOP")"   "$EXP_CONT_LOOP_SHA"
  expect "continuation controller bytes  (accepted Repair 27A)" \
         "$(bytes_of "$CONT_LOOP")" "$EXP_CONT_LOOP_BYTES"
  expect "continuation controller lines  (accepted Repair 27A)" \
         "$(wc -l < "$CONT_LOOP")"  "$EXP_CONT_LOOP_LINES"
  expect "continuation controller is the accepted Repair-27A identity" \
         "$EXP_CONT_LOOP_SHA" "$EXP_R27A_SHA"
  ...
  if [ -d "$PROOF" ]; then
    {
      ...
      printf '  head sha256             %s\n' "$EXP_CONT_HEAD_SHA"
      printf '  authenticity key sha256 %s\n' "$EXP_CONT_KEY_SHA"
      printf '  v20 shadow marker sha256 %s\n' "$EXP_CONT_V20_MARKER_SHA"
      printf '  routed signal id        %s\n' "$EXP_ROUTED_SIGNAL_ID"
      printf '  standing target id      %s\n' "$EXP_STANDING_TARGET_ID"
      printf '  package scope id        %s\n' "$EXP_PKG_SCOPE"
      ...
      printf '  controller sha256       %s  (accepted Repair 27A)\n' "$EXP_CONT_LOOP_SHA"
      printf '  controller bytes        %s\n' "$EXP_CONT_LOOP_BYTES"
      printf '  controller lines        %s\n' "$EXP_CONT_LOOP_LINES"
      printf '\n'
      printf 'THIS IS THE COMPLETED v20 SHADOW. IT IS NOT ASSERTED TO EQUAL THE\n'
      ...
      printf 'stages 1, 2 and 3 produced. That is why v21 does not rerun them,\n'
...
${cont_out}
      ...
      NOTHING IS REPAIRED HERE. The v20 lab is not normalised, no expectation is
...
#
# The continuation source was run with the accepted Repair-27A controller, which
# it proves by digest. This launcher must therefore NOT simply run the
# byte-for-byte v20 shadow unchanged -- that would reproduce the Repair-27A crash
# again and would test nothing Repair 27B changed -- and must NOT copy the real
...
#
#   BEFORE     the new copy's controller is the EXACT accepted Repair-27A file.
#   AFTER      the refreshed file is the EXACT accepted Repair-27B identity --
...
assert_new_copy_before_refresh() {
  ...
  expect "new copy controller sha256 BEFORE refresh (accepted Repair 27A)" \
         "$(sha_of "$target")"   "$EXP_R27A_SHA"
  expect "new copy controller bytes  BEFORE refresh (accepted Repair 27A)" \
         "$(bytes_of "$target")" "$EXP_R27A_BYTES"
  expect "new copy controller lines  BEFORE refresh (accepted Repair 27A)" \
         "$(wc -l < "$target")"  "$EXP_R27A_LINES"
  note "the new copy's controller is the exact accepted Repair-27A file the continuation source carried"
}

refresh_new_copy_controller() {
  local target="${SHADOW_WS}/controller/nh_loop.py"
  local staging="${SHADOW_WS}/controller/.nh_loop.py.repair27b-refresh"

  # THE REFRESH SOURCE IS THE ACCEPTED REPAIR-27B CANDIDATE, NOT THE REAL
  ...
  [ -L "$R27B_CANDIDATE" ] && die "the accepted Repair-27B candidate is a SYMLINK:
        ${R27B_CANDIDATE}
      Refusing to refresh the shadow through it."
  [ -f "$R27B_CANDIDATE" ] \
    || die "the accepted Repair-27B candidate is missing at ${R27B_CANDIDATE}"
  expect "refresh source sha256 (accepted Repair 27B candidate)" \
         "$(sha_of "$R27B_CANDIDATE")"   "$EXP_R27B_SHA"
  expect "refresh source bytes  (accepted Repair 27B candidate)" \
         "$(bytes_of "$R27B_CANDIDATE")" "$EXP_R27B_BYTES"
  expect "refresh source lines  (accepted Repair 27B candidate)" \
         "$(wc -l < "$R27B_CANDIDATE")"  "$EXP_R27B_LINES"
  ...
  cp -- "$R27B_CANDIDATE" "$staging" \
    || die "could not stage the accepted Repair-27B candidate into the new shadow copy:
        ${staging}
      Nothing has been renamed over the new copy's controller."
  chmod 0644 -- "$staging" \
    || die "could not set the mode on the staged Repair-27B candidate: ${staging}"
  mv -f -- "$staging" "$target" \
    || die "could not move the staged Repair-27B candidate into place:
        ${target}"
  CONTROLLER_REFRESH="PERFORMED"
  note "the new copy's controller/nh_loop.py refreshed to the accepted Repair-27B candidate"
}
...
assert_new_copy_after_refresh() {
  ...
  expect "new copy controller sha256 AFTER refresh (accepted Repair 27B candidate)" \
         "$(sha_of "$target")"   "$EXP_R27B_SHA"
  expect "new copy controller bytes  AFTER refresh (accepted Repair 27B candidate)" \
         "$(bytes_of "$target")" "$EXP_R27B_BYTES"
  expect "new copy controller lines  AFTER refresh (accepted Repair 27B candidate)" \
  ...
  expect "Repair27A unchanged AFTER refresh (preserved, not the source)" \
         "$(sha_of "$R27A_CANDIDATE")" "$EXP_R27A_SHA"
  note "the refreshed shadow controller is the exact accepted Repair-27B identity"
}
...
assert_new_copy_differs_only_by_controller() {
  ...
  cmp_out="$(
    PYTHONDONTWRITEBYTECODE=1 "$PY3" -B -c "$TREE_COMPARATOR" \
      ...
      "NEWCOPY" \
      "SOURCE" \
      "DIFFERS_EXACT:${REFRESH_ALLOWED_EXACT_LOOP}:${EXP_R27B_SHA}:${EXP_R27A_SHA}" 2>&1
  )"
  cmp_rc=$?
  set -e
...
${cmp_out}
      ...
      the exact Repair-27B/Repair-27A pair.
...
${cmp_out}
      ...
      rehearse the Repair-27A controller -- the one that crashed -- while
      reporting a Repair-27B run."
      ;;
    *)
...
assert_continuation_source_unmoved_after_refresh() {
  ...
  expect "continuation controller sha256 AFTER the refresh (still Repair 27A)" \
         "$(sha_of "$CONT_LOOP")"   "$EXP_CONT_LOOP_SHA"
  expect "continuation controller bytes  AFTER the refresh (still Repair 27A)" \
         "$(bytes_of "$CONT_LOOP")" "$EXP_CONT_LOOP_BYTES"
  ...
  expect "continuation v20 shadow marker sha256 AFTER the refresh" \
         "$(sha_of "${CONTINUATION_SOURCE}/${SHADOW_MARKER_NAME}")" \
         "$EXP_CONT_V20_MARKER_SHA"
  note "the completed v20 continuation source is unmoved across the controller refresh"
}
...
assert_frozen_identities() {
  ...
  # THE ACCEPTED REPAIR-27A CANDIDATE -- PRESERVED REPAIR EVIDENCE in v21, and
  ...
  # THE ACCEPTED REPAIR-27B CANDIDATE -- THE SOLE v21 REFRESH SOURCE, gated exactly
  # as strictly as the real controller and equally never written to. It is the one
  # file this continuation exists to exercise, and it is asserted here as a
  ...
  # v20 is THIS file's DIRECT PREDECESSOR AND IT RAN, exactly once, against the
  # accepted Repair-27A controller. Its Stages 1-3 passed and its Stage 4 crashed
  # inside the preparation report; its completed lab holds that exact crash and is
  # this run's continuation source. It is now gated by exact identity like
  # v1..v19, and that expectation is the identity read from disk when v21 was
  # frozen. v21 never runs it.
  ...
  # Launcher v21 is THIS file. It cannot hash-gate on itself, so it is accounted
  ...
  [ -s "${CTRL}/${LAUNCHER_V21_BASENAME}" ] \
    || die "launcher v21 is missing or empty at ${CTRL}/${LAUNCHER_V21_BASENAME}"
  note "all twenty-one launcher artefacts present in ${CTRL}"
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${R27B_CANDIDATE_BASENAME}" | LC_ALL=C sort
  )"
  ...
  note "the completed v20 result authenticates, so a Stage-4-only continuation is justified"
  note "the continuation source is the authenticated completed v20 shadow"
...
step "CONTINUATION SHADOW COPY (SOURCE: THE COMPLETED v20 SHADOW)"

# THE SOURCE OF THIS COPY IS THE COMPLETED v20 SHADOW WORKSPACE, NOT THE REAL
# WORKSPACE, NOT THE v10 LINEAGE SEED, AND NOT ANY QUESTION-DELIVERY COPY. It is
# the exact state the Repair-27A crash happened in, and it carries the
...
if [ "$COPY_RC" -ne 0 ] || [ -n "$COPY_DIFF" ]; then
  ...
  die "the continuation shadow could not be proved identical to the preserved v20
...
note "the new shadow is a byte-truthful copy of the preserved v20 continuation source"

# ==========================================================================
# SECTION 7b — THE REPAIR-27A -> ACCEPTED-REPAIR-27B NEW-COPY CONTROLLER REFRESH
...
step "NEW-COPY CONTROLLER REFRESH: BEFORE-STATE (must be the exact accepted Repair-27A file)"
assert_new_copy_before_refresh

step "NEW-COPY CONTROLLER REFRESH: REPLACING ONLY controller/nh_loop.py"
refresh_new_copy_controller

step "NEW-COPY CONTROLLER REFRESH: AFTER-STATE (must be the exact accepted Repair-27B file)"
assert_new_copy_after_refresh
...
step "COMPLETED v20 CONTINUATION SOURCE UNMOVED ACROSS THE REFRESH"
assert_continuation_source_unmoved_after_refresh
...
if ! compare_proof_set "${PROOF}/original.before" "${PROOF}/original.during" "during-cont" \
  ...
  die "THE CONTINUATION SOURCE OR THE RECORDED v20 RESULT CHANGED WHILE THE
      SHADOW WAS BEING COPIED OR REFRESHED. The completed v20 shadow this whole
      continuation depends on must never move. Refusing to continue."
fi
note "the continuation source and the recorded v20 result unchanged across the whole copy and refresh window"
...
#
# The token identifies FOUR things: Repair 27B, the v21 Stage-4-only
# continuation, the exact accepted Repair-27B controller sha256, and the routed
# signal this rehearsal carries forward. It therefore cannot collide with any
# earlier rehearsal marker, including the v20 marker the continuation source
# carries.
SHADOW_TOKEN="nh-shadow-repair27b-continuation-v21-${EXP_R27B_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...
# ==========================================================================
# SECTION 11 — THE ONE-STAGE REAL CONTINUATION (ACCEPTED REPAIR 27B)
# ==========================================================================
#
...

{
  ...
  printf 'STAGE 4 execute-next-claude-task (the only stage v21 runs)\n'
  printf '  exit                          %s\n' "$EXEC_RC"
  printf '  ok                            %s\n' "$OBS_S4_OK"
...
# ==========================================================================
# SECTION 11b — v21 STAGE OBSERVATIONS
...
#     exactly the state Repair 27B exists to report correctly, and is not an
#     error;
...
# truncated.
step "v21 STAGE OBSERVATIONS"

observe_v21_stage() {
  local out="${PROOF}/v21-stage-observations.txt"
  local s4="$S4_OUT"
  ...
  {
    printf 'v21 STAGE OBSERVATIONS -- descriptive only, no verdict\n'
    printf '=====================================================================\n\n'
    ...
    printf '\n'
    printf '  THIS IS THE COMPLETED v20 SHADOW, THE EXACT STATE THE REPAIR-27A\n'
    printf '  CRASH HAPPENED IN. Its ninth event is the question-coverage-review\n'
    ...
    printf '  whole against the preserved v11 witness, and in v21 it was never\n'
    ...
    printf '\n'
    printf 'THE COMPLETED v20 RESULT THIS CONTINUATION RESTS ON\n'
    printf '  V20_RESULT_AUTHENTICATED : %s\n' "$V20_RESULT_AUTHENTICATED"
    ...
    printf '  source controller : %s  (accepted Repair 27A)\n' "$EXP_R27A_SHA"
    printf '  refreshed to      : %s  (accepted Repair 27B candidate)\n' "$EXP_R27B_SHA"
    printf '  real controller   : %s  (unchanged, NOT installed, never a\n' "$EXP_LOOP_SHA"
    printf '                       refresh source and never a destination)\n'
    printf '  lineage seed      : %s  (Repair 17, preserved, NOT copied in v21)\n' "$EXP_SEED_LOOP_SHA"
    ...
    printf '  completed v20 lab was not written to at any point.\n\n'
    ...
    printf '  mechanical_design_requirement_count %s\n' "$OBS_S4_MDS_REQ_COUNT"
    printf '\n'
    printf '  HOW TO READ THOSE FIVE. Under Repair 27B the requirement count is\n'
  ...
  note "v21 stage observations written to ${out}"
}
observe_v21_stage || true
...
# invariant. The controller under test in v21 is ACCEPTED REPAIR 27B, which is
...
# already independently audited and offline-tested; v21 is a CONTINUATION
...
observe_repair25_chatgpt_stop_handoff() {
...

  {
    ...
    printf '  shadow controller : %s  (accepted Repair 27B)\n' "$EXP_R27B_SHA"
    printf '  Repair 27B is based on accepted Repair 27A, which is based on\n'
    ...
    printf '  Repair 27B did not invent it.\n'
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v21, ACCEPTED REPAIR-27B, STAGE-4-ONLY CONTINUATION) — RESULT\n'
printf '===========================================================\n'
printf 'SHADOW_LAB=%s\n'                        "$LAB"
printf 'RUN_KIND=STAGE_4_ONLY_CONTINUATION_OF_THE_COMPLETED_V20_SHADOW\n'
printf 'CONTINUATION_SOURCE=%s\n'               "$CONTINUATION_SOURCE"
printf 'CONTINUATION_SOURCE_MODE=%s\n'          "$CONTINUATION_SOURCE_MODE"
...
printf 'HISTORICAL_LINEAGE_SEED_COPIED_IN_V21=NO\n'
printf '\n'
...
printf 'REPAIR27B_ACCEPTED_BY_NESS=YES\n'
printf 'REPAIR27B_ACCEPTANCE_AUTHORISES_ONLY_THIS_SHADOW_REFRESH=YES\n'
printf 'REPAIR27B_INSTALLED=NO\n'
printf 'NH_ADOPTION_OR_ACCEPTANCE_PERFORMED=NO\n'
printf 'SHADOW_CONTROLLER_UNDER_TEST=REPAIR27B\n'
printf 'SHADOW_CONTROLLER_SHA256=%s\n'          "$EXP_R27B_SHA"
printf 'SHADOW_CONTROLLER_SOURCE=%s\n'          "$R27B_CANDIDATE"
printf 'SHADOW_REFRESH_SOURCE_IS_REPAIR27B_ONLY=YES\n'
...
printf 'REPAIR27B_KNOWN_MANUAL_ROUTE_AUDIT_PROVENANCE_DEPENDENCY=OPEN\n'
...
printf 'V20_LAB_IS_THIS_RUN_CONTINUATION_SOURCE=YES\n'
printf '\n'
printf 'DOORS_1_TO_3_RERUN_BY_V21=NO\n'
printf 'DOOR1_QUESTION_VALIDATION_STAGE_EXISTS_IN_V21=NO\n'
printf 'DOOR2_QUESTION_COVERAGE_REVIEW_STAGE_EXISTS_IN_V21=NO\n'
printf 'DOOR3_INTERVIEW_GATE_STAGE_EXISTS_IN_V21=NO\n'
...
printf 'REPAIR25_HANDOFF_IS_AN_INHERITED_SEAM_UNDER_REPAIR27B=YES\n'
printf 'REPAIR25_HANDOFF_NOT_EXERCISED_IS_NOT_A_FAILURE=YES\n'
printf 'REPAIR25_HANDOFF_OBSERVATION_GATES_NO_STAGE=YES\n'
printf 'V21_INJECTED_A_FINDING_TO_FORCE_THE_BRANCH=NO\n'
printf 'V21_INVOKED_ROUTE_AUDIT_FINDING=NO\n'
printf 'V21_INVOKED_QUESTION_VALIDATION=NO\n'
printf 'V21_INVOKED_QUESTION_COVERAGE_REVIEW=NO\n'
printf 'V21_INVOKED_INTERVIEW_GATE=NO\n'
printf 'V21_INVOKED_NEXT_QUESTION_GROUP=NO\n'
printf 'V21_INVOKED_RECORD_NESS_ANSWER=NO\n'
printf 'V21_INVOKED_DECLARE_STRANDED_CANDIDATE=NO\n'
printf 'V21_MANUFACTURED_A_NESS_QUESTION=NO\n'
printf 'V21_MANUFACTURED_A_FINDING=NO\n'
printf '\n'
...
printf 'PROOF_V21_OBSERVATIONS=%s\n'                 "${PROOF}/v21-stage-observations.txt"
...
if [ "$CONTINUATION_SOURCE_PROOF" = "FAIL" ]; then
  printf '\nTHE CONTINUATION SOURCE CHANGED DURING THIS RUN. The completed v20\n'
...
printf '  The CONTINUATION SOURCE is the completed v20 shadow -- the exact state\n'
printf '  the Repair-27A crash happened in, and the only thing copied. The\n'
...
printf '  preserved v11 witness, and NEVER COPIED in v21. The PROTECTED ORIGINAL\n'
...
printf '  controller/nh_loop.py inside the disposable v21 shadow was replaced\n'
printf '  with the ACCEPTED REPAIR-27B CANDIDATE, which was opened read-only as\n'
printf '  the source and was never written to. Repair 27B was the ONLY refresh\n'
...
printf '  REPAIR 27B IS ACCEPTED BY NESS FOR ITS BOUNDED CONTROLLER-REPAIR SCOPE,\n'
...
printf '  ACCEPTED REPAIR 27B reads the requirement count only where a\n'
printf '  specification exists, leaves the field at None otherwise -- not zero,\n'
printf '  which would falsely report a specification carrying no requirements --\n'
printf '  and appends one plain line saying no new specification was required. It\n'
printf '  is a report-only block; which kind of not-ready this is stays read from\n'
printf '  the controller\047s own bounded not_ready_reason field.\n'
printf '  THIS RUN TESTED THAT CORRECTION AND NOTHING BROADER. It assumed no\n'
...
printf '  happen, and not that the loop must reach PASS. The real Repair-27B\n'
...
printf '  THE STAGE-4 OBSERVER WATCHES AN INHERITED REPAIR-25 SEAM. Repair 27B\n'
printf '  inherits it through 27A, 27, 26 and 25; it did not invent it. If Stage 4\n'
...
printf '  repaired the dependency, and does not treat its being open as a v21\n'
