#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v11.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v11
#   (REPAIR-18 CONTROLLER, CONTINUATION OF THE COMPLETED v10 SHADOW STATE)
#
# WHAT THIS IS
...
#   makes an exact throwaway copy of THE PRESERVED v10 SHADOW WORKSPACE rather
#   than of the real workspace, proves that seed still represents the current
#   real source, REFRESHES EXACTLY ONE FILE IN THAT NEW COPY -- and only in the
#   new copy -- to the independently audited Repair-18 controller, hides the
#   real /home/ness behind a tmpfs, republishes that continuation copy at the
#   EXACT normal path /home/ness/NH_CLAUDE_CODEX_DESIGN_LOOP inside a Bubblewrap
#   namespace, runs the REPAIR-18 sequence
#
...
#
# ==========================================================================
# WHAT v11 IS, AND WHAT IT CHANGES
# ==========================================================================
#
#   v11 IS A BOUNDED CONTINUATION-PLUS-CONTROLLER-REFRESH REBASE OF v10, NOT A
#   REDESIGN. It starts from the exact bytes of
#   NH_SHADOW_REHEARSAL_LAUNCHER_v10.sh (sha256 088da6ce..., 237741 bytes, 4501
#   lines) and changes ONLY what CONTINUING THE COMPLETED v10 SHADOW STATE
#   UNDER THE REPAIR-18 CONTROLLER mechanically requires. Every isolation
#   guarantee and every proof v1..v10 earned is carried forward unchanged, and
#   the v10..v2 histories below are retained deliberately so none of it can be
#   lost.
#
#   THE TWO DEFINING CHANGES.
#     1. THE SHADOW COPY SOURCE IS THE COMPLETED v10 SHADOW WORKSPACE:
#          /home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR17_CONTINUATION_V10_6a3ce02e...
#            _rs_2593b06ab1684bbf19a6ffb70d915bd2/shadow/NH_CLAUDE_CODEX_DESIGN_LOOP
#        exactly as v10's source was the completed v9 shadow.
#     2. THE NEW COPY'S controller/nh_loop.py IS REFRESHED, AND NOTHING ELSE IS.
#        This is new in v11 and has no ancestor in v1..v10.
#
#   WHY THE REFRESH EXISTS. The completed v10 shadow was run with the Repair-17
#   controller (sha256 6a3ce02e...). The CURRENT real controller is the
#   independently audited Repair-18 one (sha256 440cf8cf...). Running the v10
#   shadow byte-for-byte unchanged would therefore rehearse Repair 17 again and
#   would test nothing new; copying the real workspace instead would throw the
#   seven-event continuation state away and lose the routed review signal
#   entirely. v11 does neither. It takes
#
#       CONTINUATION STATE FROM v10  +  CONTROLLER LOGIC FROM REPAIR 18
#
#   and nothing else.
#
#   WHAT v10 ACTUALLY RECORDED, STATED ONCE AND ONLY AS PROVENANCE. Its stage 1
#   question-validation exited 1 and fail-closed on INCOMPLETE_SOURCE_CHECK --
#   the package source binding was ambiguous across the three Authority
#   Integrity candidates v1.0 / v1.1 / v1.2 -- so stages 2, 3 and 4 were never
#   reached and the routed review signal rs_2593b06ab1684bbf19a6ffb70d915bd2 is
#   still UNRESOLVED as event 7. Repair 18 repaired package identity. NOTHING in
#   this launcher depends on any of that: the seed is authenticated here, from
#   the seed itself, by the controller's own semantics.
#
#   THE EXACT ORDER THE REFRESH HAPPENS IN, AND WHY IT IS THIS ORDER.
#     1. the preserved v10 seed is authenticated (section 4b);
#     2. the seed is proved to still represent current real source (section 4c);
#     3. the seed is copied into the NEW disposable v11 lab;
#     4. that initial copy is proved BYTE-FOR-BYTE against the seed, BEFORE a
#        single byte of it is refreshed -- so the refresh is applied to a copy
#        already proved truthful, never to an unverified one;
#     5. the new copy's nh_loop.py is proved to be the exact Repair-17 file;
#     6. ONLY that one path is replaced, from the real Repair-18 file opened
#        READ-ONLY as the source;
#     7. the refreshed file is proved to be the exact Repair-18 identity;
#     8. the new copy is proved to differ from the seed in EXACTLY that one
#        path and in no other, by content, at every depth;
#     9. the new copy's journal, journal head and authenticity key are proved
#        still byte-identical to the seed's seven-event state;
#    10. the historical v10 seed is re-read and proved unmoved;
#    11. only then is the new v11 shadow marker placed;
#    12. only later, in section 11, does anything enter Bubblewrap.
#
#   WHAT IS NEVER REFRESHED, AND THE LIST IS EXHAUSTIVE. The interview journal,
#   the journal head, the authenticity key, the candidates, the NH-GOVERNANCE
#   tree, any source package, any Git metadata and every launcher file are left
#   exactly as the seed carried them. controller/nh_loop.py is the ONLY path
#   this launcher writes into the new copy before the marker, and step 8 above
#   is what proves that rather than merely asserting it.
#
#   THE HISTORICAL v10 SEED IS NEVER WRITTEN TO. It appears in this file only in
#   readonly assignments, in printf, in comparisons, and as the SOURCE side of
#   an rsync. It is never a write target, never deleted, never renamed, never
#   chmod-ed, never normalised, and no Git command is ever run inside it. It is
#   also added to the preserved-lab set, so it is witnessed before the copy,
#   during the copy, after every stage, and in the final outside-Bubblewrap
#   after-proof, and a change to it outranks any apparent controller success.
#
#   WHAT v11 CHANGES FROM v10, AND ONLY THIS:
#     - the frozen controller identity, to the independently audited Repair-18
#       controller (sha256 440cf8cf..., 1760546 bytes, 38541 lines);
#     - THE SHADOW COPY SOURCE, and the copy VERIFICATION source with it: both
#       are now the completed v10 continuation seed;
#     - the seed's OWN controller identity is now frozen and gated as the exact
#       Repair-17 file, so an arbitrarily drifted seed controller fails closed
#       just as an arbitrarily drifted real controller does;
#     - THE NEW-COPY CONTROLLER REFRESH and its four proofs (before-identity,
#       after-identity, composite one-path-only, continuation-state-preserved),
#       plus the named proof that the seed itself did not move across it. All of
#       this is new; none of it has an ancestor in v1..v10;
#     - the current-source compatibility gate now admits SIX classified
#       differences instead of four, and the nh_loop.py one is admitted ONLY as
#       an exact (real = Repair 18, seed = Repair 17) pair rather than as a
#       tolerated "differs" -- arbitrary controller drift on either side fails
#       the run closed;
#     - v10's own identity is now frozen and gated, as v1..v9 already are;
#     - v11 itself now exists in controller/, so the exact expected controller
#       worktree gains one more entry -- still an EXACT match, never
#       "anything untracked is acceptable";
#     - a fresh disposable CONTINUATION lab path, keyed to the Repair-18
#       controller AND to the routed signal being continued;
#     - the shadow marker token, keyed to Repair 18, to the v11 continuation,
#       to the exact controller sha256 and to the routed signal id;
#     - the completed v10 lab is now named as PRESERVED evidence, so this run
...
#     - the observations pass is now REPAIR-18 CONTINUATION V11, replacing the
...
#
#   WHAT v11 IS ACTUALLY TESTING, AND WHAT IT REFUSES TO DECIDE.
#   v11 finally exercises the substantive routed issue
#       authority_control_retry_values_scope
#   under a controller whose package-identity defect is repaired. It does NOT
#   decide the retry values, does NOT hard-code a classification, does NOT treat
#   the coverage finding as true, does NOT manufacture a Ness question and does
#   NOT resolve the routed signal itself. The FRESH real GPT question-validation
#   is the only thing that may decide whether that issue is settled,
#   mechanically implied, mechanical work, waiting on another choice, genuinely
#   open for Ness, future non-blocking, or later building/disk work. This
#   launcher contains no classification vocabulary, no disposition vocabulary
#   and no interpretation of either.
#
#   QUESTION DELIVERY IS NOT PART OF v11. There is no next-question-group call
#   and no record-ness-answer call anywhere in this file, and this launcher asks
#   Ness nothing. If the fresh validation admits a genuine question it is left
#   safely in the disposable v11 interview state, to be inspected and delivered
#   afterwards through the ordinary question path.
#
#   REPAIR 18 / 17 / 16 REMAIN ENTIRELY CONTROLLER-OWNED, AND v11 INTERPRETS
#   NONE OF THEM. Repair 18 -- a coverage routed scope may reconnect only to an
#   already-authenticated package identity. Repair 17 -- genuinely_open_for_ness
#   maps to admit_ness_question from the controller's own single disposition
#   source of truth. Repair 16 -- official Codex errors stay fail-closed and
#   their bounded diagnostics flow to ordinary stage stdout/stderr. All three
#   live inside nh_loop.py. This launcher contains no executable handling of any
#   classification, disposition, package-identity rule or Codex error wording;
#   it runs the same controller stages it ran before and preserves each stage's
#   stdout and stderr verbatim and in full. There is NO parser of controller
#   meaning, NO classifier, NO retry and NO provider fallback. Raw Codex JSONL is
#   never exposed by this launcher, exactly as before.
#
...
#   none of them and passes no environment variable that could influence any of
#   them: --clearenv wipes the environment and the only NH_-prefixed variable
#   set back for a real stage is NH_LOOP_INTERVIEW_STATE_DIR.
#
#   THE REAL WORKSPACE STILL GETS v10's FULL PROTECTION, UNWEAKENED. It is
#   frozen by exact branch, HEAD and complete status; proved before the copy;
#   re-asserted under both advisory locks; proved during the copy; proved after
#   every stage that runs; and proved finally from OUTSIDE the Bubblewrap
#   namespace. The real controller file is opened READ-ONLY as the refresh
#   source and is never written to. No continuation event is ever written back
#   to the real interview journal, head or key.
#
...
#   v10: all three are still present, still real directories, still not
#   symlinks, still recursively empty, and the workspace root still does not
#   resolve as a repository. Only controller/ and NH-GOVERNANCE/ are real Git
#   repositories, and nothing under the three explained directories is created,
#   deleted, populated or normalised by this launcher on any path.
#
#   THE HISTORY BELOW IS v10's, v9's, v8's, v7's, v6's, v5's, v4's, v3's AND
#   v2's, retained in full.
#
#   v10 WAS A BOUNDED CONTINUATION REBASE OF v9, NOT A REDESIGN.
...
#
# USAGE
#   bash /home/ness/NH_CLAUDE_CODEX_DESIGN_LOOP/controller/NH_SHADOW_REHEARSAL_LAUNCHER_v11.sh
#
...
# Eleven launcher files now live in controller/. All eleven are accounted for
# explicitly: ten by exact identity, and this one by presence.
...
#
# v9 RAN, exactly once, against the Repair-17 controller. Its lab is the
# Repair-17 evidence and was v10's continuation seed. It is gated by identity
# exactly like v1..v8.
#
# v10 is this file's DIRECT PREDECESSOR and is preserved evidence in its own
# right. It RAN, exactly once, against the Repair-17 controller over the
# preserved seven-event continuation state, and its lab is BOTH the completed
# v10 evidence AND the read-only continuation seed this launcher copies from.
# Being the seed authorises nothing to be written into it: it is gated by
# identity exactly like v1..v9, its lab is witnessed before, during and after
# this run like every other preserved lab, and no code path anywhere in this
# file opens any path under it for writing.
...
# --- controller: THE REPAIR-18 IDENTITY ---
#
# This is the CURRENT REAL controller, the one that passed the independent Codex
# audit, and it is the file this launcher refreshes the NEW SHADOW COPY to. It is
# never modified by this launcher and is opened read-only on every path.
readonly EXP_LOOP_SHA="440cf8cf4286b4682c5add47e49d8e19de4020d4c8a0ffbe320573dc654969c2"
readonly EXP_LOOP_BYTES="1760546"
readonly EXP_LOOP_LINES="38541"
...
# --------------------------------------------------------------------------
# THE v10 CONTINUATION SEED  (the first defining change of v11)
# --------------------------------------------------------------------------
#
# The completed v10 lab. It is preserved evidence in its own right AND it is the
...
# the whole point of v11: it carries the seven-event interview journal whose
# event 7 is the routed review signal the v9 coverage review produced and which
# the completed v10 run left STILL UNRESOLVED, and that event exists NOWHERE
# else -- not in the real workspace, not in any earlier lab.
readonly V10_CONTINUATION_SEED="${PRESERVED_LAB_REPAIR17_CONTINUATION_V10}/shadow/NH_CLAUDE_CODEX_DESIGN_LOOP"

# The seed's OWN controller file. It is the REPAIR-17 controller, because that
# is what the completed v10 run was executed with, and it is the file the
# refresh in section 7b replaces IN THE NEW COPY ONLY. This path is read; it is
# never written.
readonly SEED_LOOP="${V10_CONTINUATION_SEED}/controller/nh_loop.py"

readonly SEED_STATE="${V10_CONTINUATION_SEED}/nh_interview_state"
...
# THE SEED'S OWN CONTROLLER IDENTITY -- THE EXACT REPAIR-17 FILE.  (NEW IN v11)
#
...
# controller must be EXACTLY Repair 18; either side being anything else fails
# the run CLOSED. No other controller drift is admitted on either side, and the
# section 4c comparator enforces the pair as a PAIR rather than tolerating a
# bare "these two files differ".
...
# The v10 shadow marker the seed already carries. It is one of the exactly six
...
# purpose: to prove that the state being continued is the state v9 recorded and
...
# --------------------------------------------------------------------------
# THE EXACTLY SIX PERMITTED REAL-vs-SEED DIFFERENCES  (section 4c)
# --------------------------------------------------------------------------
#
# A continuation may not run stale source. The seed is a HISTORICAL copy of the
# real workspace plus whatever the completed v9 and v10 runs legitimately
# created, so it is allowed to differ from the current real workspace in exactly
# these six places and nowhere else. Every one of them is classified, not
# tolerated, and each carries its category letter from the authorising
# instruction:
#
#   1. .nh_shadow_marker
#        SEED ONLY.  [B: historical shadow provenance]  the marker v10 wrote
#        after its copy verification so the namespace could prove the
#        substitution happened.
#   2. nh_interview_state/nh_interview_journal.jsonl
#        DIFFERS.  [A: authenticated continuation state]  the seed holds seven
#        authenticated events, the real workspace holds its own untouched five.
#        Events 6 and 7 exist only in the seed.
#   3. nh_interview_state/nh_interview_journal.head
#        DIFFERS.  [A: authenticated continuation state]  the seed head is
#        committed at event 7, the real head at event 5.
#   4. controller/nh_loop.py
#        DIFFERS AS AN EXACT PAIR.  [C: explicitly authorised Repair-18
#        controller refresh]  real MUST be exactly the Repair-18 identity and
#        seed MUST be exactly the Repair-17 identity. This is deliberately NOT
#        a bare "these differ" allowance: an arbitrarily drifted controller on
#        EITHER side is unexplained and fails the run closed. This one
#        difference is the entire reason v11 exists.
#   5. controller/NH_SHADOW_REHEARSAL_LAUNCHER_v10.sh
#        REAL ONLY.  [D: launcher-generation difference]  created after the
#        historical v9 seed the v10 shadow descends from, so the seed cannot
#        contain it.
#   6. controller/NH_SHADOW_REHEARSAL_LAUNCHER_v11.sh
#        REAL ONLY.  [D: launcher-generation difference]  this launcher. The
#        historical seed was copied before it existed.
#
# ANY OTHER DIFFERENCE OF ANY KIND FAILS THE RUN CLOSED, before any provider
# call: an extra path, a missing path, a changed byte in any launcher v1..v10,
# in the NH-GOVERNANCE tree, in a candidate, in an authority file, in the
# Decision Defaults, in cursorrules, in the Companion, in the Working Map, in
# the interview authenticity key, or in any ordinary workspace file. There is no
# category E: an unexplained difference is not a category, it is a refusal.
readonly SEED_DIFF_ALLOWED_SEED_ONLY=".nh_shadow_marker"
readonly SEED_DIFF_ALLOWED_REAL_ONLY_V10="controller/NH_SHADOW_REHEARSAL_LAUNCHER_v10.sh"
readonly SEED_DIFF_ALLOWED_REAL_ONLY_V11="controller/NH_SHADOW_REHEARSAL_LAUNCHER_v11.sh"
...
readonly SEED_DIFF_ALLOWED_DIFFERS_EXACT_LOOP="controller/nh_loop.py"
...
# --- lab: NEW, keyed to the Repair-18 controller identity AND to the exact
...
#
# It is deliberately NOT the v10 lab path. The v10 lab is preserved evidence and
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR18_CONTINUATION_V11_440cf8cf4286b4682c5add47e49d8e19de4020d4c8a0ffbe320573dc654969c2_rs_2593b06ab1684bbf19a6ffb70d915bd2"
...
# rehearsal -- is preserved evidence in its own right. v11 must never collide
# with it and must witness that it did not move. It was v10's continuation seed
# and is NOT v11's; v11 copies from the completed v10 lab instead.
...
# The completed v10 rehearsal -- the Repair-17 CONTINUATION run -- is preserved
# evidence in its own right. v11 must never collide with it and must witness
# that it did not move.
#
# IT IS ALSO THIS RUN'S CONTINUATION SEED, and that changes nothing about its
# protection. It is declared as PRESERVED_LAB_REPAIR17_CONTINUATION_V10 above,
# next to the seed constants, so the two roles are visibly the same path; it is
# named here so the collision refusal, the preserved-lab sweep and the report
# all cover it explicitly rather than only by glob.
#
# A CHANGE TO IT OUTRANKS ANY APPARENT CONTROLLER SUCCESS: it is compared in the
# PRESERVED-LAB proof set, which has its own verdict and its own reserved exit
# code (92), and that code is returned ahead of any ordinary controller status.
#
# NO HISTORICAL METADATA OF IT IS REQUIRED HERE, and none is asserted. Like
# every other preserved lab it is captured at the start of THIS run and compared
...
# Repair-10, Repair-14, Repair-15, Repair-16, Repair-17 and Repair-17
# CONTINUATION V10 labs, the last four being the completed v7, v8, v9 and v10
# rehearsals. Kept as their own set, with their own verdict
...
LAB_CREATED=0
RUN_STARTED=0
# The controller refresh (section 7b). NOT-RUN until the refresh actually
# happens, PRESERVED/PERFORMED only once its own proof passed, so the report can
# never claim a refresh that did not occur.
CONTROLLER_REFRESH="NOT-RUN"
NEW_COPY_CONTINUATION_STATE="NOT-RUN"
REPORTED=0
AFTER_PROOF_DONE=0
...
capture_original_proof() {
  ...
  # --- all eleven launcher artefacts: v1-v10 must stay byte-identical, and
  ...
  # --- THE v10 CONTINUATION SEED, WITNESSED BY NAME ---
  #
  # The seed lives inside the preserved v10 lab, so it is already inside the
  ...
  # taken, and a sha256 is a one-way digest from which no key can be recovered.
  {
    ...
    printf 'seed_marker_sha256               %s\n' "$(sha_of "${V10_CONTINUATION_SEED}/${SHADOW_MARKER_NAME}")"
...
for _plab in "$PRESERVED_LAB_V1" "$PRESERVED_LAB_REPAIR9" \
             ...
             "$PRESERVED_LAB_REPAIR17_CONTINUATION_V10"; do
  if [ "$LAB" = "$_plab" ]; then
...
# --------------------------------------------------------------------------
# SECTION 4b — THE CONTINUATION SEED, AND THE SIGNAL IT CARRIES
# --------------------------------------------------------------------------
#
...
#
#   1. SHAPE. The preserved v10 lab and the seed workspace inside it exist, are
...
#      The seed's own controller file is shape-checked here too, because it is
#      the one path the refresh in section 7b replaces in the new copy.
#
...
#      controller is the EXACT Repair-17 file. That last one is new in v11 and
#      is what makes the refresh a bounded transition between two named
#      identities rather than an overwrite of whatever happened to be there.
#
...
assert_continuation_seed() {
  ...
  if [ -L "$V10_CONTINUATION_SEED" ]; then
    die "the v10 continuation seed workspace is a SYMLINK:
        ${V10_CONTINUATION_SEED}
      Refusing to rehearse."
  fi
  if [ ! -d "$V10_CONTINUATION_SEED" ]; then
    die "the v10 continuation seed workspace is missing or is not a directory:
        ${V10_CONTINUATION_SEED}
      Refusing to rehearse."
  fi
  if [ -L "$SEED_STATE" ] || [ ! -d "$SEED_STATE" ]; then
    die "the v10 continuation seed interview state is missing or is not a real
      directory:
        ${SEED_STATE}
      Refusing to rehearse."
  fi
  for _sf in "$SEED_JOURNAL" "$SEED_JOURNAL_HEAD" "$SEED_AUTH_KEY" "$SEED_LOOP"; do
    if [ -L "$_sf" ] || [ ! -f "$_sf" ]; then
      die "a v10 continuation seed file is missing or is not an ordinary,
      non-symlink file:
        ${_sf}
      Refusing to rehearse."
    fi
  ...
  unset _sf
  note "the preserved v10 lab and its continuation seed are real, non-symlink directories"
  ...
  expect "seed v10 shadow marker sha256" \
         "$(sha_of "${V10_CONTINUATION_SEED}/${SHADOW_MARKER_NAME}")" "$EXP_SEED_V10_MARKER_SHA"
  expect "seed authenticated package scope occurrences" \
         "$(grep -c -F -- "$EXP_PKG_SCOPE" "$SEED_JOURNAL" || true)" \
         "$EXP_SEED_PKG_SCOPE_OCCURRENCES"

  # 2b. THE SEED'S OWN CONTROLLER -- THE EXACT REPAIR-17 FILE.  (NEW IN v11)
  #
  # The refresh in section 7b is authorised as a transition between two NAMED
  # identities. If the seed controller is anything other than the exact
  # Repair-17 file this launcher was frozen against, then whatever the refresh
  # would be replacing is not what this launcher believes it is replacing, and
  # the run is refused rather than guessed at.
  expect "seed controller sha256 (Repair 17)" "$(sha_of "$SEED_LOOP")"   "$EXP_SEED_LOOP_SHA"
  expect "seed controller bytes  (Repair 17)" "$(bytes_of "$SEED_LOOP")" "$EXP_SEED_LOOP_BYTES"
  expect "seed controller lines  (Repair 17)" "$(wc -l < "$SEED_LOOP")"  "$EXP_SEED_LOOP_LINES"
  ...
  if [ "$seed_auth_rc" -ne 0 ] \
    ...
    die "the v10 continuation seed did NOT authenticate under the controller's own
      journal semantics (status ${seed_auth_rc}):
${seed_auth_out}
      The continuation state is not proved to be the state v10 left. Nothing is
...
# --------------------------------------------------------------------------
# SECTION 4c — CURRENT-SOURCE COMPATIBILITY GATE
# --------------------------------------------------------------------------
#
# A CONTINUATION MAY NOT RUN STALE SOURCE.
#
# The seed is a copy of the real workspace as it stood when v9 ran, plus what
# the completed v9 and v10 runs legitimately created. If the real project has
# moved since -- an edited candidate, a new authority file, a changed Working
# Map -- then continuing from the seed would silently rehearse against source
# that no longer exists, and would do it while claiming to be current.
#
# So the COMPLETE path-and-content difference set is computed here and required
# to be EXACTLY the six classified differences frozen in section 0. Not "at
...
#
# THE CONTROLLER DIFFERENCE IS ADMITTED AS A PAIR, NOT AS A TOLERANCE. v11
# expects nh_loop.py to differ, because refreshing it is the whole point -- but
# it is admitted ONLY when the real side is exactly the Repair-18 digest and the
# seed side is exactly the Repair-17 digest. A real controller that is not
# Repair 18, or a seed controller that is not Repair 17, is reported as
# DIFFERS-NOT-THE-AUTHORISED-PAIR and fails the run closed. That is what stops
# "the controller is allowed to differ" from quietly becoming "any controller is
# allowed".
#
...
assert_seed_matches_current_source() {
  ...
  cmp_out="$(
    PYTHONDONTWRITEBYTECODE=1 "$PY3" -B -c "$SEED_SOURCE_COMPARATOR" \
      ...
      "REAL" \
      "SEED" \
      "RIGHT_ONLY:${SEED_DIFF_ALLOWED_SEED_ONLY}" \
      "LEFT_ONLY:${SEED_DIFF_ALLOWED_REAL_ONLY_V10}" \
      "LEFT_ONLY:${SEED_DIFF_ALLOWED_REAL_ONLY_V11}" \
      "DIFFERS:${SEED_DIFF_ALLOWED_DIFFERS_JOURNAL}" \
      "DIFFERS:${SEED_DIFF_ALLOWED_DIFFERS_HEAD}" \
      "DIFFERS_EXACT:${SEED_DIFF_ALLOWED_DIFFERS_EXACT_LOOP}:${EXP_LOOP_SHA}:${EXP_SEED_LOOP_SHA}" 2>&1
  )"
  cmp_rc=$?
  set -e
  ...
  case "$cmp_rc" in
    0)
      ...
      ;;
    1)
      die "THE REAL PROJECT SOURCE HAS DRIFTED FROM THE v10 CONTINUATION SEED.
${cmp_out}
      A continuation may not run stale source. The seed is permitted to differ
      from the current real workspace in exactly six classified places -- the
      v10 shadow marker, the seven-event interview journal, its head, the
      controller as the EXACT Repair-18/Repair-17 pair, and launchers v10 and
      v11, which the historical seed cannot contain. Everything listed above is
      outside that set.
      A DIFFERS-NOT-THE-AUTHORISED-PAIR line above means the controller drifted:
      the real controller is not exactly Repair 18, or the seed controller is
      not exactly Repair 17. Being allowed to differ is not being allowed to be
      anything.
...
${cmp_out}
      ...
      v10 continuation seed must. The most serious form of this is a seed whose
      ...
      An absent authorised-pair difference means the real and seed controllers
      are now the same file, so there is no Repair-17 -> Repair-18 transition
      left for this launcher to perform, and running it would be a no-op
      pretending to be a refresh. Refusing to rehearse."
      ;;
    *)
...
${cmp_out}
  ...
  note "the v10 continuation seed still represents the current real source"
  note "  permitted differences, and only these six:"
  note "    seed only : ${SEED_DIFF_ALLOWED_SEED_ONLY}  [B] v10 shadow provenance"
  note "    differs   : ${SEED_DIFF_ALLOWED_DIFFERS_JOURNAL}  [A] authenticated continuation state"
  note "    differs   : ${SEED_DIFF_ALLOWED_DIFFERS_HEAD}  [A] authenticated continuation state"
  note "    pair      : ${SEED_DIFF_ALLOWED_DIFFERS_EXACT_LOOP}  [C] real=Repair 18, seed=Repair 17, exactly"
  note "    real only : ${SEED_DIFF_ALLOWED_REAL_ONLY_V10}  [D] launcher generation"
  note "    real only : ${SEED_DIFF_ALLOWED_REAL_ONLY_V11}  [D] launcher generation"
}

# --------------------------------------------------------------------------
# SECTION 4d — THE NEW-COPY CONTROLLER REFRESH  (NEW IN v11)
# --------------------------------------------------------------------------
#
# THE DEFINING v11 TRANSITION, AND THE ONLY WRITE THIS LAUNCHER MAKES INTO THE
# NEW SHADOW BEFORE THE MARKER.
#
# The completed v10 shadow was run with the Repair-17 controller. The current
# real controller is the independently audited Repair-18 one. v11 must therefore
# NOT simply run the byte-for-byte v10 shadow unchanged -- that would rehearse
# Repair 17 again -- and must NOT copy the real workspace instead, which would
# discard the seven-event continuation state and the unresolved routed signal
# with it.
#
...
#
#   BEFORE   the new copy's controller is the EXACT Repair-17 file. If it is
#            not, the copy is not what the verification said it was, and no
#            refresh is attempted.
#   AFTER    the refreshed file is the EXACT Repair-18 identity -- sha256, byte
#            count AND line count. A short read, a truncated write or a
#            substituted source all fail here.
#   COMPOSITE  the new copy differs from the seed in EXACTLY controller/nh_loop.py
#            and in NO other path, compared by content at every depth. This is
#            what proves "only the controller was refreshed" instead of merely
#            asserting it, and it is also what catches a stray temporary file,
#            a permission change that rewrote something, or an rsync that moved
...
#            still carry event 7 exactly once. Stated separately from the
#            composite proof on purpose: this is the state the whole
#            continuation exists to carry, and it deserves its own named
#            verdict rather than being one line inside a listing.
#
# WHAT IS NEVER REFRESHED. The interview journal, the journal head, the
# authenticity key, the candidates, the NH-GOVERNANCE tree, any source package,
# any Git metadata, and every launcher file. The composite proof above is what
# makes that list enforceable rather than aspirational.
#
...
refresh_new_copy_controller() {
  local target="${SHADOW_WS}/controller/nh_loop.py"
  local staging="${SHADOW_WS}/controller/.nh_loop.py.repair18-refresh"

  # The real controller is asserted ONE more time, immediately, at the moment it
  # is about to be read as a source. The gate has already passed twice; this
  # costs nothing and closes the window between the last assertion and the read.
  expect "refresh source controller sha256 (Repair 18)" \
         "$(sha_of "${CTRL}/nh_loop.py")"   "$EXP_LOOP_SHA"
  expect "refresh source controller bytes  (Repair 18)" \
         "$(bytes_of "${CTRL}/nh_loop.py")" "$EXP_LOOP_BYTES"
  expect "refresh source controller lines  (Repair 18)" \
         "$(wc -l < "${CTRL}/nh_loop.py")"  "$EXP_LOOP_LINES"

  # Staged, then renamed over the target within the same directory, so the
  # target is never observable half-written. A failed copy leaves the staging
  # name behind in the disposable lab and the composite proof below reports it
  # as an unexplained path rather than ignoring it.
  cp -- "${CTRL}/nh_loop.py" "$staging" \
    || die "could not stage the Repair-18 controller into the new shadow copy:
        ${staging}
      Nothing has been renamed over the new copy's controller. Refusing to
      rehearse."
  chmod 0644 -- "$staging" \
    || die "could not set the mode on the staged Repair-18 controller: ${staging}"
  mv -f -- "$staging" "$target" \
    || die "could not move the staged Repair-18 controller into place:
        ${target}
      Refusing to rehearse."
  CONTROLLER_REFRESH="PERFORMED"
  note "the new copy's controller/nh_loop.py refreshed to the Repair-18 file"
}
...
assert_new_copy_after_refresh() {
  ...
  expect "new copy controller sha256 AFTER refresh (Repair 18)" \
         "$(sha_of "$target")"   "$EXP_LOOP_SHA"
  expect "new copy controller bytes  AFTER refresh (Repair 18)" \
         "$(bytes_of "$target")" "$EXP_LOOP_BYTES"
  expect "new copy controller lines  AFTER refresh (Repair 18)" \
         "$(wc -l < "$target")"  "$EXP_LOOP_LINES"
  note "the refreshed controller is the exact Repair-18 identity"
}

# THE COMPOSITE PROOF: ONE PATH CHANGED, AND ONLY ONE.
#
# Content comparison at every depth, using the SAME comparator the section 4c
# gate uses, so there is one implementation of "what a difference is" and it
# cannot drift between the two callers. Directory and file MODIFICATION TIMES
# are deliberately not compared -- writing a file necessarily moves its parent
# directory's mtime, and comparing that would make this gate fire on every run
# while proving nothing about content.
#
# The allowance list is exactly ONE entry. No seed-only allowance, no
# new-copy-only allowance, no other content allowance: at this point the new
# copy still carries the seed's own v10 marker, because the v11 marker is not
# placed until after this proof passes. That ordering is required, not
# incidental -- placing the marker first would put a second legitimate
# difference into this comparison and weaken it.
...
${cmp_out}
      ...
      controller that is not the exact Repair-18/Repair-17 pair.
      Nothing is reverted or reconciled here. Refusing to rehearse."
      ;;
    3)
      die "THE CONTROLLER REFRESH DID NOT TAKE EFFECT.
${cmp_out}
      The new copy no longer differs from the seed in the one place it must.
      A refresh that left the copy identical to the seed would rehearse the
      Repair-17 controller while reporting a Repair-18 run. Refusing to
      rehearse."
      ;;
    *)
...
${cmp_out}
  ...
  note "the new copy differs from the v10 seed in EXACTLY controller/nh_loop.py"
}

# THE CONTINUATION STATE ITSELF, PROVED PRESERVED BY NAME.
#
...
# continuation exists: a reader must be able to see "the seven-event journal
...
assert_new_copy_continuation_state_preserved() {
  ...
  expect "new copy standing target occurrences (unchanged by the refresh)" \
         ...
         "$EXP_SEED_STANDING_TARGET_OCCURRENCES"
  NEW_COPY_CONTINUATION_STATE="PRESERVED"
  note "the new copy still carries the v10 seven-event continuation state, unaltered"
}

# THE HISTORICAL SEED, RE-READ AND PROVED UNMOVED ACROSS THE REFRESH.
#
# The preserved-lab before/during/after proof already covers the whole v10 lab.
# This is a NAMED, IMMEDIATE re-read of the exact files that matter, taken the
# moment the refresh finishes, so a seed that moved is attributed to the refresh
# rather than discovered later with no way to tell what moved it.
assert_seed_unmoved_after_refresh() {
  {
    ...
    printf 'seed_key_sha256                  %s\n' "$(sha_of "$SEED_AUTH_KEY")"
    printf 'seed_marker_sha256               %s\n' "$(sha_of "${V10_CONTINUATION_SEED}/${SHADOW_MARKER_NAME}")"
  } > "${PROOF}/continuation-seed-unmoved.txt"

  expect "seed controller sha256 AFTER the refresh (still Repair 17)" \
         "$(sha_of "$SEED_LOOP")"   "$EXP_SEED_LOOP_SHA"
  expect "seed controller bytes  AFTER the refresh (still Repair 17)" \
         "$(bytes_of "$SEED_LOOP")" "$EXP_SEED_LOOP_BYTES"
  expect "seed journal sha256 AFTER the refresh" \
         "$(sha_of "$SEED_JOURNAL")" "$EXP_SEED_JOURNAL_SHA"
  expect "seed journal head sha256 AFTER the refresh" \
         "$(sha_of "$SEED_JOURNAL_HEAD")" "$EXP_SEED_HEAD_SHA"
  expect "seed authenticity key sha256 AFTER the refresh" \
         "$(sha_of "$SEED_AUTH_KEY")" "$EXP_SEED_KEY_SHA"
  expect "seed v10 shadow marker sha256 AFTER the refresh" \
         "$(sha_of "${V10_CONTINUATION_SEED}/${SHADOW_MARKER_NAME}")" \
         "$EXP_SEED_V10_MARKER_SHA"
  note "the historical v10 seed is unmoved across the controller refresh"
}
...
assert_frozen_identities() {
  ...
  # v10 is this file's DIRECT PREDECESSOR. It RAN once, against the Repair-17
  # controller over the preserved seven-event state; its lab is preserved
  # evidence AND this run's continuation seed. It is gated by identity exactly
  # like v1..v9, and being the seed's author earns it no exemption of any kind.
  ...
  # Launcher v11 is THIS file. It cannot hash-gate on itself, but it must be the
  # file that is present, and it is carried in the before/after proof set so any
  # change to it during the run is caught.
  [ -s "${CTRL}/${LAUNCHER_V11_BASENAME}" ] \
    || die "launcher v11 is missing or empty at ${CTRL}/${LAUNCHER_V11_BASENAME}"
  note "all eleven launcher artefacts present in ${CTRL}"
  ...
  # The controller worktree must be EXACTLY these thirteen entries and nothing
  # else: the modified nh_loop.py, the eleven launcher artefacts, and the one
  # gitignored editor FILE. Nothing is filtered out.
  #
  # NOTE ON __pycache__, RESTATED FOR v11 BECAUSE IT MATTERS MORE HERE. This
  # gate lists no bytecode entry, and none exists on disk. v11 imports
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${LAUNCHER_V11_BASENAME}" | LC_ALL=C sort
  )"
...
step "CONTINUATION SHADOW COPY (SOURCE: THE COMPLETED v10 SHADOW)"

# THE SOURCE OF THIS COPY IS THE COMPLETED v10 SHADOW WORKSPACE, NOT THE REAL
# WORKSPACE. That is the first defining change of v11 and it is stated here, at
# the exact line where it takes effect, so no reader can mistake what is being
# copied.
#
...
# this copy or in the namespace. Event 7 is NOT replaced. The seed journal is
# NOT reset to five events. Real candidate state is NOT copied over the seed.
# The seed is NOT cleaned, normalised or repaired in any way. The whole point of
# v11 is that the seven-event continuation state -- and the unresolved routed
# review signal that is its last event -- survives into this run intact, while
# the CONTROLLER that reads it becomes Repair 18.
#
# EXACTLY ONE FILE IN THE NEW COPY IS TOUCHED AFTER THIS RSYNC, and not until
# the copy has been proved byte-truthful against the seed: controller/nh_loop.py,
# in section 7b immediately below. Nothing else in the new copy is written
...
rsync -aHAX --numeric-ids --delete "${V10_CONTINUATION_SEED}/" "${SHADOW_WS}/" \
  || die "rsync copy of the v10 continuation seed into the new shadow failed"
note "v10 continuation seed copied into ${SHADOW_WS}"

step "CONTINUATION SHADOW COPY VERIFICATION (AGAINST THE v10 SEED, BEFORE ANY REFRESH)"

# THIS VERIFICATION HAPPENS BEFORE THE CONTROLLER REFRESH, NOT AFTER IT.
# The order is load-bearing: refreshing first and verifying afterwards would
# leave the initial copy itself unverified, and the composite proof in section
# 7b would then be comparing a refreshed copy against a seed with no established
# baseline in between. Verified first, refreshed second, re-proved third.
#
# Checksum dry-run compare, AGAINST THE v10 CONTINUATION SEED -- never against
# the real workspace. Comparing the new shadow to the real workspace would be
# wrong on its face: the two legitimately differ, in exactly the six ways
...
# The claim being made here is precise: THIS SHADOW IS, AT THIS MOMENT, A
# BYTE-TRUTHFUL COPY OF THE COMPLETED v10 SHADOW, which is a different and
# stronger claim than "resembles the real workspace".
#
...
set +e
COPY_DIFF="$(rsync -aHAXn --delete --checksum --itemize-changes "${V10_CONTINUATION_SEED}/" "${SHADOW_WS}/" 2>&1)"
COPY_RC=$?
set -e
if [ "$COPY_RC" -ne 0 ] || [ -n "$COPY_DIFF" ]; then
  ...
  die "the continuation shadow could not be proved identical to the completed
      v10 seed (rsync exit status ${COPY_RC}).
      See ${PROOF}/DIFF.shadow-copy.txt
      Refusing to rehearse against a copy that is not proved truthful."
fi
note "the new shadow is a byte-truthful copy of the completed v10 continuation seed"

# ==========================================================================
# SECTION 7b — THE REPAIR-17 -> REPAIR-18 NEW-COPY CONTROLLER REFRESH
#              (NEW IN v11; still under both original locks)
# ==========================================================================
#
...
# marker. It is inside the lock window on purpose, so the real controller that
# is read as the refresh source is the same one the under-lock frozen-identity
...
step "NEW-COPY CONTROLLER REFRESH: AFTER-STATE (must be the exact Repair-18 file)"
assert_new_copy_after_refresh

step "NEW-COPY CONTROLLER REFRESH: COMPOSITE PROOF (exactly one path changed)"
assert_new_copy_differs_only_by_controller

step "NEW-COPY CONTINUATION STATE PRESERVED (seven events, event 7 intact)"
assert_new_copy_continuation_state_preserved

step "HISTORICAL v10 SEED UNMOVED ACROSS THE REFRESH"
assert_seed_unmoved_after_refresh
...
if ! compare_proof_set "${PROOF}/original.before" "${PROOF}/original.during" "during-labs" \
  ...
  die "a preserved earlier rehearsal lab changed WHILE the shadow was being
      copied or refreshed. Preserved rehearsal evidence must never move, and
      that includes the completed v10 lab this run reads its seed from.
      Refusing to rehearse."
fi
note "preserved earlier rehearsal labs unchanged across the whole copy and refresh window"
...
# what is mounted.
#
# ONE THING IS DIFFERENT IN v10 AND v11 AND IS STATED PLAINLY. The seed already
# carries v10's own marker, because the seed IS a v10 shadow, so this write
# OVERWRITES an inherited marker in the NEW lab rather than creating a fresh
# one. Nothing is overwritten in the v10 lab: the target below is ${SHADOW_WS},
# which is the new disposable lab, and the seed itself is never opened for
# writing. The write still happens AFTER copy verification AND AFTER the whole
# section 7b refresh proof, so it can never pollute either comparison -- both
# legitimately saw the v10 marker on both sides and matched it. This is why the
# marker is placed here and not earlier: it is the SECOND authorised difference
# between the new copy and the seed, and the composite proof is only exact while
# it is still the first one that does not yet exist.
#
# It sits at the workspace root, outside both Git checkouts, so neither
# repository worktree status is affected.
#
# The token identifies FOUR things: Repair 18, the v11 continuation, the exact
# Repair-18 controller sha256, and the routed signal this continuation exists to
# carry forward. It therefore cannot collide with any earlier rehearsal marker
# -- including the v10 one, which the seed carried in and which this line
# replaces. If anything ever mounted a preserved shadow here instead of the new
# one, the token check in the self-test would fail rather than pass.
SHADOW_TOKEN="nh-shadow-repair18-continuation-v11-${EXP_LOOP_SHA}-${EXP_ROUTED_SIGNAL_ID}"
...
# ==========================================================================
# SECTION 11 — THE REPAIR-18 REAL TEST SEQUENCE, OVER CONTINUATION STATE
# ==========================================================================
#
...
#
# THE SEQUENCE IS v10's, UNCHANGED -- exactly one call site per stage, in this
# order, each at most once, with no retry, no fallback, no alternate validator
# and no duplicate execute anywhere in this file. What differs is the state it
# starts from AND the controller that reads it: the shadow carries the preserved
# seven-event journal, and its controller is now Repair 18. Stage 1 is a FRESH
# question-validation over a history whose last event is the still-unresolved
# routed review signal rs_2593b06ab1684bbf19a6ffb70d915bd2. Nothing here tells
# the controller what to make of that signal.
#
# THIS IS WHAT v11 IS ACTUALLY TESTING: the substantive routed issue
# authority_control_retry_values_scope, under a controller whose package-identity
# defect is repaired. v10 could not reach it -- its stage 1 fail-closed on
# INCOMPLETE_SOURCE_CHECK because the package source binding was ambiguous
# across the three Authority Integrity candidates. Whether Repair 18 changes
# that is the controller's own result and this launcher neither predicts nor
# influences it.
#
# QUESTION DELIVERY IS NOT PART OF THIS SEQUENCE. next-question-group is never
# called. record-ness-answer is never called. This launcher asks Ness nothing.
# If stage 1 admits a genuine question, the existing gate below stops the
# sequence there and the question is left in the disposable v11 interview state
# for inspection and ordinary delivery afterwards.
#
...
# ==========================================================================
# SECTION 11b — REPAIR-18 CONTINUATION V11 OBSERVATIONS
...
step "REPAIR-18 CONTINUATION V11 OBSERVATIONS"

observe_repair18_continuation_v11() {
  local out="${PROOF}/repair18-continuation-v11-observations.txt"
  ...
  {
    printf 'REPAIR-18 CONTINUATION V11 OBSERVATIONS -- descriptive only, no verdict\n'
    printf '======================================================================\n\n'
    ...
    printf '  refreshed to      : %s  (Repair 18)\n' "$EXP_LOOP_SHA"
    printf '  refresh state     : %s\n' "$CONTROLLER_REFRESH"
    printf '  continuation state: %s\n' "$NEW_COPY_CONTINUATION_STATE"
    ...
    printf '  source apart from six classified differences.\n'
    printf '\n'
  ...
  note "repair-18 continuation v11 observations written to ${out}"
}
observe_repair18_continuation_v11 || true
...
printf '\n'
printf '===========================================================\n'
printf 'N.H SHADOW REHEARSAL (v11, REPAIR-18 CONTINUATION) — RESULT\n'
printf '===========================================================\n'
printf 'SHADOW_LAB=%s\n'                     "$LAB"
printf 'FROZEN_CONTROLLER_SHA256=%s\n'       "$EXP_LOOP_SHA"
printf 'SEED_CONTROLLER_SHA256=%s\n'         "$EXP_SEED_LOOP_SHA"
printf 'CONTROLLER_REFRESH=%s\n'             "$CONTROLLER_REFRESH"
printf 'NEW_COPY_CONTINUATION_STATE=%s\n'    "$NEW_COPY_CONTINUATION_STATE"
...
unset _slug
printf 'REPAIR18_CONTINUATION_V11_OBSERVATIONS=%s\n' "${PROOF}/repair18-continuation-v11-observations.txt"
printf 'CONTINUATION_SEED_COMPATIBILITY=%s\n'      "${PROOF}/continuation-seed-compatibility.txt"
printf 'CONTROLLER_REFRESH_BEFORE=%s\n'            "${PROOF}/controller-refresh.before"
printf 'CONTROLLER_REFRESH_AFTER=%s\n'             "${PROOF}/controller-refresh.after"
printf 'CONTROLLER_REFRESH_COMPOSITE=%s\n'         "${PROOF}/controller-refresh.composite"
printf 'CONTINUATION_SEED_UNMOVED=%s\n'            "${PROOF}/continuation-seed-unmoved.txt"
printf '===========================================================\n'
...
printf '  The shadow was seeded from the completed v10 shadow so the routed\n'
...
printf '  v10 lab was read only -- to be copied and to be authenticated -- and is\n'
...
printf '  controller/nh_loop.py inside the disposable v11 shadow was replaced\n'
printf '  with the current real Repair-18 file, which was opened read-only as\n'
printf '  the source and was never written to. The historical v10 seed still\n'
printf '  carries its own Repair-17 controller. The seven-event journal, its\n'
printf '  head and the authenticity key were not refreshed and are byte-identical\n'
printf '  to the seed. CONTROLLER_REFRESH and NEW_COPY_CONTINUATION_STATE above\n'
printf '  are this launcher stating both facts, each with its own proof file.\n'
