#!/usr/bin/env bash
#
# NH_SHADOW_REHEARSAL_LAUNCHER_v2.sh
#
# N.H SHADOW LIVE REHEARSAL LAUNCHER — v2  (REPAIR-8 CONTROLLER)
#
# WHAT THIS IS
...
#
# ==========================================================================
# WHAT v2 CHANGES FROM v1, AND WHY
# ==========================================================================
#
#   v1 (NH_SHADOW_REHEARSAL_LAUNCHER_v1.sh) is the proven isolation design.
#   It is NOT edited, renamed, deleted or superseded on disk by this file, and
#   this launcher gates on v1 still being byte-identical.
#
#   1. FROZEN CONTROLLER IDENTITY IS THE REPAIRED ONE.
#      Repair 8 is closed. nh_loop.py is now
#        sha256 a52d383fe1a2dd5aa2adc2bf1859020ad73391d44f8c1b6f23db8d4359631a2e
#        bytes  1367891
#        lines  30243
#      v1's frozen controller identity (13ef2b13..., 1354304 bytes, 30002 lines)
#      is the PRE-repair controller and is deliberately not accepted here.
#
...
#
# USAGE
#   bash /home/ness/NH_CLAUDE_CODEX_DESIGN_LOOP/controller/NH_SHADOW_REHEARSAL_LAUNCHER_v2.sh
#
...
# v1 is preserved unchanged. Frozen so this launcher fails closed if v1 moved.
...
# --- controller: THE REPAIR-8 IDENTITY ---
readonly EXP_LOOP_SHA="a52d383fe1a2dd5aa2adc2bf1859020ad73391d44f8c1b6f23db8d4359631a2e"
readonly EXP_LOOP_BYTES="1367891"
readonly EXP_LOOP_LINES="30243"
...
readonly LAB="/home/ness/NH_AICP_SHADOW_REHEARSAL_REPAIR8_a52d383fe1a2dd5aa2adc2bf1859020ad73391d44f8c1b6f23db8d4359631a2e"
...
capture_original_proof() {
  ...
  {
    local lf
    for lf in "$LAUNCHER_V1_BASENAME" "$LAUNCHER_V2_BASENAME"; do
      if [ -f "${CTRL}/${lf}" ]; then
...
if [ "$LAB" = "$PRESERVED_LAB_V1" ]; then
  die "the configured lab path is the preserved v1 rehearsal lab:
        ${LAB}
      That lab is evidence. Refusing to reuse it."
fi
note "new lab path is distinct from the preserved v1 rehearsal lab"
...
assert_frozen_identities() {
  ...
  # Launcher v2 is this file. It cannot hash-gate on itself, but it must be the
  # file that is present, and it is carried in the before/after proof set so any
  # change to it during the run is caught.
  [ -s "${CTRL}/${LAUNCHER_V2_BASENAME}" ] \
    || die "launcher v2 is missing or empty at ${CTRL}/${LAUNCHER_V2_BASENAME}"
  note "both launcher artefacts present in ${CTRL}"
  ...
  ctrl_status_expected="$(
    printf '%s\n' \
      ...
      "?? ${LAUNCHER_V2_BASENAME}" | LC_ALL=C sort
  )"
...
#
# The token is keyed to the REPAIRED controller identity, so it cannot collide
# with the v1 rehearsal's marker: if anything ever mounted the preserved v1
# shadow here instead, the token check below would fail rather than pass.
SHADOW_TOKEN="nh-shadow-repair8-${EXP_LOOP_SHA}"
...
#       -> Claude
#
# NOTE ON THE REPAIR-8 PREPARATION CONTRACT:
#   Nothing here injects, fakes or pre-answers not_ready_reason. --clearenv
#   wipes the environment and the ONLY NH_-prefixed variable set back for the
#   real run is NH_LOOP_INTERVIEW_STATE_DIR, which points at the shadow's own
#   interview state. Whether the real Codex preparation returns the required
#   field correctly is exactly what this rehearsal is for.
build_bwrap_args() {
...
# correction-round limit, its own Codex (gpt-5.6-sol / xhigh / read-only-
# ephemeral where it says so) and its own Claude configuration, and all of that
...
printf '\n'
printf '==================================================\n'
printf 'N.H SHADOW LIVE REHEARSAL (v2, REPAIR-8) — RESULT\n'
printf '==================================================\n'
printf 'SHADOW_LAB=%s\n'               "$LAB"
