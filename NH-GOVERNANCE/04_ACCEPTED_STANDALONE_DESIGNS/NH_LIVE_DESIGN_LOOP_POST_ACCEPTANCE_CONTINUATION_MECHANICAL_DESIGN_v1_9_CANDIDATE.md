# NH_LIVE_DESIGN_LOOP_POST_ACCEPTANCE_CONTINUATION_MECHANICAL_DESIGN_v1_9_CANDIDATE.md
...
**Date:** August 23, 2026 (creation of this v1_9 candidate was explicitly authorized by Ness on August 22, 2026; the work crossed local midnight).
...
**Predecessor:** `NH-GOVERNANCE/05_ACTIVE_CANDIDATE/NH_LIVE_DESIGN_LOOP_POST_ACCEPTANCE_CONTINUATION_MECHANICAL_DESIGN_v1_8_CANDIDATE.md`
...
All three values were independently recomputed from the actual v1_8 file on disk immediately before this file was written, and matched. **v1_8 is preserved byte-identically and is not edited, overwritten, moved, renamed, or re-headed by this file.**

#### 0.1a v1_8 is the currently ACCEPTED standalone predecessor — stated accurately, not diminished

**v1_8 is not merely the previous candidate. It is the currently accepted standalone design for this package**, and this file says so plainly rather than copying forward wording that would imply otherwise.

| Fact | Value |
|---|---|
| Accepted source | `NH-GOVERNANCE/05_ACTIVE_CANDIDATE/NH_LIVE_DESIGN_LOOP_POST_ACCEPTANCE_CONTINUATION_MECHANICAL_DESIGN_v1_8_CANDIDATE.md` |
| Accepted source SHA-256 | `448daa63b4899d4639dc5b7612985cb34e97732dcf4cbeac2c224c54f8ae51e2` |
| Separate closure record | `NH-GOVERNANCE/04_ACCEPTED_STANDALONE_DESIGNS/NH_LIVE_DESIGN_LOOP_POST_ACCEPTANCE_CONTINUATION_MECHANICAL_DESIGN_PACKAGE_COMPLETE_CLOSURE_RECORD_v1_0.md` |
| Closure-record SHA-256 | `940cf3a29b0d41836322e6a55dbe9c56d491e840848d6ec80c2d24b4a9663dd4` |
| Closure-record status | `PACKAGE_COMPLETE` for v1_8's current standalone mechanical-design scope only |

Both identities were independently recomputed from the actual files on disk immediately before this file was written, and matched.

**Therefore, exactly and without embellishment:**

- **v1_8 remains preserved byte-identically**, in its current location, and is not edited, patched, moved, renamed, re-headed, superseded in place, or re-described by this file.
- **v1_8 is the currently accepted standalone predecessor** of this file. Ness explicitly accepted it, and ChatGPT independently audited it PASS, on August 22, 2026. **That historical acceptance did not disappear and is not diminished here.**
- **v1_8's separate closure record remains untouched and byte-identical.** No closure, acceptance, adoption, integration or receipt record is created, amended, superseded or annotated by this file.
- **A later live execution of the accepted design exposed one IMPORTANT mechanical contradiction inside it** (§0.2, §5 P-33). That discovery is a real mechanical defect in an accepted design — **not** a cosmetic note, **not** a wording preference, and **not** one of the two MINOR non-blocking notes the v1_8 closure record already records as explicitly accepted.
- **This file, v1_9, is a NEW CORRECTED CANDIDATE.** It has **not** been accepted by Ness. It is not adopted, not authoritative, not integrated, not implemented, and not `PACKAGE_COMPLETE`.
- **v1_9 does not replace, retire, supersede, or reduce the accepted v1_8 unless and until Ness explicitly accepts v1_9** for this scope. Until that happens, **v1_8 remains the accepted standalone design of record**, and v1_9 is a candidate awaiting an independent audit of the actual file and Ness's own decision.
- **No authority, Master, Map, Defaults, `cursorrules`, Companion or Register integration is performed here**, and none is claimed, implied or anticipated.

**Preserved lineage.** `v1_0` (SHA-256 `6cfe8d07cce712d5fa8077fdb4930a53ac10c52d9cdc02551c15a316e4695ab0`), `v1_1` (SHA-256 `7ba76ef61d736cef07f83f4a814200bbc3776ff031cc4eb40230e7fa4e6843e3`), `v1_2` (SHA-256 `40f217aa018a9fecb4581e0a70004550b22e75234d94543813e7da0f0537f545`), `v1_3` (SHA-256 `78d57db9e4cc434161b05cb2c0945b9c5b8d22a3fb91ebb4e134543e3987701d`), `v1_4` (SHA-256 `7fd023c61fdcaf204c2394c28a15ae0f17c30c0e9ce3cb3c269c8b75490b9067`), `v1_5` (SHA-256 `80284a20da80a87d08617051d7a0207c3ef06bd4f902cfa28619bb1cf7c55f36`), `v1_6` (SHA-256 `f211ffd90a0ffc6732589d7442c925f2bcac7e7e0740bfb4abaea8d0d9852948`) and `v1_7` (SHA-256 `747a045675c4db771e9ac9490343da82fb986bc9bf255aa42cb85b4a432c62c3`) all remain preserved unchanged. Their corrections were carried into v1_8 and are carried forward unchanged here.

### 0.2 The corrections this version applies

**Every substantive mechanic settled in v1_0 through v1_8 is preserved and is not redesigned.** v1_9 is a bounded **actionable-root consistency** correction of one IMPORTANT internal contradiction that a real live execution of the accepted v1_8 design exposed, together with the one bounded transition rule that contradiction makes necessary.

**The contradiction, stated exactly.** The accepted v1_8 design records — correctly — that the installed T1 result validator requires `package_source_path` when `controller_action == prepare_claude_task`, and **permits a null `package_source_path`** for `controller_action == hold_for_question_validation` (§5 P-14). It then separately establishes `SEL-ROOT-IDENTIFIES` (§9.3): a package entering **either** mechanical continuation **or** possible-Ness Piece-3 question validation requires the controller to hold **one exact proved package source root** — and states that `genuinely_open_for_ness` cannot route without one, because `review_signal_recorded` must bind to `scope_root_path`.

Both statements stood in the same accepted design. Together they permit a T1 result that is **validly received** and **mechanically impossible to admit or route** — a result the controller must accept and can then do nothing with. **That is a real internal mechanical contradiction, and it is IMPORTANT, not cosmetic.** It was not hypothetical: it occurred on the real authenticated journal (§3.4, §5 P-33).

| # | Correction | Sections rewritten |
|---|---|---|
| 1 | **`T1-ACTIONABLE-ROOT` — the corrected contract.** `package_source_path` must be non-null and fully proved whenever `controller_action` is **either** `prepare_claude_task` **or** `hold_for_question_validation`. Those two are the **actionable** results; the four non-actionable classifications keep their installed semantics exactly and gain no root requirement invented for symmetry. | §5 P-14, §9.3, **new §9.3a**, §9.4, §9.5, §9.5a, §11.2, §11.5 |
| 2 | **`SEL-LEGACY-CONTRACT-REPLACEMENT` — the bounded earlier-contract transition rule.** One already-completed, still-custody-verifiable, earlier-contract T1 result that is mechanically non-operative under the corrected contract permits **exactly one** fresh current-contract T1 selection, under a **new** provider request identity, with a **fresh** source check. The earlier result is preserved forever as truthful history; its root is never fabricated; its request identity is never re-dispatched; and it never becomes a review signal or a Ness question. | **new §9.7**, §17.2, §18 Rows D / D.1, §19.3 |
| 3 | **`SEL-LEGACY-ONE-SHOT` — no hot re-selection loop.** The one replacement permission is consumed deterministically by the durability of the replacement attempt itself, after which ordinary current-contract result validation, bounded output-retry, provider attempt, reconciliation, backoff and user-action/safety behaviour govern — unchanged. | **new §9.7**, §9.6, §23 |
| 4 | **The fail-closed and rehearsal coverage the above requires**, so the original contradiction cannot pass again: an actionable rootless T1 result is rejected and never becomes an admitted Q, and every affected cross-reference is corrected. | §23, §24, §25 (**new R104–R112**, corrected R3), §27, §28 |

**What v1_9 deliberately does NOT change.** `genuinely_open_for_ness` does **not** become a mechanical design-run classification. It still cannot ask Ness directly, cannot authorize Claude, cannot bypass Piece-3 validation, and cannot become a question composed from `simple_explanation`. The full route — proved root → `review_signal_recorded` → independent question validation → independent question-coverage review → **only if still genuinely open** → `NEEDS_NESS_DECISION` — is preserved exactly (§11). The already-settled routed-signal identity and coverage-stop semantics are preserved and **not** redesigned (§11.5).

Plus the smallest new rehearsals that prove each of the above (§25).

Everything v1_0 through v1_8 got right is preserved unchanged in substance; §27 lists it.
...
### 1.5 The accepted predecessor and the live evidence this correction is derived from *(new in v1_9)*

Two further sources are part of this file's register, and both were read **read-only**.

**(a) The accepted predecessor and its separate closure record.**

| Source | Identity | Standing |
|---|---|---|
| `05_ACTIVE_CANDIDATE/NH_LIVE_DESIGN_LOOP_POST_ACCEPTANCE_CONTINUATION_MECHANICAL_DESIGN_v1_8_CANDIDATE.md` | SHA-256 `448daa63b4899d4639dc5b7612985cb34e97732dcf4cbeac2c224c54f8ae51e2`, 303,820 bytes, 2,826 lines | **the currently accepted standalone design** for this package; preserved byte-identically; the exact correction base of this file |
| `04_ACCEPTED_STANDALONE_DESIGNS/NH_LIVE_DESIGN_LOOP_POST_ACCEPTANCE_CONTINUATION_MECHANICAL_DESIGN_PACKAGE_COMPLETE_CLOSURE_RECORD_v1_0.md` | SHA-256 `940cf3a29b0d41836322e6a55dbe9c56d491e840848d6ec80c2d24b4a9663dd4` | v1_8's separate accepted closure record; **untouched, byte-identical, and not amended by this file** |

**(b) The real authenticated live evidence.** The authenticated interview journal at `nh_interview_state/nh_interview_journal.jsonl` and its head file were read read-only at the position recorded in §3.4, and the exact custodied T1 selection bytes carried by the authenticated `provider_result_custody_recorded` at `event_seq 88` were decoded and re-hashed. **No event was appended, no state was written, no provider was contacted, and no controller was modified by the work that produced this candidate.**

**One boundary stated plainly.** A blocked implementation attempt against the accepted v1_8 design produced a written report of that attempt, and that report contains a recommendation. **That recommendation is not authority and is not policy, and this file does not adopt it.** What that report is used for here is exactly one thing: **evidence that the mechanical contradiction is real and was reached by execution.** The corrective rule below is derived from the accepted design's own mechanics and from Ness's settled decisions, not from that recommendation — and it deliberately does **not** take the route that report recommended.
...
**`SEL-B6D-ACTIONABLE-ROOT-IS-INVALID-OUTPUT`** *(new in v1_9)*. An actionable rootless reply is an **invalid output**, and it takes the installed invalid-output route without exception: a `result_invalid` terminal, the installed **bounded output-retry budget**, and `close-open-consumption` when that budget is exhausted. It is **not** a new selection episode, **not** a new request identity opened at will, and **never** the §9.7 earlier-contract condition — §9.7 applies only to a result that was **valid when received under an earlier contract**, never to one refused under the current one. This is what keeps a model that will not satisfy the contract from opening selector identities without end (§9.7 `SEL-LEGACY-NO-LOOP`).
...
- an **actionable** reply that names no proved root (`T1-ACTIONABLE-ROOT`, §9.3a) → **not** an honest stop and **not** a new selection episode: it is an **invalid output** on the installed bounded output-retry route (`SEL-B6D-ACTIONABLE-ROOT-IS-INVALID-OUTPUT`, §9.5a), and exhausting that budget routes to `close-open-consumption` and then to the ordinary user-action / safety behaviour — never to an unbounded series of fresh selector identities.
...
| terminal `result_invalid` / `result_oversize` — including an **actionable rootless** reply | **fails** | consumed — the installed **bounded output-retry** budget governs, then `close-open-consumption` |
...
| **terminal `result_invalid` / `result_oversize`** | bounded output-retry budget decides: `SAFE_TO_RETRY` while budget remains, else `close-open-consumption` | exceed the budget; silently accept invalid bytes |
...
Expanded from v1_0's ten rows to fourteen, covering the two new phases separately as Correction 3 requires. **In every row: no duplicate provider call where the outcome is uncertain, no duplicate candidate, no skipped package, no lost accepted state, no false success.**
...
**The three positions are kept mechanically apart, and must never be conflated:**
...
| **custody unverifiable** | terminal exists; saved bytes **fail** re-verification | `SAFETY_HOLD`, **zero** replacement calls (`CUSTODY-UNVERIFIABLE-HOLD`) — **§9.7 is unavailable here** |
...
Each is proved from durable evidence, not inferred from the others' absence, and **no failure of one ever grants the permission of another.**
...
`NH_MASTER-20_CORRECTED_v10.md`; either Decision Defaults file; `cursorrules`; the Companion; the Design and Wiring Map; every workflow file; the five-additions decision package; **`v1_0` through `v1_8` of this bridge — including the accepted `v1_8` source and its separate `PACKAGE_COMPLETE` closure record**; every version of the acceptance-button design including v1.14 and v1.15 and its closure record; the AIC and UDOK accepted sources and closure records; every accepted standalone design and closure record; every historical candidate; the real journal, head, key and lock files; **the preserved earlier-contract selection request, its result custody and its terminal**; the real AIC candidate chain.
...
| **R104** | **An actionable rootless `genuinely_open_for_ness` result is refused and never becomes a Q.** *(new in v1_9)* A current-contract T1 reply with `classification: genuinely_open_for_ness`, `controller_action: hold_for_question_validation`, `package_source_path: null`, `whole_check_complete: true` and a full, coverage-passing `source_paths_checked`. Drive it through B6d, then through restart re-admission. | **refused at B6d** — not schema-valid, terminal `result_invalid`, the installed bounded output-retry budget consumed and no more; **never** an admitted selection; **no scope established**; **no `review_signal_recorded` appended**; **no root derived, name-matched or fabricated from `source_paths_checked`, `package_id` or `package_title`**; no question composed; no Claude call; the journal is byte-identical apart from the ordinary invalid-output records |
...
| **R109** | **Unverifiable custody still SAFETY_HOLDs and never gains a replacement call.** *(new in v1_9)* Two fixtures: (a) the **earlier-contract** custody is made missing/corrupt; (b) the **replacement** custody is made missing/corrupt. | in both, `engine.custodied_bytes()` refuses → `provider_result_custody_unverifiable` → `SAFETY_HOLD` → `LOOP_SAFETY_HOLD`; **zero** new provider calls of any kind; **§9.7 is unavailable** because proof 1 fails; every durable record is preserved byte-identically |
...
**What v1_9 settled, and what it deliberately did not open** *(new in v1_9)*:

- **Settled here, mechanically:** the actionable-root contract (§9.3a) and the one bounded earlier-contract transition rule (§9.7). **Neither is a Ness question.** Ness is not asked how internal path or provenance plumbing works, which is exactly what the detailed design-interview decision reserves to mechanical work.
...
- **No new open item is created by this correction.** The earlier-contract condition is closed by construction: it requires validity under a contract no longer in force, so it cannot recur (`SEL-LEGACY-NOT-A-NEW-LEGACY`).
...
**It never modifies an existing file.** The work that produced this candidate created exactly one new file — this one — and left every other file byte-identical, including **`v1_0` through `v1_8` of this bridge and `v1_8`'s separate accepted `PACKAGE_COMPLETE` closure record**, the real journal, the real head, the real authenticated event 90 and the preserved earlier-contract request, custody and terminal, the live controller and worker sources, the real AIC candidate chain, every accepted package, every closure record, every authority file, the Master, the Map, both Defaults files, `cursorrules`, the Companion, the workflow files, the five-additions decision package, and every version of the acceptance-button design. **It started no production process, made no provider call of any kind, and performed no Git operation.**
...
*`NH_LIVE_DESIGN_LOOP_POST_ACCEPTANCE_CONTINUATION_MECHANICAL_DESIGN_v1_9_CANDIDATE.md` — a bounded **actionable-root consistency** correction of `v1_8` (SHA-256 `448daa63b4899d4639dc5b7612985cb34e97732dcf4cbeac2c224c54f8ae51e2`, 303,820 bytes, 2,826 lines), which is preserved unchanged and which remains **the currently accepted standalone design** for this package under its own separate closure record (SHA-256 `940cf3a29b0d41836322e6a55dbe9c56d491e840848d6ec80c2d24b4a9663dd4`), likewise untouched. Also preserved unchanged: `v1_7` (SHA-256 `747a045675c4db771e9ac9490343da82fb986bc9bf255aa42cb85b4a432c62c3`), `v1_6` (SHA-256 `f211ffd90a0ffc6732589d7442c925f2bcac7e7e0740bfb4abaea8d0d9852948`), `v1_5` (SHA-256 `80284a20da80a87d08617051d7a0207c3ef06bd4f902cfa28619bb1cf7c55f36`), `v1_4` (SHA-256 `7fd023c61fdcaf204c2394c28a15ae0f17c30c0e9ce3cb3c269c8b75490b9067`), `v1_3` (SHA-256 `78d57db9e4cc434161b05cb2c0945b9c5b8d22a3fb91ebb4e134543e3987701d`), `v1_2` (SHA-256 `40f217aa018a9fecb4581e0a70004550b22e75234d94543813e7da0f0537f545`), `v1_1` (SHA-256 `7ba76ef61d736cef07f83f4a814200bbc3776ff031cc4eb40230e7fa4e6843e3`) and `v1_0` (SHA-256 `6cfe8d07cce712d5fa8077fdb4930a53ac10c52d9cdc02551c15a316e4695ab0`). **Corrected here:** a real live execution of the accepted v1_8 design exposed one IMPORTANT internal mechanical contradiction — a T1 selection result could be **validly received** and **impossible to admit or route**, because the result contract permitted a null `package_source_path` for `hold_for_question_validation` while `SEL-ROOT-IDENTIFIES` required one exact proved root for that same route. v1_9 makes the contract require a proved root for **both** actionable actions (`T1-ACTIONABLE-ROOT`, §9.3a), leaves the four non-actionable classifications unchanged, and adds the one bounded, deterministically-consumed earlier-contract transition rule that a strengthened contract makes necessary (`SEL-LEGACY-CONTRACT-REPLACEMENT`, §9.7) — under which the already-completed earlier-contract result is preserved for ever, its root is never fabricated, its request identity is never re-dispatched, it never becomes a review signal or a Ness question, and **exactly one** fresh current-contract selection may be taken under a **new** request identity with a **fresh** frontier check. **`genuinely_open_for_ness` remains evidence only: it still cannot ask Ness, still cannot authorize a design run, and still routes only through the full independent Piece-3 validation and coverage path.** **No continuation architecture is redesigned, no settled user-facing meaning is changed, the post-acceptance decision is not reopened, and every substantive mechanic of v1_0 through v1_8 is preserved.** Status: `CANDIDATE — NOT ACCEPTED — NOT ADOPTED — NOT AUTHORITATIVE — NOT INTEGRATED — NOT IMPLEMENTED`. **It does not replace the accepted v1_8 unless and until Ness explicitly accepts v1_9.** Creates exactly one new file — itself — and modifies nothing. Makes no architecture change, performs no implementation, appends no journal event, runs no provider, mutates no Git, creates no closure or acceptance record, and authorizes no building.*
