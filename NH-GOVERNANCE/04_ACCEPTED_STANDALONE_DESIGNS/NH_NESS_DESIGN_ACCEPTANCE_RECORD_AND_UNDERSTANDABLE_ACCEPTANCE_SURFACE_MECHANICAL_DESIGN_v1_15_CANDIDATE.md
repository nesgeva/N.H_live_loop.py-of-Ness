...
2. **Every ordering check is restated in terms of journal position — never in terms of an operation identity, which the records do not carry and recovery would not preserve.** The relation v1_3 proved is one the append-only authenticated chain always witnesses: **the bound explanation record's `event_seq` is strictly less than the bound pass event's**. v1_3 additionally required that **no** `mechanical_pass_recorded` **in the package scope at all** sit at or below the explanation record's position; **v1_15 narrows that second clause to the PASS this explanation actually binds** (§11.3 gates 11b, 11c1). The v1_3 form is preserved here as provenance, not as the current rule: a `mechanical_pass_recorded` bound to a superseded audit and carrying a different `pass_identity` is preserved history, and its position is evidence about nothing this explanation claims. An `event_seq` **is** the record of write order; it is invariant under re-entry, readable from every event, and true in every arrangement a crash can produce. The distinctness of the two operations remains a **write-order contract** of this design, discharged by the offline sequence and crash tests of §21 — **not asserted as a lock-time check of something the installed records cannot witness.**
...
**Version note (v1_15) — bounded narrowing of gate 11b(ii) on Ness's explicit decision, after a live rehearsal proved the v1_3 form unreachable.** v1_15 was created from the exact unchanged `NH_NESS_DESIGN_ACCEPTANCE_RECORD_AND_UNDERSTANDABLE_ACCEPTANCE_SURFACE_MECHANICAL_DESIGN_v1_14_CANDIDATE.md`, SHA-256 `c4f055f271cbeea246291986b1b11367fcfb1748466f346e642176df2b36b46a`, 401,581 bytes, 2,324 lines. **Its actual predecessor is that file, stated exactly and not inferred from the filename, and that file remains byte-identical.**

**Why this version exists.** A bounded live rehearsal of the real loop ran the full route this design specifies over a disposable copy of a real authenticated history: parts 1–6 prepared as preparation only, a content-stage record fixed, one independent audit that read the exact fixed bytes and recorded the digest it covered, A5's final explanation, A5's completion, A6's fresh PASS, and A6's completion. Every other gate passed. **The offer was still refused, by gate 11b(ii) alone**, because that package scope already contained a `mechanical_pass_recorded` written **before this design's explanation machinery existed** — bound to a superseded audit, carrying a different `pass_identity`, and preserved as authenticated history that may never be deleted. Every explanation appended afterwards necessarily sits above it, so the v1_3 clause refuses **every reachable arrangement** of the very route that prepares, audits and binds fresh prose for that same candidate. **A gate that cannot be satisfied by any correct history is corrected, not worked around by manufacturing a replacement candidate to escape it.**

**The decision this version implements is Ness's, quoted exactly:** *"Narrow clause 11b(ii) so historical unrelated PASS records do not block acceptance. The required ordering applies to the PASS bound to the current explanation/audit chain. Preserve all other acceptance safety gates."*

**What v1_15 changes, and nothing else.** Gate 11b(ii) now asks whether a `mechanical_pass_recorded` **belonging to this bound chain** sits at or below the bound explanation record; a PASS carrying neither the bound `pass_identity` nor the bound `audit_identity` is unrelated preserved history and does not fail the clause, whatever its position. New gate row 11c1 records the withdrawn v1_3 form and why it is withdrawn, in the same way §11.3 gate 11c already records the clause v1_3 itself withdrew. §0's v1_3 ordering summary and §25's delivery statement are updated to match, and §21 gains one test that proves the new rule. **No other statement, gate, test, mechanic, identity rule, recovery rule, or policy is changed.**

**What v1_15 deliberately does not weaken.** Stale-offer rejection, the complete-head and tail binding, the candidate and source binding, the audit binding, the explanation-content digest binding, the A5/A6 completion proof, and the requirement that the bound explanation precede **its own** bound PASS all stand exactly as v1_14 wrote them. **An unrelated historical PASS satisfies neither gate 10, gate 11 nor gate 11a for the fresh chain and can never be substituted for the bound one.** What v1_15 gives up, and says so plainly rather than presenting the change as a strengthening, is the stronger statement that a candidate which has ever passed may never afterwards have prose prepared for it.

**Standing.** Every earlier candidate, including v1_14 and interrupted v1_5, remains unchanged. v1_15 performs no implementation and claims no acceptance and no adoption.
...
| 11b | **the ordering contract held, proved from journal position alone (rewritten in v1_3; clause (ii) narrowed in v1_15)**: (i) the bound final-stage explanation record's `event_seq` is **strictly less than** the **bound** pass event's `event_seq`; (ii) **no** `mechanical_pass_recorded` **belonging to this bound chain** sits at an `event_seq` **at or below** the bound explanation record's — that is, no scoped `mechanical_pass_recorded` carrying the bound `pass_identity`, and none carrying the bound `audit_identity`, sits at or below it. **A scoped `mechanical_pass_recorded` that carries neither the bound `pass_identity` nor the bound `audit_identity` is unrelated preserved history: it does not fail this clause, whatever its position, and it is never treated as evidence about prose it does not bind** (§11c1); (iii) the bound explanation record's `event_seq` is **strictly greater than** both the bound audit event's and the named content-stage record's, so the prose binding genuinely followed the evidence it names. A final record at or after the **bound** pass event's position **contradicts §7.1a and is refused**, never accepted as an equivalent arrangement. **Every clause is an `event_seq` comparison over events already required to exist by gates 10, 11, 12 and 12b, and is therefore evaluable in every arrangement a crash can leave** (§7.1b) |
...
| 11c1 | **the v1_3 every-PASS clause narrowed, recorded rather than dropped silently (v1_15)**: v1_3's gate 11b(ii) required the bound explanation to precede **every** `mechanical_pass_recorded` in the package scope, *"not merely the one it binds"*. **That form is withdrawn and replaced by the bound-chain form above, on Ness's explicit decision**, and the reason is a fact about a real authenticated history rather than a preference. Where a package already carries a PASS written **before this design's explanation machinery existed** — bound to a superseded audit, carrying a different `pass_identity`, and preserved as history that may never be deleted — every explanation appended afterwards necessarily sits above it. **The v1_3 clause therefore refuses every reachable arrangement of the very route that prepares, audits and binds fresh prose for that same candidate**, from a state whose only design-conformant escape would be to manufacture a replacement candidate solely to get out from under a preserved record. **The safety the clause reached for is not lost, because it was never carried by that clause alone:** gate 10 proves the exact final usable audit; gate 11 proves exactly one scoped PASS carries the bound `pass_identity`, that it **is** the current pass event for this candidate, and that its `audit_identity` is the bound audit; gate 11a proves that identity recomputes from the controller's own current inputs; gate 11b(i) proves the explanation was durable before it; gate 11b(iii) proves the explanation followed both the audit that read it and its own content stage; gate 11d proves the effect-owning A5 and A6 pairs; gates 12–12e prove the explanation's identity, content digest, derived part 7, fixed part 8, and the absence of any forward pass reference. **An unrelated historical PASS satisfies none of gates 10, 11 or 11a for this chain, is never bound, is never current, and can never be substituted for the bound one.** **What v1_15 gives up, stated plainly rather than presented as a strengthening, is the stronger statement that a candidate which has ever passed may never afterwards have prose prepared for it.** No other binding is loosened and no other gate is relaxed (§11.3 gates 10, 11, 11a, 11b, 11d, 12–12e; §21 T-13a1c) |
...
| T-13a1c | **Unrelated historical PASS test (v1_15):** build a disposable history in which a `mechanical_pass_recorded` bound to an **earlier, superseded** audit already exists and is preserved, then run A2→A7 for that same candidate so the content-stage record, the fresh audit that reads the exact fixed bytes, the final explanation and the fresh bound PASS all land at higher `event_seq` values than it | gate 11b(ii) does **not** fail on the historical PASS and the offer is actionable once every other gate passes; the historical PASS carries neither the bound `pass_identity` nor the bound `audit_identity`, satisfies neither gate 11 nor gate 11a, is never bound, never current, and never substituted; it remains byte-identical in the journal and is neither deleted, edited, reinterpreted, nor reaped; and clause (i) still fails closed if the final explanation is placed at or after its **own** bound PASS |
...
This candidate creates **exactly one new file — itself** — at `NH-GOVERNANCE/05_ACTIVE_CANDIDATE/NH_NESS_DESIGN_ACCEPTANCE_RECORD_AND_UNDERSTANDABLE_ACCEPTANCE_SURFACE_MECHANICAL_DESIGN_v1_15_CANDIDATE.md`.

Its exact unchanged source is `NH_NESS_DESIGN_ACCEPTANCE_RECORD_AND_UNDERSTANDABLE_ACCEPTANCE_SURFACE_MECHANICAL_DESIGN_v1_14_CANDIDATE.md`, SHA-256 `c4f055f271cbeea246291986b1b11367fcfb1748466f346e642176df2b36b46a`, 401,581 bytes, 2,324 lines.
...
v1_15 performs no implementation, no controller change, no UI change, no test change, no real journal append, no provider call, no Git operation, no Master or Map integration, no Register assignment, no `PACKAGE_COMPLETE`, no closure, and no acceptance record. **The controller correction that this narrowed gate unlocks is deliberately NOT performed here and is not claimed to have been performed anywhere.**
