# N.H — DECISION DEFAULTS (S19) — v2.3
# Behavioral guide for N.H design and build sessions.
#
# AUTHORITATIVE — ADOPTED BY NESS 2026-08-13.
#
# Supersedes NH_DECISION_DEFAULTS-S19_v2_2.md and all prior Decision Defaults
# versions. This is the active behavioral guide from the date above.
#
# ===================== ADOPTION RECORD — READ FIRST ======================
#
# ADOPTION. Ness explicitly adopted this file on 2026-08-13. It becomes the
# authoritative Decision Defaults on that adoption, not by its filename.
#
# WHAT THIS ADOPTION CHANGED — ONE BOUNDED RULE ONLY.
# It changes only the §0 live-loop working-role rule already reviewed in the
# adoption source candidate. Every other rule is carried through from v2.2
# unchanged. The reviewed change replaced v2.2's §0 sentence — "Cursor writes
# code; Claude designs and audits; Ness runs commands and verifies on disk.
# This division is not renegotiable." — with the revised working division and
# its supporting bullets in §0 below. The reconciliation follows Ness's
# approved decision recorded in:
#   NESS_DESIGN_INPUTS/NH_LIVE_LOOP_DETAILED_DESIGN_INTERVIEW_DECISION_v1_1.md
#   SHA-256 00160bff6187784ebfc8ad6babbf8f190b8ac5be9fc1711d1023a835d2620551
#   1035 lines, 45870 bytes
#
# PRESERVED PREDECESSOR — HISTORICAL PRIOR AUTHORITY.
# NH_DECISION_DEFAULTS-S19_v2_2.md remains preserved unchanged as the prior
# adopted authority (adopted by Ness June 28 2026). It is history, not a
# competing current guide.
#   SHA-256 6cd09329e12ba9de78b96d02347a765b65191ec6f7050f62f71d4a831baee696
#   314 lines, 37048 bytes
#
# PRESERVED ADOPTION SOURCE — THE REVIEWED CANDIDATE.
# 05_ACTIVE_CANDIDATE/NH_DECISION_DEFAULTS-S19_v2_3_CANDIDATE.md remains
# preserved unchanged as the exact reviewed source of this adoption. Everything
# in this file from the first "---" separator onward is byte-for-byte identical
# to that candidate; only this header differs.
#   SHA-256 87003365532eee78cb7388be9beccccc4950bfaaf8074749dafad6b146e4e367
#   355 lines, 41782 bytes
#
# NO MASTER RESYNCHRONIZATION WAS PERFORMED.
# This adoption performed NO new Decision-Defaults-to-Master resynchronization.
# The Master-19 "Synced to" and Master-hash lines below are CARRIED HISTORICAL
# LINEAGE ONLY — they record what this file was last synced against, and they
# are not a claim of currency against Master V10. The Master-19 -> Master V10
# drift remains recorded where it already is (Map Register D; regeneration
# task C5) and is untouched by this adoption.
#
# HIGHEST AUTHORITY.
# NH_MASTER-20_CORRECTED_v10.md remains the highest governing N.H authority and
# wins every conflict with this file. This file is the behavioral guide; it is
# not architectural authority and does not modify Master or the Map.
#
# =========================================================================
#
# Carried historical lineage from v2.2 (NOT a current sync claim — see above):
...
# Adoption lineage: v2.3 (adopted 2026-08-13) <- v2.2 (adopted June 28 2026)
#   <- v2.1 and all prior Decision Defaults <- NH_DECISION_DEFAULTS-S17_AUDITED_v1.md
#
...
- **The working division — revised by Ness 2026-08-13.** **Ness** owns concept, meaning, policy, permissions, privacy choices, priorities, genuine user-facing trade-offs, acceptance, adoption, and permission to build; he runs commands and verifies on disk, and continues to run or authorize every protected action. **ChatGPT/GPT** inspects the real current project, classifies the consequential design points, finds the genuine Ness choices, and — once meaning is settled — determines the exact bounded mechanical design or correction specification. **Claude** applies and drafts that specification; it does not independently choose a different N.H design. **Cursor** writes code only in the later, separately authorized implementation phase. **This division is not renegotiable by any AI role — only Ness may change it, and only explicitly.**
- **No AI role may decide a genuine Ness meaning/policy choice.** A choice belongs to Ness whenever two or more materially different N.H behaviours remain compatible with the settled sources *and* choosing between them changes what N.H means, does, permits, protects, remembers, surfaces, prioritizes, or feels like in real use. A mechanical-only choice — one where only the internal mechanism differs and no user-facing meaning or policy consequence changes — does not go to Ness.
- **Claude stops rather than deciding.** If a supplied specification is ambiguous, contradictory, incomplete, or requires another consequential design choice, Claude stops instead of deciding it, and the issue returns to ChatGPT/GPT — and, if a genuine Ness choice is found, to Ness. Stopping is the correct outcome, not a failure.
- **A single completeness claim does not unlock Claude.** A fresh, independent ChatGPT/GPT coverage check must look for important Ness choices the first pass may have missed before Claude begins design work. It is a coverage challenge, not proof that nothing was missed.
- **The controller owns the live-loop safety envelope — never model-generated design content.** A design specification may determine bounded design content only. It may never widen file access, tool permissions, target paths, authority order, correction count, Git permissions, implementation permission, or acceptance/adoption.
- **Provenance of this bullet group.** v2.2's adopted §0 rule read: *"Cursor writes code; Claude designs and audits; Ness runs commands and verifies on disk. This division is not renegotiable."* That wording is preserved here as history and is not withdrawn as a record of the earlier approved role. Ness explicitly revisited it for the live design loop on 2026-08-13; the full record is `NH_LIVE_LOOP_DETAILED_DESIGN_INTERVIEW_DECISION_v1_1.md`, which governs the live-loop working role only and claims no architectural authority. Master V10 remains highest authority and wins every architectural conflict.
